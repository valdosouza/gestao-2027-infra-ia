...
| 2026-09-04 | Regras de negócio nascem como DADOS (catálogo estruturado: id, classe, módulo, etapas, evidência, status) e viram documentos por geração — YAML consistente, índice automático, checagem programática de ids órfãos citados pelos processos; editar o .md gerado à mão é proibido | caso Gestao2016 (F5, 146 regras) |
| 2026-09-04 | Depois do processo piloto, mapear o seu INVERSO (cancelar/estornar) antes de trocar de domínio: custa ~1/4 do piloto e testa se o modelo de estados fecha (achou bug de atomicidade invisível na leitura direta); ao mapear um processo IRMÃO, a tabela de diferenças contra o já mapeado é o artefato mais útil (≈60% de reaproveitamento medido); módulos que emergem ficam "propostos" até o dono validar | caso Gestao2016 (P2/P3) |
| 2026-09-04 | "Pergunta ao autor" nasce separada em FATO (o código responde — 4 de 8 pendências fecharam por grep em 1 h; a legenda `0..5` no comentário valia mais que a memória do dono) × INTENÇÃO (só o dono decide); e toda decisão nova passa antes pela tabela de decisões vigente do destino (evitou uma decisão redundante). Aplicado ao kit 0.4 | caso Gestao2016 (sessão 3) |
