...
**Versão**: 8.1 (acompanha a tabela "Histórico de Atualizações")  
...
| **Gestao2016** | 7 + governança | — | — | 📖 Aprendizado do legado sob instrução do AUTOR · **Investigação de Conhecimento ENCERRADA (Decisão 65, 2026-09-04)** — 6 processos F0–F6, 15 módulos, 251 regras; workspace em `D:\Gestao2016\Infra-IA\Gestao\` (comece por `INDEX.md`) |
...
### "Quero entender como uma regra de negócio funciona no legado antes de implementar"
```
1. Abra D:\Gestao2016\Infra-IA\Gestao\INDEX.md (FAQ → documento; navega por processo/módulo/regra)
2. Ache a BusinessRule (05-regras/<ID>.md — regra + evidência arquivo:linha) ou o Process
   (04-processos/proc-*.md — etapas, pontos de decisão, estados)
3. Ao consumir a regra na implementação, preencha o campo `web:` dela (fica "pendente" até então)
```
**Exemplo**: baixa parcial → `05-regras/BX-10.md` (título residual) + `04-processos/proc-baixa-financeira.md` §B04
...
| Data | O quê | Versão |
|------|-------|--------|
| 2026-09-19 | **Primeiro Cliente — Rodada 5 DECIDIDA e EXECUTADA (D23–D29)** (`prompts/prompt_primeiro_cliente_setes.md` §11; Valdo: "siga as recomendações, P0 e ondas eu respondo depois"). **D23** condições de cobrança (dia + forma) CONGELADAS no fato da competência (`tb_contract_item_competence`, migration 054 + backfill; lote lê do fato, sem JOIN com o contrato — editar/excluir contrato só governa meses futuros, princípio da D-A36); **D24** MANTER 200/`ok:true` com tudo recusado (conhecimento negativo); **D25** contenção ganha UMA passada extra ao final do lote, resto volta `retryable`; **D26** cobrança POR PARCELA (`chargedParcels/chargeableParcels`, `uncharged` conta a parcial, `partiallyCharged`; `chargeable` nasce na composição title-automation e vale para OS e venda); **D27** teto **50** ordens/requisição — o app fatia em blocos sequenciais e AGREGA os relatórios (bloco que falha depois de outro faturado vira linhas `BATCH_ABORTED`, nunca Failure); **D28** MANTER baixa automática bloqueando o cancelamento (não é cascata como o cheque); **D29** MANTER `auto_bank_slip` na interface billing (limite registrado no código). Trilha: P6c confere contra o FATO, **P6d** novo (cobrança por parcela), e a régua passou a **limpar o próprio resíduo** (17 contratos sem forma do cliente da trilha recusavam o lote — comportamento certo do produto). Gates: socrático **0.72 ✅** (H1 duplo disparo do lote no app, M1 TOCTOU das condições → reconferência DENTRO da transação com FOR UPDATE, M2 mensagens sob D23 — corrigidos em sessão) · adversarial **0.78 ✅** sem HIGH (2 lotes paralelos = 5 notas, zero 500; fatos divergentes na mesma OS e custo 22 s/ordem sob trava → Q-R5.1/Q-R5.4). **897/897** jest · **67/67** app + analyze limpo · trilha **17 OK · 7 PENDENTE · 0 FALHA**. **D30** (P0.1): cobrança recorrente = 1 parcela sempre, parcelamento só no faturamento individual (lote sem campo de parcelas; trilha 18 OK). **D31/D32**: teto do desconto só para quem não tem o privilégio (default 0 vale); FATURAR/CANCELAR por usuário, só não admin — **P0 FECHADO**, trilha 20 OK · 4 PENDENTE (= ondas 2/3/4). **D33**: ordem das ondas = Inter → NFS-e → Produção (webhook só na Onda 4; polling faz a ponte). **Onda 2 EM EXECUÇÃO** (`prompts/prompt_onda2_banco_inter.md`, D-I1…D-I19 decididas; spec oficial em `setes-api/integracoes/banco-inter/`; peças secret-store/bank-channel/bank-slip-registration + migration 055 + módulos + app entregues; retomar pelo §9 — faltam credenciais do sandbox, smoke e gates). Q-R5.1…Q-R5.6 (§11) podem esperar. | 8.1 |
| 2026-09-20 | **Onda 2 (Banco Inter) — GATES executados com retrabalho em sessão** (`prompts/prompt_onda2_banco_inter.md` §10; sem credenciais do sandbox ainda). Testes do app das entidades novas (92/92). Sondas ao vivo no dev (conta 077 descartável + PEM autoassinado, removidos no fim): chave PEM com corpo lixo era aceita, handshake mTLS recusado virava 503 "indisponível", `clientId` aceitava traversal. **Socrático 1ª rodada 0.64 ✗** — HIGH-1 efeito recusado por causa TRANSITÓRIA virava pendência permanente; HIGH-2 throttle pelo último evento = starvation (os 8 mais antigos sempre, o 9º nunca); HIGH-3 tentativa interrompida virava F sem consultar o banco (2 cobranças vivas no Inter). **Adversarial 1ª rodada 0.62 ✗** (64 testes, 11 achados): A1 HIGH desfecho ambíguo do POST fechava em F; A2 registrar × cancelar; A3 voz atrasada → UNIQUE 500; A4 cancelar antes de validar local; A5 voz na tentativa errada; A6 token do webhook a usuário comum; A7 chave ilegível. **Retrabalho**: migration 056 `last_queried_at` (o fato "nós olhamos o banco", distinto da voz dele — D-I20 assunção); recusa de REGRA × transitório (`isRuleRefusal`); reconciliação por seuNumero ANTES de F e antes de nova apresentação (tentativa sem código); ambíguo fica em voo (D-I21); cancelar = reserva em voo bloqueia → estado local sob lock → consulta → pedido → C+K (D-I22); rotina com orçamento 20 s + coalescência por institution; webhook teto 50 + fila serial (D-I23); `validatePrivateKeyPem` + par cert×chave; GET do canal redigido p/ não admin; TLS no handshake = 409 BANK_AUTH_FAILED (D-I24); token fora da retentativa. `onda2-gate-rework.test.ts` (24) + adversarial (64) verdes — **1036/1036** api · **92/92** app · re-prova ao vivo V1–V8. Palavras ocupadas (registro/canal/apresentação/integração/segredo) na skill do guardião. Q-I1…Q-I13 para o Valdo (§10.6); **re-score socrático 0.76 ✅** (M1 cofre não atômico, M2 falha persistente pré-voz e L1 corrigidos em sessão; E1 consistência do GET por seuNumero vai para o smoke); 1039/1039; nada commitado. | 8.1 |
| 2026-09-20 | **Onda 3 (NFS-e pelo ADN) — Rodada 0 ORGANIZADA** (`prompts/prompt_onda3_nfse_adn.md`, aberta enquanto o sandbox do Inter está fechado — janela seg–sex 8h–20h). Levantamento do que existe (fase-mãe D1/A2, `tb_invoice_service` mínimo, T/A/R/D/I reservados em `tb_invoice_event`, `secret-store` owner `establishment` só PEM, legado ACBrNFSeX por provedor municipal) + parecer do guardião: **4 conceitos** — habilitação do emissor `tb_establishment_issuer` (PK institution × authority; .pfx+senha convertidos para PEM no upload, senha nunca persistida) · transmissão do DPS `tb_invoice_service_transmission` (1 ramo × N `attempt`, write-once chave/número/protocolo, `last_queried_at`) · voz do fisco `_transmission_event` (S/A/R/C/K/F, append-only) · efeito na nota só E/C (**liberar a reserva T/A/R/D/I**, espelho da D-I5); ramo de serviço cresce por COLUNAS (um DPS = um serviço); cancelamento = plano local → fisco → voz → C na mesma transação (C local só com voz). **Contrato oficial LEVANTADO e guardado em `setes-api/integracoes/nfse-adn/`** (manuais, Anexos I/II, NTs 007/008/009, Res. 3/2023, FAQ, manual de Curitiba): emissor próprio fala com a **API da Sefin Nacional** (não "direto ao ADN"), mTLS e-CNPJ + XMLDSig, sem procuração; **Curitiba CONFIRMADA** conveniada (obrigatório desde 01/01/2026; cancelamento 60 dias, nunca após o ISS recolhido); **API de DANFSe suspensa (NT 008) → PDF é nosso**; IBS/CBS obrigatório em 01/10 ou 01/12/2026; produção restrita existe; swagger/XSD só com certificado. Q-N1…Q-N16 aguardam o Valdo. | 8.1 |
| 2026-09-08 | **Cancelamento de nota — prompt ABERTO, Rodada 1** (`prompts/prompt_cancelamento_nota.md`) — Valdo escolheu "cancelamento" como próximo processo. §2 fatos verificados: legado C00–C05 + CAN-V1..V7/H1/01..06 (pendente apaga × autorizada marca C; nenhum título baixado — Decisão 24; justificativa; cStat 135) e web (toda nota é "pendente": status '0' sem transmissão; efeitos do invoice: nota + ramos, vínculos/snapshots fiscais, tb_financial/_bills, comissão imutável, baixa por contrato, cheques R, boletos automáticos, devoluções, pedido 'F'; sem estoque). ⚠️ conflito D7 do contrato (recusar com baixado) × decisão #15 da fase (estorno em cadeia) → Q2. §3 modelo proposto: `tb_invoice_event` append-only (estado derivado) + peça `@shared/invoice` que COMPÕE settlement/bank-slip/check/commission/order-return/order; tela = ação na aba Faturados. §4 Rodada 1 = 17 questões com recomendação (escopo só não-transmitida, numeração reaproveitada, pedido volta a 'A', soft-delete do financeiro aberto, compensação de comissão, X automático de cheque em custódia, C automático de boleto aberto, devolução vigente bloqueia, motivo obrigatório, privilégio próprio, eventos). Parecer setes-conceito depois da Rodada 1. | 8.0 |
| 2026-09-08 | **Passeio logado do módulo Cheques — visual VALIDADO** (`prompts/prompt_cheque_rastreabilidade.md` §9) — Valdo: "passeio curto o modulo de cheques". Lista por estado (6 abas) + busca; detalhe com cabeçalho imutável, chip de estado, linha do tempo e 5 ações; Depositar (lookup de contas) → "No banco", código 107; Estornar com motivo obrigatório → código 108 espelhando 107, volta à custódia. Evidência: eventos R/B/X do cheque 42 e extrato 106/107/108 conferidos. **Q-CH1 aberta**: estorno do cheque (e dos satélites do boleto) grava espelho sem `tb_financial_statement_id_origin`, sem status 'R' e sem herdar dt_record — diverge do núcleo settlement-batch (D-G3); **D-CH1 EXECUTADA no mesmo dia ("Q-CH1 vai", §10)**: peça `@shared/financial-settlement/statement-reversal` (`reverseStatementLines` + `mirrorStatementLine`: espelho 'R' com origem, dt_record herdado, dt_original = fato gerador, original 'E'); `insertStatement` com `status`/`originId` opcionais; `reverseStatementOnly` do cheque removida; +4 testes → 650/650; smoke no dev com depósito retroativo (109) e estorno (110) conferidos. **Q-CH2 ABERTA** (achado sistêmico): saldo e conferência do caixa somam `status IN ('N','R')` e excluem as originais 'E' → estorno total conta uma vez só (caixa 5 do dev: −871,00 pelo filtro × 278,00 somando tudo); bug pré-existente do núcleo → **D-CH2 EXECUTADA no mesmo dia ("Q-CH2 vai", §11)**: filtro de status removido do saldo e da conferência do caixa (soft-delete mantido — pergunta do Valdo respondida antes do vai); REGRA: saldo = todas as linhas `deleted='N'`, status informativo; teste do caixa proíbe filtro de status; `GET /api/cashier/5` = 278,00; 650/650. Lições: janela da extensão pode nascer oculta (rAF não dispara → Flutter não sobe); Valdo negou acesso de desktop ao Chrome. | 8.0 |
| 2026-09-09 | **Cancelamento de nota — Rodada 3 DECIDIDA e EXECUTADA** (`prompts/prompt_cancelamento_nota.md` §10.9) — D-G7a: cheque que TRANSITOU não interfere em momento algum (peça `reversePaymentWithChecks`: Baixas desfaz a baixa, cancela os cheques em custódia e mantém os que transitaram; no cancel quem bloqueia é a BAIXA, pelo título); **D-G11: CICLO da OS em tabela própria `tb_service_order`** (parecer setes-conceito — natureza por PRESENÇA × processo por ATO; migration 047; fecha o vazamento venda-com-serviço → módulo de OS e a fragilidade N-1/Q-G18; sync não toca — coluna `open_lock` fica até o sync mudar, Q-C4); D-G16 "Cancelar nota" no documento faturado da OS (app + guard por lista de interfaces + seed 52); D-G17 REPEATABLE READ fixado por conexão + assert no boot + PADROES_BANCO §9; D-A7 teto da baixa manual (TITLE_EXCEEDS_BALANCE); D-A8 validate acusa ordem sem cobrança; D-A9 registrado; Q-G9a explicada (§10.9.2). 715/715 api · 31/31 app; smoke 12/12 do ciclo + corridas sem 500. Socrático 0.66 → H1 (teto × boleto agrupado com juros) e H2 (cadeia PA pela peça do cheque) corrigidos em sessão, 718/718; Q-G19…Q-G24 abertas (§10.10). Adversarial 0.55 → HIGH 2 (OS pelo billing de venda → 409 SERVICE_ORDER_OWN_ENDPOINT) e HIGH 3 (deadlock do MAX+1 → `lockInstitutionCounters`, regra 7 do PADROES §9, também em orders) corrigidos e re-provados com 6 concorrentes sem 500 (§10.11); 722/722. Re-score socrático 0.77 ✅ (N1 → migration 048; N2 → devolução com lock + retry). **Rodada 4 EXECUTADA (§10.12: Q-G19…Q-G24 + Q-A17b "Rec.")** — saldo em aberto como peça única `title-balance` (desconto quita), cheque mantido que volta gera CH, X do R órfão, CANCELAR pelo ramo (`requirePrivilegeFor`), FATURAR na OS (seed 53), exceção nomeada `/api/billing/*`, item de OS sem valor → 422. 728/728. Re-prova adversarial 0.62 → Q-A18 (regra 7 dentro de `nextSettledCode`) e Q-A20 (guarda única do produto no PUT + cinto no faturamento da OS) corrigidos, 731/731 api · app 31/31. Re-score socrático FINAL 0.79 ✅ (A1 → migration 049; L1/L2/L3/L5 baixados; Q-G26…Q-G29 abertas, §10.14). 732/732. **Re-prova adversarial FINAL 0.62 → 1 HIGH corrigido em sessão (§10.15)**: P de cheque contra título a RECEBER (2º crédito no caixa) → 422 CHECK_TITLE_NOT_PAYABLE (Q-A24); lookup a pagar → `OPEN_BALANCE_SQL` (Q-A26); float × DECIMAL → peça `@shared/money` + regra "o que valida é o que grava" (`setes-api/02-VALIDACAO.md`, Q-A27). 740/740 · ao vivo 12/12. **Rodada 5 DECIDIDA e EXECUTADA (2026-09-10/11, §10.16)**: D-G28 desconto sobre o SALDO em aberto + `discount_value` gravado (migration 050, regra BX-10 do legado); D-G27 peça `@shared/service-product` (contrato valida, rotina pula e reporta); D-G29 privilégio do ramo na devolução (seed 54 + resolver único + gates FATURAR nas telas); D-A23 `FOR UPDATE WAIT 10` no lock da institution; D-G26/D-A22/D-A25 registradas. 753/753 · ao vivo 16/16. Q-G30 aberta (boleto com desconto congelado × D-G28). Socrático R5 0.74 ✅ (M2: arredondamento do app = peça money; Q-G31…Q-G35 abertas, §10.17). 33/33 app. Adversarial R5 0.58 → HIGH (boleto fora da peça do saldo) + MEDIUM (rotina mensal mascarava contenção) corrigidos, 757/757 · ao vivo 11/11; Q-A28…Q-A31 abertas (§10.18). **Rodada 6 DECIDIDA e EXECUTADA (§11)**: D-G30 (boleto com desconto congelado quita — valor honrado gravado na baixa), D-G31 (a) manter, **D-G32 autoridade do desconto** (teto por config × privilégio DESCONTO id 8, seed 55 — tabela→tela→código, BX-15 do legado), D-G33 (sem OS vazia), D-G34 (revalida só item novo/alterado), D-G35 (100 % morreu; teto saldo − 0,01), **D-A29 competência da rotina mensal** (peça `tb_contract_item_competence`, migration 051), D-A30 aceito, D-A31 (âncora morta decide o ramo → 409 RETURN_ANCHOR_INCONSISTENT). 773/773 · 33/33 · ao vivo 20/23 + 6/6. Q-A28 aberta (desconto em baixa parcial, §11.1). Socrático R6 0.75 ✅ (§11.2: aritmética do boleto fecha e o estorno devolve o saldo; Q-G36 carteira do boleto concede desconto sem privilégio e agora quita, Q-G37 default 0, Q-A32…Q-A35 abertas; M1/L2–L7 em sessão; implantação em §10.3 — seed 55 é pré-requisito do app). Adversarial R6 0.61 → 2 HIGH corrigidos (§11.3): rateio do boleto agrupado com desconto (invariante principal + desconto = face, teste de propriedade) e a 3ª leitura da âncora no plano de cancelamento (âncora morta cancelava e matava o saldo devolvível); + desconto que não cabe recusado no campo certo, política em centavos, regra do desconto também na tela, 3ª fórmula morta removida. 774/774 · 41/41 · ao vivo 12/12. Abertas Q-A28, Q-G36/Q-G37, Q-A32…Q-A37. **Passeio logado FEITO 2026-09-13 (§12)**: venda cancela (evento C + nº reaproveitável), venda com baixa mostra o bloqueio legível, OS reabre com a trava D5 — dados removidos. D-G36/D-G37 EXECUTADAS (desconto da carteira sob o privilégio DESCONTO + liquidação não aceita data futura; seed 55 estendido). 776/776 · ao vivo 8/8. **Rodada 7 EXECUTADA (§13)**: D-A34 remover item da OS libera a competência (fato acompanha o objeto), D-A35 varredura da rotina não trava a institution (só o ato de cunhar, com reconferência travante), D-A32/D-A33/D-A37 registradas. 779/779 · ao vivo 9/9. D-A36 EXECUTADA (saldo devolvível conta só devolução VIGENTE — peça CURRENT_RETURN_SQL; nada reescrito no passado), 783/783 · ao vivo 7/7. D-A28 DECIDIDA (opção 1 — manter o desconto sobre o saldo de cada ato; a autoridade contém o risco, não a aritmética: comentário na peça + teste que fixa a semântica). 784/784. **Nenhuma questão aberta na fase** (7 rodadas; 784/784 · 41/41; passeio logado feito). COMMITADO e PUBLICADO em 2026-09-13 (main: sql 3cc8b1a · api 24e00a3 · app f460a13 · Infra-IA 329bc92). **Estado para retomar: §14 do prompt** (ambiente do dev, resíduo dos gates, 3 frentes possíveis). COMMITADO e PUBLICADO em 2026-09-09 (main: sql 4d3720c · api 861d848 · app e41de32 · Infra-IA 7141a32) até a re-prova 0.62 — correções posteriores (049, caixa, L1/L3/L5, Q-A24/26/27, money, Rodada 5) NÃO commitadas | 8.0 |
| 2026-09-09 | **Cancelamento de nota — Rodada 2 DECIDIDA e EXECUTADA** (`prompts/prompt_cancelamento_nota.md` §10) — Valdo respondeu Q-G1…Q-G10 e Q-A1…Q-A4. DIVERGIU em D-G7: Baixas estorna baixa feita com cheque em CASCATA pela peça do cheque (X do R = "cancelamento" na linha do tempo; princípio "portador substitui a dívida"; só cheque que já avançou recusa — Q-G7a) e em D-G9: boleto agrupado com outro pedido é CANCELADO e os títulos vizinhos ficam independentes (`releasedTitles[]`; vínculo fica como história — Q-G9a). D-G3: nota da OS pela peça `@shared/invoice` e CANCELÁVEL (peça nova `@shared/service-order` reabre a OS restaurando a trava D5; migration 046 backfill). D-G4 migration 045 (`number_seq` gerada + índice). D-A1 `openReturn` relê a venda FOR UPDATE; D-A4 devolução HERDA `tb_order_billing` (faturar devolução volta a funcionar); D-A2 cinto do status; D-A3 `@shared/db/contention` (lock wait → 409 RESOURCE_BUSY transversal); D-G10 teste do retry. Socrático da rodada 0.58 → C-1 (venda com serviço tratada como OS — identidade = tb_order_service SEM tb_order_sale), H-1 (cascata também para P), M-1 (retry em settlements + ordem cheque → baixa no plano), M-2 (046 com a identidade) corrigidos em sessão; Q-G11…Q-G17 abertas. 704/704 api; banco voltou → migrations 045/046 aplicadas + smoke 13/13 no dev (§10.6). socrático re-score **0.76 PASSOU** (L-1 travante + N-1/Q-G18 registrados, §10.7). adversarial 3ª rodada 0.62 → HIGH (service-orders sem retry: deadlock com a trava D5 e com number_seq → 500) e MEDIUM (plano dizia 'cheque avançou' quando a Baixas venceu) corrigidos em sessão e re-provados 8/8 sem 500 (§10.8); Q-A5…Q-A9 abertas. 709/709. PENDENTE: passeio logado. Código NÃO commitado| 8.0 |
| 2026-09-09 | **Cancelamento de nota — Onda 1 ENTREGUE (API + app) com gates** (`prompts/prompt_cancelamento_nota.md` §6–§9) — Valdo: "cancelamento", Rodada 1 D1–D17 e "Q-P1 a Q-P8 - Rec.". `tb_invoice_event` append-only (E/C + snapshot; estado derivado), peça `@shared/invoice` (`issueInvoice` revive cabeçalho/ramos + `buildCancelPlan`/`cancelInvoice` → 409 único INVOICE_CANCEL_BLOCKED com blocos title/bankSlip/check/return), `POST /billing/cancel`, guard `requirePrivilege` (1º privilégio de ação da API: FATURAR/CANCELAR, seed sql/51), botão "Cancelar nota" gated por `CurrentInterface.can`. Gates em 2 rodadas: socrático 0.58 → **0.72**, adversarial 0.45 (2 CRITICAL de corrida provados: cancel × baixa, cancel × depósito) → **0.82 PASSOU** após C1 (plano trava `tb_financial` e lê tudo FOR UPDATE) e C2 (`isCheckEventCurrent` travante; X que aponta para o alvo = não vigente). Q-G7 (Baixas recusa estornar R vigente de cheque — 409 PAYMENT_HAS_CHECK) e Q-G8 (migration 044 índices de tb_check_event) executadas como assunção. Colaterais: migration 043 (tb_state_mva/fcp_ncm estavam na forma do legado — 1ª nota de MERCADORIA do dev) e HIGH em order-returns (faturar devolução → 422 ORDER_NO_BILLING desde a negociação). 678/678 api, 31/31 app. Aguardam o Valdo: Q-G1…Q-G10 e Q-A1…Q-A4. Código NÃO commitado| 8.0 |
| 2026-09-08 | **Passeio logado nº 2 da negociação FEITO** (`prompts/prompt_negociacao_pedido.md` §13) — Valdo: "vamos fazer o passeio logado desses caminhos no app" / "reinicie os serviços api e app". Claude in Chrome com o Valdo autenticado; API e app reiniciados pelo preview. Os 4 caminhos de §11 + D-N6 provados ao vivo e conferidos no MySQL: prazo legado avisado e trocado por 030/060; vencimento no passado com selo + decisão "Salvar assim"; base alterada por fora (100 → 110 via API) consumida pelo `expected` → decisão "Ajustar última parcela" (parcela 2 = 50); limite reduzido por fora (6 → 1) consumido pelo `expected` → cabeçalho "até 1 parcelas"; cheque na diferença D7 → dialog reabriu com "Valor a cobrir na nota R$ 45,00", 2º cheque sugerido em 5,00, **fatura 6190**, `tb_financial` 45/30, cheques 42+43 no mesmo settled_code 106, baixa 45 status N. **Achado corrigido antes de retomar**: marca da pendência local ficava no campo após corrigir/salvar → `_revalidateIfMarked` no onChanged (seção + dialog de cheques) e após sync; +2 casos de teste, 28/28. Lições de condução do browser na memória `passeio-logado-browser`. Dados de dev restaurados. **Commit de checkpoint no mesmo dia** ("pode commitar" + "pode fazer o push"; main, publicado no GitHub no mesmo dia): sql 0e009af · api af4d83a · app 3dbfa2d · Infra-IA 077ab16 — fecha o "NÃO commitado" das linhas de 2026-09-04 a 2026-09-08. | 8.0 |
| 2026-09-07 | **Tela da negociação consumindo `expected` e `deadlineValid`** — Valdo: "agora a tela consumindo expected e deadlineValid" (`prompts/prompt_negociacao_pedido.md` §11; fecha a pendência de produto da Rodada 2, sem mudança na API). Core: `FailureField.expected` + `Failure.fieldExpected`. Seção: prazo LEGADO avisado enquanto o texto não muda e validado localmente pelo espelho do `normalizeDeadline` (pendência nasce na tela); INSTALLMENT_MISMATCH com `expected` = base do pedido mudou fora da tela → base nova no cabeçalho/rodapé + decisão tipada "Ajustar última parcela" (também para o 422 do faturamento, encaminhado à seção); MAX_PARCELS_EXCEEDED com `expected` atualiza o limite exibido/validado quando é o do cabeçalho. Dialog de cheques: `expectedAmounts` do CHECK_SUM_MISMATCH — ACHADO: a validação local fechava contra o valor NEGOCIADO e travava o usuário na diferença de impostos da 1ª parcela (D7); agora fecha contra o valor da nota, com a linha "Valor a cobrir na nota". +7 chaves i18n. Achado do PNG: canônico ausente virava "será gravado como à vista" → só avisa quando a API manda o canônico. Testes 26/26 app (+7) + 3/3 core, analyze limpo. **D-N6 na tela ENTREGUE em seguida (§12)**: vencimento anterior a hoje aceito com selo na linha + contagem no rodapé + decisão tipada "Salvar assim" (nunca bloqueia); "hoje" injetável no widget (testes determinísticos — fixtures de out/nov 2026 não viram passado com o relógio real). 27/27 app. Rodada 2 D-N1–D-N6 refletida na API e na tela. Código NÃO commitado. | 8.0 |
| 2026-09-07 | **Q-G5 executada + Rodada 2 da negociação executada** — Valdo: "Q-G5 vai" e "Rodada 2 — siga as recomendações". Migration 041 (`idx_statement_inst_id`/`idx_check_inst_id` — contadores MAX+1 por institution saíram de varredura sob FOR UPDATE para "optimized away"; bloco canônico de tb_check). Rodada 2 (`prompt_negociacao_pedido.md` §9): D-N1 `max_parcels` é LIMITE de negócio nas três portas via composição `assertPaymentRules` (formas por parcela contam contra o limite DELAS — fecha a burla do adversarial; faturamento revalida forma habilitada; 0 vale 1; DEFAULT 1 = tarefa de implantação); D-N2 regra de collation em PADROES_BANCO + revisar-ddl (tabela nova que faz JOIN por VARCHAR com o baseline adota general_ci); D-N3 `fields[].expected` no envelope (INSTALLMENT_MISMATCH/MAX_PARCELS/CHECK_SUM); D-N4 prazo legado do sync tolerado se inalterado (`deadlineCanonical`/`deadlineValid` no GET); D-N5 pedido faturado sem preview; D-N6 vencimento no passado aceito. 646/646, smoke Rodada 2 5/5 + base 11/11. Pendente: app consumir `expected`/`deadlineValid`. **Passeio logado FEITO** no mesmo dia (§10 do prompt — Claude in Chrome com o Valdo logado, o agente nunca digita senha: negociação elaborada com forma própria + herança, Salvar bloqueado na divergência, dialog de cheque no faturamento, fatura 6188, financeiro/cheque/baixa conferidos no banco, somente-leitura no pedido faturado; dados de dev restaurados). Código NÃO commitado. | 8.0 |
| 2026-09-06 | **Negociação do pedido ENTREGUE — API + app** — `prompts/prompt_negociacao_pedido.md` §7–§8 (nasceu da Q-C1 do cheque: o app não tinha como definir forma/prazo — pedido criado pelo app NÃO faturava sem SQL). Rodada 1 (D1–D7) + parecer setes-conceito: RELATIVO × ABSOLUTO — `@shared/order-billing` (tb_order_billing: forma + prazo, `normalizeDeadline` estrito p/ gravar, parser tolerante p/ ler), `@shared/order-installment` (tb_order_installment: parcela combinada, soft delete padrão syncAddresses, puras `parcelQuotas`/`materializeParcels`, composição ÚNICA `resolveOrderParcels` p/ tela e faturamento), `@shared/order` (base do PEDIDO = itens `set_financial` + frete, sem impostos — espelho de `ValidaParcelamento` do legado; `lockOpenOrder` agnóstico), `@shared/payment-types` (formas habilitadas/catálogo/lookup; barrel pré-existente preservado). D7 (Valdo): no elaborado a diferença nota−pedido (ST/IPI/despesas) entra na 1ª parcela; billing compara com a base do PEDIDO (achado: R5-Q2 comparava com a base da nota — com ST/IPI toda negociação falharia). Módulo `orders`: GET/PUT `/api/orders/:id/negotiation`, `payment-types-lookup`, `banks-lookup`; app: seção "Negociação" no pedido + dialog de cheques no "Validar e Faturar" (setes-form-builder, verificado no disco; validação visual por PNG de widget test — lição de método). Sem DDL. Gates: socrático 0.72 (TOCTOU do faturamento corrigido — resolução dentro da transação após o FOR UPDATE, exploit LOCK TABLES confirmou) · adversarial 0.80 (~115 ataques; JSON malformado 500 → 400 INVALID_JSON transversal). Achados: collation mista no schema (baseline general_ci × migrations unicode_ci — COLLATE explícito + Q-N2); `max_parcels` DEFAULT 1 bloqueia 2+ parcelas até configurar (Q-N1). 602 → 641/641. Rodada 2 (Q-N1–Q-N6) aguarda o Valdo; Q-G5 aguarda "vai". Código NÃO commitado. | 8.0 |
| 2026-09-06 | **Cheque (onda 3 do financeiro) ENTREGUE — API + app** — `prompts/prompt_cheque_rastreabilidade.md` §7–§8: migration 040 (`tb_check` cabeçalho imutável com UNIQUE de identidade D5 + `tb_check_event` R/B/D/P/T/F/V/X append-only; estado DERIVADO do último evento, nunca coluna), seed 50, peça `@shared/check`, módulos gêmeos `checks` (tela de processo; Flutter verificado no disco — o agente setes-form-builder encerrou 2× sem relatório), bloco `checks` no `POST /billing/invoice` (D9: soma = parcela; sem caixa aberto derruba a nota). Achado do smoke: `nextSettledCode` lia tb_financial_payment e movimentos SEM título colidiam código — fonte única virou tb_financial_statement, 3 cópias inline (`settleOneTitle`/`settleBatchTx`/`reverseOnePayment`) unificadas. Gates em 3 rodadas (0.68/0.42 → 0.79/0.58 → **0.90 passou, 0 vulnerabilidades**): 4 CRITICAL provados ao vivo e corrigidos em sessão — P pagava pelo valor de face sem teto; estorno de R em grupo forçava X em irmão já avançado (D10 estendido ao grupo); V de 'bank' não revertia o depósito; **X tratado como 'custody' reabria cheque estornado** (`stateFromLastEvent` no default) → X é META-evento, `effectiveState()` resolve pela precondição do evento revertido, R estornado = 'voided'. D11 (Valdo): voided libera a identidade e a correção ATUALIZA o cabeçalho. 583 → 602/602. Lição de método: `default:` num switch de estado é buraco — todo kind novo precisa de ramo explícito. Pendências: visual do app; **Q-C1** — o app não tem tela de negociação do pedido (`tb_order_billing`), pré-requisito p/ coletar cheques no faturamento. Código NÃO commitado. | 8.0 |
| 2026-09-04 | **Gestao2016-Knowledge: INICIATIVA ENCERRADA (Decisão 65)** — o Valdo encerrou a "Investigação de Conhecimento Gestao2016" após o 6º processo (NFC-e/PDV), decidindo não mapear um 7º. Estado final do vault (`D:\Gestao2016\Infra-IA\Gestao`): 6 processos completos F0–F6 (Faturamento de venda, Cancelamento de NF-e, Compra, Baixa financeira, Devolução/Troca, NFC-e/PDV), 15 módulos, 251 BusinessRules, 44 perguntas de teste, 296 arquivos, 0 links quebrados, 0 corrupção de encoding — 65 decisões registradas no BRIEFING §7. Os 4 candidatos a processo do dossiê original (`candidatos-processo4.md`) ficaram todos endereçados. `[INCERTO]` remanescentes (fronteira `TB_DEVOL_IPI`/ajuste; regularização de contingência NFC-e; carga de Cartão Pré-pago) ficam registrados nos próprios documentos, não viram plano legado. Reabertura futura (novo processo, revisão, ou consumo pelo Gestao2027 setes) é decisão nova do Valdo, não retomada automática. | 7.7 |
| 2026-09-04 | **Gestao2016-Knowledge: 6º processo (NFC-e/PDV) COMPLETO F0–F6 (Decisões 61–64)** — o PDV é a mesma venda do piloto por trás das cortinas (mesmo motor fiscal `Tributacao`, mesma `TControllerNotaFiscal`), só que por 3 telas próprias: pedido (`Un_FrenteCaixa`), fatura (`Un_Cupom_Fiscal`) e autorização/cancelamento SEFAZ (`tas_gera_nfce_autorizacao`, com um modo de contingência off-line que o piloto não tem, e sem a trava de título baixado que a NF-e comum tem no cancelamento — intencional, PDV é venda à vista). Achado de encoding: `Un_FrenteCaixa.pas` é UTF-8 com BOM, não catalogado antes (CLAUDE.md do legado atualizado). Dois mecanismos financeiros exclusivos do PDV — PayBack (cashback por %) e Cartão Pré-pago (saldo por número de cartão) — viraram módulo próprio, **M15**, decisão que divergiu da recomendação do agente (mesmo padrão da M14 Base de Troca do processo 5). `04-processos/proc-nfce-pdv.md` (PDV00–PDV08); 10 regras novas + 4 de configuração (251 no total, 15 módulos); 6 perguntas de teste novas (total 44). Achado de método: um script de edição via `python3 -c` com aspas duplas apagou silenciosamente dois termos entre crases antes de chegar ao Python — pego só num grep de verificação posterior, motivo de uma nova regra de método (nunca usar aspas duplas nesse padrão quando o conteúdo tem crase). Seis processos completos; próxima decisão do Valdo: encerrar a iniciativa ou escolher um 7º processo (nenhum candidato óbvio remanescente). | 7.7 |
| 2026-09-04 | **Gestao2016-Knowledge: 5º processo (Devolução/Troca) COMPLETO F0–F6 (Decisões 57–60)** — dois mecanismos sob o mesmo nome: item de devolução/troca dentro do pedido de venda (M01, confirma a realocação pendente da Decisão 19; excluído do motor fiscal) × Base de Troca em peso, módulo próprio M14 (`TB_EXCHANGE_BASIS`, nasce ao vender produto configurado, liquida em lote do mais antigo para o mais novo, morre com o cancelamento da nota — fecha um `[INCERTO]` do processo 2). Achado de método: uma checagem automática de ids duplicados no catálogo pegou uma regra que já existia do piloto (enriquecida, não recriada) e um default de chave divergente entre dois pontos do código. `04-processos/proc-devolucao-troca.md` (DT00–DT04, não-linear); 9 regras novas (237 no total, 14 módulos); 6 perguntas de teste novas (total 38), 0 lacunas. Cinco processos completos; próxima decisão do Valdo: encerrar a iniciativa ou mapear NFC-e/PDV como 6º. | 7.7 |
| 2026-09-04 | **Gestao2016-Knowledge: F6 concluída — processo 4 (baixa financeira) COMPLETO F0–F6** — `06-teste/perguntas.md` ganhou 10 perguntas reais (código de quitação, caixa aberto, estorno, baixa parcial, as 4 baixas do cheque, receber×pagar, descontada, bloqueio de fechamento, retorno CNAB, bug do zera-pedido), 0 lacunas; total do vault 32 perguntas, 228 regras, 13 módulos, 0 links quebrados. Piloto (Faturamento), P2 (Cancelamento), P3 (Compra) e P4 (Baixa financeira) todos com F0–F6 fechadas. Próxima decisão do Valdo: encerrar a iniciativa ou escolher um 5º processo. | 7.7 |
| 2026-09-04 | **Gestao2016-Knowledge: F4 do processo 4 concluída — `proc-baixa-financeira.md`** — primeiro Process NÃO-LINEAR do vault (15 operações B00–B14 sobre o mesmo título, cada uma com porta de entrada própria, em vez de uma sequência E00→E08); o código de quitação (`GN_COD_BAIXA`) documentado como o elo estrutural entre título, movimento, cheque, boleto e cartão; checagem programática confirmou os 82 ids de regra citados. Processo 4 com F0–F5 completas; falta só F6 (vault + teste). | 7.7 |
| 2026-09-04 | **Gestao2016-Knowledge: F2 do processo 4 COMPLETA (lotes R0–R6, 228 regras) + rodada 6 FECHADA (Decisões 46–56)** — caixa (fechamento zera pelo valor digitado, transferência de turno, `PIZ_ZERA_PEDIDO_CXA` default deveria ser 'N'), estorno de baixa (inverso no caixa, DELETE no banco — lacuna), boleto (manual + retorno CNAB), cheque (as 4 baixas do gabarito confirmadas: depósito/desconto/retorno/devolução com nota avulsa CDEV), duplicata descontada (`FIN_SITUACAO='T'`, `TB_FINAN_DESCT`). 5 novos itens de plano legado (11–15). Próximo: F4 Process (`proc-baixa-financeira.md`) e F6 (vault + teste). SVN r1193. | 7.7 |
| 2026-09-04 | **Gestao2016-Knowledge: processo 4 (baixa financeira) — lotes R0–R2 documentados, 43 regras catalogadas (189), rodada 5 FECHADA (Decisões 38–45)** — R0 motor (quitação `GN_COD_BAIXA` como elo; baixa por quitação; cancelar baixa = N/N/N; caixa = conta 0 / `tb_cashier` aberto = `hr_end` nulo; `FIN_BAIXA` S/N/I), R1 receber (juros/multa automáticos por chave; créditos primeiro, rateio, parcial → residual em CARTEIRA, sobra → crédito; baixa libera bloqueio; lote de retaguarda), R2 pagar (tabela de diferenças: sinais invertidos, autorização `FIN_CP_AUTO_BAIXA`/SUPERVISOR, cheque de terceiro como pagamento). Gabarito `financeiro.md` × código: 2 contradições viraram correções do gabarito (D40 exclusão física; D44 juros automáticos), 1 bug (D41 cartão) → plano legado 10. SVN r1186–r1189. Próximo: R3 caixa + `Un_Fecha_Financeiro` (D39). | 7.7 |
| 2026-09-04 | **Gestao2016-Knowledge: rodada 4 FECHADA (Decisões 30–37) + 4º processo = Baixa financeira (F1 concluída)** — M13 Custo oficial (13 módulos); `PED_FATURADO='A'` = lixeira (D32); inutilização de faixa = lacuna → plano legado (D33); despacho após cancelar autorizada, `Pc_Apaga_conserto` e NFS-e só na pendente são intencionais (D34–36); leitura da D10 confirmada (D37). `01-grafo/processo4-baixa-financeira.md`: espinha B00–B14 (caixa aberto = `tb_cashier.HR_END is null`; quitação `GN_COD_BAIXA` liga título↔movimento↔cheque↔boleto; `FIN_BAIXA` tem 'I'), 7 lotes R0–R6 (≈20,4k linhas), 13 chaves, gabarito `financeiro.md` como teste do método. Próximo: F2 R0. SVN r1184 commitou `Infra-IA`. | 7.7 |
| 2026-09-04 | **Gestao2016-Knowledge: sessão 3 — lote 8 fecha pendências por código + dossiê da Decisão 31 + kit engine 0.4** — `PED_FATURADO='A'` = lixeira restaurável (`enviaLixeira`, log de pedidos); estado "inutilizada" = `NFE_CODSIT` 4 gravado em `tb_ctrl_nf` pela tela própria (série fixa '1', `LiberaNumeroNota` morta; regra NUM-03 → 146 regras); P10 fechado (bairro/CEP/número/logradouro obrigatórios); despacho permanece após cancelar nota autorizada; svn:ignore já estava aplicado. `01-grafo/candidatos-processo4.md` mede os 4 candidatos (A baixa financeira ≈17k linhas + gabarito `financeiro.md` — recomendado; C importação XML = lote K6 da compra). Kit: `assess-legacy-repo` ganhou "Lessons from legacy cases" (13 regras), `decision-rounds` Rodada 0 (decisões vigentes + fato × intenção), README 0.4, prompt mestre v1.3, diário §5 +3. Aguarda Decisões 30/31 e Q4.1–Q4.6 do Valdo. | 7.7 |
| 2026-09-03 | **Gestao2016-Knowledge: Decisões 26–29 (compra)** — compra deve rodar em transação única (ampliar Decisão Canônica nº 2); cancelar nota de entrada autorizada deve estornar custo; bonificação sem estoque anterior não grava custo cheio (bug); estorno de custo recalculado por TB_CUSTO_PEDIDO. `It_Tipo_OPer` = variável morta no motor. 29 decisões, 3 processos, 145 regras, 22/22 perguntas; plano legado com 7 itens acumulados no STATUS. Fim da sessão 2 | 7.7 |
| 2026-09-03 | **Gestao2016-Knowledge: 3º processo (Compra) COMPLETO** — Decisão 25; inventário com tabela venda × compra (estoque entra na nota; sem transação única; financeiro sempre manual PA; nota de terceiro digitada; custo médio ponderado com 1 nível de histórico; importação de XML = entrada alternativa), Process `proc-compra.md` (K00–K07), 23 regras CPA-* (145 no total), 22/22 perguntas. Módulo M13 Custo PROPOSTO (aguarda o autor). 3 processos mapeados em 2 sessões; LEARNINGS #16 | 7.7 |
| 2026-09-03 | **Gestao2016-Knowledge: 2º processo (Cancelamento de NF-e) COMPLETO no mesmo dia** — Decisões 23/24 (cancelar só após estorno da baixa), inventário pendente × autorizada, lotes C2–C4, Process `proc-cancelamento-nfe.md`, 14 regras CAN-* (120 no total), 17/17 perguntas no vault. Achados: `Pc_Retornodevolucao` commita a transação única no meio (bug de atomicidade → plano legado); `Pc_Apaga_conserto` apaga o pedido (pergunta ao autor); tabela da comissão = TB_COMISSAO. Método: mapear o inverso logo após o direto (LEARNINGS #15). Próximo: Decisão 25 | 7.7 |
| 2026-09-03 | **Gestao2016-Knowledge: PILOTO Faturamento de venda COMPLETO (F0–F6)** — lote 7 fechou 9 `[INCERTO]` (+5 chaves, 38 no total; 106 regras), `INDEX.md` do vault (FAQ → documento, 0 links quebrados) e `06-teste/perguntas.md` (12 perguntas dev/não-dev, 11 completas em ≤ 3 saltos). Método provado ponta a ponta em 2 sessões. Próximo: Decisão 23 (2º processo) | 7.7 |
| 2026-09-03 | **Gestao2016-Knowledge: F3/F4/F5 do piloto CONCLUÍDAS** — 22 decisões do Valdo no BRIEFING (denegada extinta; análise autocontida — Gestao2027 só como fonte do legado e no campo `web:`; ITF_OPER, estados de pedido/financeiro/nota, EMP_TIP_FAT, prazo, numeração; 12 módulos M01–M12 em `03-comunidades/MODULOS.md`), Process `04-processos/proc-faturamento-venda.md` (E00–E08, 14 pontos de decisão, estados) e 100 BusinessRules em `05-regras/` geradas de catálogo (62 negócio + 38 configuração; `web: pendente`). Próximo: F6 (vault + teste com perguntas) | 7.7 |
| 2026-09-03 | **Gestao2016-Knowledge: F2 do piloto Faturamento de venda FECHADA** (lotes 0–6 em `D:\Gestao2016\Infra-IA\Gestao-enriquecimento\`, 10 docs). Fatos que a web deve conhecer: 1 transação por statement no legado (salvo lote F4); insert/update engolem exceção; ORM literal ("vazio não grava"); estoque baixa ao salvar o ITEM do pedido (TB_CTRL_ESTOQUE + trigger); número da NF reservado em TB_RETORNO_NFE e gravado ANTES do envio; NFL_STATUS só F/C (autorização vive em NFE_CODSIT); meio de pagamento por substring da descrição; 27 chaves TB_GERAL decidem ramos. Revisão do lote 4 (prazo × parcelamento) confirmou que a web já cobre com D25/D29/D31 + `tb_order_installment` — nenhuma decisão nova. Próximo: F3 (sessão com o Valdo: módulos + ~10 perguntas) e F4/F5 (Process/Rule com campo `web:`) | 7.7 |
...
- `prompts/prompt_contrato_financeiro_baixa_automatica.md` — Contrato Financeiro / baixa automática por forma de pagamento (D1–D22, Onda 1 ENTREGUE 2026-09-04 — migration 038, módulos `financial-contracts`, gates 0.72/0.74; Rodada 4 Q-G1–Q-G5 aberta; escopo setes)
- `prompts/prompt_cancelamento_nota.md` — Cancelamento de nota fiscal de venda (inverso do faturamento): fatos do legado C00–C05/CAN-* + efeitos do invoice na web, modelo proposto (tb_invoice_event + peça @shared/invoice compondo settlement/bank-slip/check/commission/order-return), Rodada 1 DECIDIDA (D1–D17) + parecer aplicado, Onda 1 ENTREGUE 2026-09-09 (API + app), gates 0.72/0.82 ✅, Q-G7/Q-G8 executadas; Q-G1…Q-G10 + Q-A1…Q-A4 aguardam o Valdo; escopo setes
- `prompts/prompt_negociacao_pedido.md` — Negociação do pedido: prazo (string, `tb_order_billing`) × parcelamento elaborado (`tb_order_installment`), tela onde os cheques do faturamento nascem (Q-C1 do cheque). ENTREGUE 2026-09-06 — D1–D7, peças `@shared/order` / `order-billing` / `order-installment` (composição `resolveOrderParcels`) / `payment-types`, GET/PUT `/api/orders/:id/negotiation` + lookups, seção no pedido + dialog de cheques no app; gates 0.72/0.80; Rodada 2 EXECUTADA 2026-09-07 (§9: limite de parcelas nas três portas, collation, `expected`, prazo legado, sem preview em faturado); passeio logado FEITO 2026-09-07 (§10); tela consumindo `expected`/`deadlineValid` ENTREGUE 2026-09-07 (§11); D-N6 aviso de vencimento no passado na tela ENTREGUE 2026-09-07 (§12); passeio logado nº 2 FEITO 2026-09-08 (§13); escopo setes
- `prompts/prompt_cheque_rastreabilidade.md` — Cheque como portador de dívida (tb_check + eventos R/B/D/P/T/F/V/X; D1–D10 + D7a–c FECHADAS 2026-09-03; Onda 3 ENTREGUE 2026-09-04 §7–§8 — migration 040, `@shared/check`, módulos `checks` API+app, gates 0.90 após 3 rodadas com 4 CRITICAL corrigidos, D11 reuso pós-estorno; Q-C1 FECHADA pela negociação do pedido em 2026-09-06; visual validado em passeio 2026-09-08 (§9); D-CH1 convenção única de estorno EXECUTADA 2026-09-08 (§10, peça statement-reversal); D-CH2 saldo do caixa soma todas as linhas não deletadas EXECUTADA 2026-09-08 (§11); escopo setes)
- `prompts/prompt_boleto_emitido.md` — Boleto emitido (tb_bank_slip/_title/_event; D1–D11 + investigação BLT_CODQTC; Onda 2 ENTREGUE 2026-09-04 — migration 039, `@shared/bank-slip`, módulos `bank-slips`, gates 0.80/0.82 após 4 correções; Rodada 2 Q-B1–Q-B5 aberta; escopo setes)
| 2026-09-04 | **Boleto emitido — Onda 2 ENTREGUE** (`prompts/prompt_boleto_emitido.md` §10–§11): migration 039 (rename tb_bank_charge_slip→`tb_bank_charge_agreement` + active + our_number_next; `tb_bank_slip` imutável com taxas congeladas, `tb_bank_slip_title` vínculo sem FK ao título, `tb_bank_slip_event` E/L/C/X), seed 48 (interface bank-slips + config `auto_bank_slip` na interface billing). REFATORAÇÃO estrutural: núcleo do lote/parcerias/estorno saiu de settlements.repository para `@shared/financial-settlement/settlement-batch` (módulo nunca importa módulo). Peça `@shared/bank-slip` (emitir/liquidar/cancelar/estornar/gancho 0-1-n do faturamento) + módulo `bank-slips` (tela de processo, app com 19 arquivos). Smoke E2E real do ciclo inteiro; gates 1ª rodada REPROVARAM (3 HIGH: juros fora do extrato, 0.004 liquidava com zero, 1e12 truncado) → corrigidos em sessão + leitura travante em hasOpenSlip + guarda de reemissão no estorno → re-score 0.80/0.82 ✅; 532/532. Rodada 2 (Q-B1 baixa manual × boleto vigente, Q-B2 pagamento menor que a face, Q-B3 X reabre, Q-B4 source automático + retry, Q-B5 D10 no cancelamento de nota) aguarda o Valdo. Contrato: Rodada 4 D-G1–D-G4 executadas (só Q-G5 índices pende) | 7.9 |
| 2026-09-04 | **Contrato Financeiro (baixa automática por forma de pagamento) — Onda 1 ENTREGUE** (`prompts/prompt_contrato_financeiro_baixa_automatica.md`, D1–D22 em 3 rodadas + parecer setes-conceito): `tb_financial_contract` = especialização do vínculo institution×forma (PK compartilhada, 1 por forma; conta 0 = caixa; taxa + prazo; validade informativa) — a PRESENÇA do contrato é o único gatilho da baixa no faturamento (espécie/PIX = taxa 0/prazo 0; cheque fixo por kind; boleto nunca no nascimento); `usage_preference` APOSENTADA (migration 038). Peça `tryAutoSettleByContract` (crédito com dt_record futuro + DÉBITO da taxa no mesmo settled_code; estorno inverte satélites), módulos gêmeos `financial-contracts`, seeds 46/47. Smoke E2E real + gates (socrático 0.72, adversarial 0.74; 7 correções em sessão) + 514/514. Mesma janela: rodadas do CHEQUE (`prompt_cheque_rastreabilidade.md` — eventos R/B/D/P/T/F/V/X, estado derivado) e do BOLETO (`prompt_boleto_emitido.md` — família tb_bank_charge_* relida: slip=config→renomear agreement, ticket=carteira, kind=espécie; BLT_CODQTC = vínculo+quitação num só número → tb_bank_slip_title + settled_code do lote) FECHADAS. Ordem: contrato → boleto → cheque | 7.8 |
