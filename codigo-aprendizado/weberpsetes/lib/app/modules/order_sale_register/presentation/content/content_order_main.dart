...
import 'package:appweb/app/core/shared/constants.dart'
    show pdfOrderEstablishmentName;
...
class _ContentOrderMainState extends State<ContentOrderMain>
  ...
  String _pdfFmtMoney(double v) => NumberFormat('#,##0.00', 'pt_BR').format(v);

  String _pdfFmtQty(double q) {
    if ((q - q.round()).abs() < 1e-9) {
      return q.toInt().toString();
    }
    return NumberFormat('#,##0.##', 'pt_BR').format(q);
  }
  ...
  List<pw.Widget> _buildOrderPdfContent(OrderSaleMainModel om) {
    final tableRows = <pw.TableRow>[
      pw.TableRow(
        ...
        children: [
          _pdfTableCell('Código', fontSize: 9, weight: pw.FontWeight.bold),
          _pdfTableCell('Descrição do produto',
              fontSize: 9, weight: pw.FontWeight.bold),
          _pdfTableCell('Quant',
              fontSize: 9,
              weight: pw.FontWeight.bold,
              align: pw.TextAlign.right),
          _pdfTableCell('V. Unitário',
              fontSize: 9,
              weight: pw.FontWeight.bold,
              align: pw.TextAlign.right),
    ...
    for (final item in om.items) {
      ...
      final sub = item.quantity * item.unitValue;
      tableRows.add(
        pw.TableRow(
          children: [
            _pdfTableCell(
              item.tbProductId > 0 ? '${item.tbProductId}' : '—',
              fontSize: 9,
            ),
            _pdfTableCell(item.nameProduct, fontSize: 9),
            _pdfTableCell(
              _pdfFmtQty(item.quantity),
              fontSize: 9,
              align: pw.TextAlign.right,
            ),
            _pdfTableCell(
              _pdfFmtMoney(item.unitValue),
              fontSize: 9,
              align: pw.TextAlign.right,
            ),
            _pdfTableCell(
    ...
    final footerChildren = <pw.Widget>[
          ...
          pw.Column(
            ...
            children: [
              if (om.orderTotalizer.discountValue > 0) ...[
                pw.Text(
                  'Subtotal: ${_pdfFmtMoney(om.orderTotalizer.productValue)}',
                  style: const pw.TextStyle(fontSize: 10),
                ),
                pw.SizedBox(height: 2),
                pw.Text(
                  'Desconto: ${_pdfFmtMoney(om.orderTotalizer.discountValue)}',
                  style: const pw.TextStyle(fontSize: 10),
                ),
                pw.SizedBox(height: 4),
              ],
              pw.Text(
                'Total Itens : ${_pdfFmtQty(qttyItens)}',
                style: pw.TextStyle(
              ...
              pw.SizedBox(height: 4),
              pw.Text(
                'Valor total : ${_pdfFmtMoney(om.orderTotalizer.totalValue)}',
                style: pw.TextStyle(
                  fontWeight: pw.FontWeight.bold,
                  fontSize: 12,
                ),
      ...
      pw.SizedBox(height: 10),
      pw.Text(
        'Vendedor: ${om.orderSale.nameSalesman}',
        style: const pw.TextStyle(fontSize: 9),
      ),
      pw.Text(
        'Forma de pagamento: ${om.orderBilling.namePayment}'
        '${om.orderBilling.plots > 0 ? ' · ${om.orderBilling.plots} parcela(s)' : ''}'
        '${om.orderBilling.plots > 0 && om.orderBilling.deadline.isNotEmpty ? ' · Prazo: ${om.orderBilling.deadline}' : ''}',
        style: const pw.TextStyle(fontSize: 9),
      ),
      if (om.order.note.isNotEmpty) ...[
        ...
        pw.SizedBox(height: 2),
        pw.Text(om.order.note, style: const pw.TextStyle(fontSize: 9)),
      ],
    ];
    ...
    return [
      pw.Column(
        ...
        children: [
          if (pdfOrderEstablishmentName.isNotEmpty) ...[
            pw.Text(
              pdfOrderEstablishmentName.toUpperCase(),
              style: pw.TextStyle(
                fontWeight: pw.FontWeight.bold,
                fontSize: 11,
              ),
          ...
          if (om.orderSale.number > 0 || om.order.id > 0) ...[
            pw.SizedBox(height: 4),
            pw.Text(
              [
                if (om.orderSale.number > 0) 'Nº ${om.orderSale.number}',
                if (om.order.id > 0) 'ID ${om.order.id}',
              ].join(' · '),
                ...
                child: pw.RichText(
                  text: pw.TextSpan(
                    children: [
                      ...
                      pw.TextSpan(text: om.orderSale.nameCustomer),
                    ],
                  ),
                ),
              ),
              ...
              pw.RichText(
                text: pw.TextSpan(
                  children: [
                    ...
                    pw.TextSpan(text: om.order.dtRecord),
                  ],
                ),
              ),
            ],
