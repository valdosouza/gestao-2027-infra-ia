...
var
  Fr_Base_Produtos: TFr_Base_Produtos;
...
function TFr_Base_Produtos.Fc_EditaProduto():Boolean;
Var
  Lc_Qtde : Real;
Begin
...
procedure TFr_Base_Produtos.Pc_AtivarTabelas;
begin

end;

FUNCTION TFr_Base_Produtos.fC_Buscar(pc_TIPO:STRING;Pc_Codigo : String ; Pc_Descricao : String ): INTEGER;
var
  Sqltxt:string;
  Lc_Codigo: boolean;
...
procedure TFr_Base_Produtos.Pc_PreencheTela(Pc_Codpro : String; Pc_DESCRICAO : String;  Pc_VL_Venda : Real ;Pc_Unidade : String;Pc_Aliq_Com : Real; Pc_VL_Atual : Real);
var
  Lc_Codigo: integer;
...
procedure TFr_Base_Produtos.Pc_MostraBusca;
Var
...
procedure TFr_Base_Produtos.Pc_LimpaCampos;
BEGIN
  E_CodPro.Text := '';
  E_Produto.Clear;
...
function TFr_Base_Produtos.Fc_ValidaCampos():Boolean;
Var
  Lc_Result : Boolean;
Begin
  Lc_Result := True;
...
procedure TFr_Base_Produtos.Pc_Fr_FichaTecnica(Pc_tipo : String; Pc_Codvcl : Integer; Pc_cd_Produto : Integer; Pc_Produto : String; Pc_Unidade : String;
          Pc_Vl_Unit : real; Pc_Cs_Unit : real; Pc_Tx_Partic : real);
Var
  Lc_Oper_Reg : String;
Begin
...
function TFr_Base_Produtos.fc_Validagravacao:boolean;
Begin
  Result := True;
...
procedure TFr_Base_Produtos.SB_GravarClick(Sender: TObject);
Begin
  E_Produto.SetFocus;
...
procedure TFr_Base_Produtos.Pc_InserirNovoItem;
Begin
  It_StateChange := 2;
  E_CodPro.SetFocus;
  E_CodPro.SelectAll;
...
procedure TFr_Base_Produtos.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if shift = [] then
    begin
...
procedure TFr_Base_Produtos.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TFr_Base_Produtos.Sb_PesquisarClick(Sender: TObject);
begin
  E_CodPro.SetFocus;
  Pc_MostraBusca;
end;

procedure TFr_Base_Produtos.Sb_Sair_0Click(Sender: TObject);
begin
  close;
end;


procedure TFr_Base_Produtos.E_CodProExit(Sender: TObject);
begin
...
procedure TFr_Base_Produtos.E_ProdutoExit(Sender: TObject);
begin
  if not it_bt_pesquisa then  // se for falso nao foi clicando no pesquisado , nem apertado f7
    begin
    if It_StateChange = 1 then
...
procedure TFr_Base_Produtos.E_CodProEnter(Sender: TObject);
begin
  it_bt_pesquisa:=FALSE;
  E_CodPro.SelectAll;
end;

procedure TFr_Base_Produtos.E_ProdutoEner(Sender: TObject);
begin
  it_bt_pesquisa:=FALSE;
  E_Produto.SelectAll;
end;

procedure TFr_Base_Produtos.TabSheet1Show(Sender: TObject);
begin
  Height:= 245 - 18;
...
procedure TFr_Base_Produtos.E_Cons_UnitarioExit(Sender: TObject);
begin
  It_Cs_Unitario :=StrToFloatDef(E_Cons_Unitario.Text,1);
end;

procedure TFr_Base_Produtos.E_Cons_UnitarioEnter(Sender: TObject);
begin
  E_Cons_Unitario.SelectAll;
end;

procedure TFr_Base_Produtos.Pc_Tip_Busca(Pc_Tipo: char; Pc_cd_Prod,
  Pc_Desc_prod: String);
Var
  Lc_Nr_Itens : Integer;
...
procedure TFr_Base_Produtos.Pc_ImagemBotao;
Begin
...
procedure TFr_Base_Produtos.FormShow(Sender: TObject);
begin
  Pc_ImagemBotao;
  It_Bt_Pesquisa := False;
end;

procedure TFr_Base_Produtos.Sb_PesquisarMouseMove(Sender: TObject;
