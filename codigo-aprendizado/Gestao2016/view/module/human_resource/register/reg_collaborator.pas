...
uses
      Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.Menus, Vcl.Buttons, Vcl.ExtCtrls, Vcl.ComCtrls, Vcl.DBCtrls, Vcl.StdCtrls, Vcl.Mask, base_registry, ControllerColaborador, System.StrUtils;
...
type
  TRegCollaborator = class(TBaseRegistry)
    ...
    procedure CB_UsuarioKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
...
uses     UN_MSG, env, sea_job_position, api_route_cep, un_dm, Un_Funcoes, Un_PesqEndereco;
...
procedure TRegCollaborator.CB_UsuarioKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Shift = [] then
  case Key of
    VK_delete  :
    begin
      CB_Usuario.ItemIndex := -1;
      colaborador.Registro.Usuario := 0;
    end;
  end;
end;
...
begin
  ...
  Begin
    ...
    //outras informações
    Registro.Usuario := usuario.getCodigoLista(CB_usuario.Text);
    salva;
    VerificandoDemissao;
...
begin
  ...
  Begin
    ...
    if registro.ComissaoPorServico = 'C' then
     ...
     CB_usuario.ItemIndex := CB_usuario.Items.IndexOf(Lc_Aux);
    End;
  inherited;
end;
...
begin
  ...
  if CB_Usuario.Text <> EmptyStr then
  begin
    colaborador.Parametros.FieldName.Codigo := colaborador.Registro.Codigo;
    colaborador.Parametros.FieldName.Usuario := colaborador.Usuario.getCodigoLista(CB_Usuario.Text);
    colaborador.VerificaExistenciaUsuario();
    if colaborador.exist then
    begin
      MensagemPadrao('Mensagem de erro','A T E N Ç Ã O!.'+EOLN+EOLN+
                     'Este usuário está sendo usado pelo colaborador ' + colaborador.Registro.Nome + '.'+EOLN+
                     'Não � permitido usar o mesmo usuário em mais de um colaborador ao mesmo tempo' + EOLN+
...
procedure TRegCollaborator.VerificandoDemissao;
begin
  if E_Demissao.Text = EmptyStr then
    colaborador.RemoverDemissao;
end;
