...
uses
      Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, Vcl.Menus, Vcl.Buttons, Vcl.ExtCtrls, System.StrUtils, base_registry, ControllerUsuario;
...
type
  TRegUsers = class(TBaseRegistry)
    ...
    procedure sb_resetaClick(Sender: TObject);
  private
...
begin
  ...
  Begin
    ...
    Registro.Ativo := IfThen(Cbx_Ativo.Checked, 'S', 'N');
    ...
    Registro.emailAutenticacao := IfThen(Cbx_Req_Autenticacao.Checked, 'S', 'N');
    Registro.emailSSL := IfThen(Cbx_Req_Con_ssl.Checked, 'S', 'N');
    ...
    Registro.NotificarPorEmail := IfThen(chbx_notifica_login.Checked, 'S', 'N');
...
begin
  ...
  Begin
    ...
    if Registro.NotificarPorEmail = 'S' then
...
begin
  ...
  if (InputQuery('Envio teste de e-mail ', 'Informe um e-mail v�lido', LcEmailTo)) then
  Begin
...
procedure TRegUsers.sb_resetaClick(Sender: TObject);
begin
  if (MessageDlg('Resetar a senha do usuário : ' + E_Nome.Text + ' ? ' ,
