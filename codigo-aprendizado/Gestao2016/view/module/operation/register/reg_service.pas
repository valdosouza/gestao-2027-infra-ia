...
uses
      Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.Menus, System.Classes, Vcl.Buttons, Vcl.ExtCtrls, Vcl.ComCtrls, Vcl.StdCtrls, Vcl.DBCtrls, Vcl.Mask, QEdit_Setes, System.StrUtils, base_registry, ControllerService, ControllerCategory, ControllerGrupos, ControllerSubGrupos, ControllerMedida, ControllerPreco, sea_measure, controllerItensNFL, Data.DB, Datasnap.DBClient;
...
uses     UN_MSG, env, sea_category, sea_group, Un_DM, UN_TabelasEmListas;
...
procedure TRegService.ListaSubgrupo;
begin
...
procedure TRegService.AtivaTabelas;
begin
end;
...
begin
  ...
  for i := 0 to Pred(category.Lista.Count) do
    memCategory.AppendRecord([category.Lista[i].Codigo, category.Lista[i].Descricao, category.Lista[i].NivelPosicao]);
...
begin
  Lc_NovoNo := nil;
  Lc_NivelAtual := Fc_NivelHierarquico( Pc_Query.FieldByName('Posicao').AsString ); //Pc_Query.Fields[3].AsString - category.lista[0].Posicao
  repeat
    Lc_Descricao :=  Pc_Query.FieldByName('Descricao').AsString; //Pc_Query.Fields[2].AsString - category.lista[i].Descricao
    Lc_NovoNivel := Fc_NivelHierarquico( Pc_Query.FieldByName('Posicao').AsString );  //Pc_Query.Fields[3].AsString - category.lista[i].Posicao
...
begin
  ...
  Begin
    ...
    Begin
      ...
      Registro.Categoria := 0;

    Registro.CodigoGrupo := DBLCB_Grupo.KeyValue;
    Registro.CodigoSubGrupo := DBLCB_SubGrupo.KeyValue;
    MarcaServico.Registro.Descricao := 'PROPRIA';
    MarcaServico.getIdAutoByDesc;
    Registro.CodigoMarca := MarcaServico.Registro.Codigo;

    Registro.MedidaComercial := DBLCB_Medida.KeyValue;
  ...
  E_Codigo.Text := service.Registro.Codigo.toString;
...
begin
  ...
  Finally
    ...
    begin
      DM_ListaConsultas.Pc_ListaMedida('','');
      DBLCB_Medida.KeyValue := Lc_Form.cds_searchcodigo.AsInteger;
    end;
    FreeAndNil(Lc_Form);
  End;
end;

procedure TRegService.setPerfil;
begin
  pfMenu := 'Cadastro Serviços';
  inherited;
end;
...
begin
  ...
  Begin
    ...
    DBLCB_Grupo.KeyValue := Registro.CodigoGrupo;
    ListaSubgrupo;
    DBLCB_SubGrupo.KeyValue := Registro.CodigoSubgrupo;

    DBLCB_Medida.KeyValue := Registro.MedidaComercial;
