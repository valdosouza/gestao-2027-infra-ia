...
uses
      Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, base_registry, Vcl.Menus, Vcl.Buttons, Vcl.ExtCtrls, Vcl.StdCtrls, Vcl.Mask, Vcl.DBCtrls, ControllerPerfilUsuario;
...
begin
  ...
  Begin
    if (MensagemPadrao('Mensagem de Confirmção',
                      ...
                      ['Sim','N�o'],[bEscape,bNormal],mpConfirmacao,clRed) = mrBotao1) then
    begin
...
begin
  if (MensagemPadrao('Mensagem de Confirmção',
                     'Excluir '+Tb_Perfil.FieldByName('PFL_DESCRICAO').AsString +' de seus arquivos.'+EOLN+EOLN+
                     'Confirmar a exclusão ?',
                    ['Sim','N�o'],[bEscape,bNormal],mpConfirmacao,clRed) = mrBotao1) then
  begin
    try
