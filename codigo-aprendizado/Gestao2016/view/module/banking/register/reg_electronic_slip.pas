...
uses
      Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.Menus, Vcl.Buttons, Vcl.ExtCtrls, Vcl.StdCtrls, Vcl.DBCtrls, Vcl.Mask, ControllerFormaPagamento, base_registry, ControllerBoletoEletronico, Un_Fm_ListaEspecieDoc, Un_Fm_ListaContaBancaria, System.StrUtils, ACBrBoleto, ACBrBoletoConversao, TypInfo, reg_billing_portfolio, un_fm_billing_portfolio, base_frame_list, Vcl.FileCtrl;
...
begin
  ...
  FormaPagamento    := TControllerFormaPagamento.create(self);
end;
...
begin
  ...
  Begin
    FmBillingPortfolio.CodigoBanco := Fm_ListaContaBancaria.Qr_ContaBancaria.FieldByName('EMP_CODIGO').AsInteger;
    FmBillingPortfolio.Listar;
  End;
end;
...
begin
  ...
  Begin
    FormaPagamento.autocreate('BOLETO');
    Registro.FormaPagamento   := FormaPagamento.Registro.Codigo;
    Registro.ContaCorrente    := Fm_ListaContaBancaria.DBLCB_ContaBancaria.KeyValue;
    Registro.CodigoCarteira   := FmBillingPortfolio.Dblcb_Lista.KeyValue;
    Registro.EspecieDoc       := Fm_ListaEspecieDoc.Dblcb_Especie_Doc.KeyValue;
    Registro.Aceite           := IfThen(RG_Aceite.ItemIndex = 0, 'S', 'N');
...
begin
  ...
  Begin
    ...
    FmBillingPortfolio.CodigoBanco := Fm_ListaContaBancaria.Qr_ContaBancaria.FieldByName('EMP_CODIGO').AsInteger;
    FmBillingPortfolio.Listar;
    FmBillingPortfolio.Dblcb_Lista.KeyValue := Registro.CodigoCarteira;
    ...
    IF Registro.LayRemessa = 'c240' then
      Cb_LayoutRemessa.ItemIndex := 0
    else
      Cb_LayoutRemessa.ItemIndex := 1;
...
begin
  ...
  if (Trim(FmBillingPortfolio.Dblcb_Lista.Text) = EmptyStr) then
  begin
    TMsgSetes.ValidaPreenchimentoCampo(FmBillingPortfolio.L_name_list.Caption);
    Result := False;
    FmBillingPortfolio.Dblcb_Lista.SetFocus;
    Exit;
  end;

  if (trim( Fm_ListaEspecieDoc.Dblcb_Especie_Doc.Text ) = EmptyStr) then
  begin
    TMsgSetes.ValidaPreenchimentoCampo(Fm_ListaEspecieDoc.L_Lista.Caption);
    Result := False;
