...
uses
       ACBrdevice, FavoritoButtons, ACBrGAV, Winapi.WinSpool, Vcl.StdCtrls, Vcl.DBCtrls, ACBrBase, ACBrBoleto, ACBrUtil, ACBrBoletoFCFortesFr, Vcl.Imaging.jpeg, Classes, Vcl.Grids, STQuery, Vcl.Forms, SysUtils, Variants, Data.DB, System.Math, UITypes, Vcl.Dialogs, Types, IniFiles, Vcl.ExtCtrls, Vcl.Graphics, STDatabase;
...
//-------------------------Permissíes de Usuarios do Sistema  -------------------------------
...
uses     UN_Sistema, un_mp2032, Un_Principal, Un_Liberacao, env, Un_DM, UN_MSG, Un_Fc_Sored_Procedures, reg_users, Un_caixa, ControllerBase, UN_RL_OrcaConserto, UN_RL_OrcaCst_Entg, Un_WebService, Un_MinhaEmpresa, RN_Estoque, RN_NotaFiscal, RN_Permissao, UN_TabelasEmListas, ControllerDespacho;
procedure Pc_AtivaEstabelecimento();
Begin
   ...
   try
    ...
    Begin
      ...
      begin
        ...
        Except
          MensagemPadrao(MENSAGEM, ATENCAO + EOLN + EOLN +
                         'Imagem '+ Pc_Descricao + ' está corrompida ou num formato Inválido.' + EOLN +
                         'Verifique no cadastro de minha Empresa.' + EOLN,
                         ['OK'], [bEscape], mpAlerta);
        End;
      end
  ...
  Try
    ...
    with Lc_Qry do
      ...
      if (fc_IBGE > 0) then
        ...
        ParamByName('CDD_DESCRICAO').AsString := AnsiUpperCase( Fc_Descricao );
        ...
        begin
          ...
          Begin
            if (Fc_Tb_Geral('L','PRO_G_CUSTO_ATUAL_PRECO','R') = 'R') then
              Lc_Qry_Insere.ParamByName('PRC_VL_VDA').AsCurrency := Fc_CalcularPrecoPelaMargemLucro(
                                                                          Pc_Vl_CustoMedio,
  ...
  Try
    ...
    Begin
      ...
      Begin
        ...
        Lc_Despacho.Registro.Pedido        := Pc_cd_Pedido;
        Lc_Despacho.Registro.ItemPedido    := Pc_Cd_Item;
        Lc_Despacho.Registro.Produto       := Pc_cd_Produto;
        Lc_Despacho.Registro.DataRealizada := Pc_Dt_Prevista;
        Lc_Despacho.Registro.Qtde          := Pc_qtde;
        Lc_Despacho.Registro.Situacao      := 'A';
        Lc_Despacho.Insert;
      end
      else
      Begin
        ...
        Begin
          ...
          if ( Lc_Qtde_saldo > 0 ) then
            Lc_Despacho.AlteraQtdeDespacho(Lc_Cd_Dsp_Aguardando,Lc_Qtde_saldo)
          else
        ...
        Begin
          ...
          //Verifica se Altera a Quantidade do Produto que esta aguardando liberação
          if ( Lc_Qtde_saldo > 0 ) then
            Lc_Despacho.AlteraQtdeDespacho(Lc_Cd_Dsp_Pendente,Lc_Qtde_saldo)
          else
        ...
        Begin
          if (Pc_Qtde >0) then
            Lc_Despacho.AlteraQtdeDespacho(Lc_Cd_Dsp_Separada,Pc_Qtde)
          else
            ...
            Lc_Despacho.Registro.ItemPedido    := Pc_Cd_Item;
            Lc_Despacho.Registro.Produto       := Pc_cd_Produto;
            Lc_Despacho.Registro.DataRealizada := Lc_Dt_Devolucao;
            Lc_Despacho.Registro.Qtde          := (Lc_Qtde_saldo * -1);
            Lc_Despacho.Registro.Situacao      := 'D';
            Lc_Despacho.Insert;
        end
        else
        Begin
          ...
          Begin
            ...
            Lc_Despacho.Registro.Produto       := Pc_cd_Produto;
            Lc_Despacho.Registro.DataRealizada := Pc_Dt_Prevista;
            Lc_Despacho.Registro.Qtde          := Lc_Qtde_saldo;
            Lc_Despacho.Registro.Situacao      := 'A';
            Lc_Despacho.Insert;
          end;
        end;
...
Begin
  ...
  //    if (Fc_PedidoFechado or Fc_NotaFiscal)
      ...
      Active := true;
      FetchAll;
      Result := FieldByName('VL_VENDA').AsCurrency;
...
Begin
  Try
    ...
    Begin
      ...
      Active := true;
      FetchAll;
      Result := FieldByName('CTC_VL_TL_FRETE').AsCurrency;
      Screen.Cursor:=crDefault;
    end;
  Finally
    LcBase.FinalizaQuery(Lc_Qry);
...
Begin
  ...
  if not Usa then
    Begin
    MensagemPadrao(MENSAGEM,ATENCAO+ EOLN + EOLN +
                   'A Abertura da gaveta não foi ativada nas configuraçães.' + EOLN +
                   '               Verifique e tente novamente.' + EOLN,
                   ['OK'], [bEscape], mpAlerta);
    Result := False;
    exit;
...
begin
  ...
  Begin
    if (Fc_Aq_Geral('L','CONTROLE DE FRENTE','FRT_P_GVT_STR_ABERT', '') = 'Impressora Epson (via Impressío)') then
    Begin
    ...
    if (Fc_Aq_Geral('L','CONTROLE DE FRENTE','FRT_P_GVT_STR_ABERT', '') = 'Impressora Diebold  (via Impressío)') then
    Begin
