...
uses
        Datasnap.DBClient, ControllerBase, QEdit_Setes, System.Threading, Classes, Vcl.Forms, STDatabase, QuickRpt, Vcl.Grids, Vcl.Mask, SHDocVw, Winapi.Windows, Vcl.Controls, Vcl.ComCtrls, STQuery, Vcl.DBGrids, Vcl.Samples.Gauges, UITypes, Vcl.StdCtrls, AnsiStrings, IniFiles, SysUtils, QRCtrls, Vcl.Imaging.jpeg, Vcl.Graphics, TypInfo, Vcl.Dialogs, Data.DB, System.Math, ACBrUtil, Vcl.OleCtrls, Variants, Vcl.Printers, Winapi.ShellAPI, Vcl.ExtCtrls, Winapi.WinInet, IdHTTP, Registry, IdRawBase, IdRawClient, IdTCPClient, IdSSLOpenSSL, Vcl.Imaging.GIFImg, STTransaction, ControllerPafCtrl;
...
function Fc_GeneratorBcoDestino(FcDataBase : TSTDatabase; FcTransacao : TSTTransaction; pc_Gen: string;pc_NomeTabela:String;Pc_campo : String): Integer;
...
function ValidatePafNfceForcedClosure:Boolean;

implementation

uses     Un_Regra_Negocio, RI_MovimentoFinanceiro, ControllerVehicleOs, Un_VehicleSo, Un_VehicleCheckList, Un_Funcoes, PrintPizzaria, UN_Principal, env, UnFunctions, ControllerTabelaPreco, UN_MSG, Un_Fc_Sored_Procedures, UN_RL_Orcamento_1, UN_RL_OrcaConserto, Un_Produtos, UN_RL_OrcaCst_Entg, Un_RL_PedidoVenda_10, Un_WebService, RN_Permissao, RN_Estoque, UN_DM, RN_Financeiro, Un_VideoTreinamento, Un_Pesq_Empresa, UN_TabelasEmListas, Un_Cliente, RN_Cliente, ControllerGeral, ControllerCtrlIcmsST, RN_NovaEstruturaDados, ControllerCtrlEstoque, ControllerPreco, ControllerProduto, Un_ProcessoAguarde, UnGeraLog;
const
...
begin
  ...
  Try
    ...
    DM.IBD_Gestao.Params.Values['DriverID'] := 'FB';
    DM.IBD_Gestao.Params.Values['Database'] := Gb_PathBD;
...
begin
  ...
  with DM do
    Begin
    Qr_GeneratorBcoDestino.Active := False;
    Qr_GeneratorBcoDestino.Database := FcDataBase;
    Qr_GeneratorBcoDestino.Transaction := FcTransacao;
    IBT_GeneratorBcoDestino.DefaultDatabase := FcDataBase;
    IBT_ConsultaBcoDestino.DefaultDatabase  := FcDataBase;
    IF not FcDataBase.Connected then FcDataBase.Connected := True;
    if IBT_GeneratorBcoDestino.InTransaction then IBT_GeneratorBcoDestino.Commit;
    if not IBT_GeneratorBcoDestino.InTransaction then IBT_GeneratorBcoDestino.StartTransaction;
    Qr_GeneratorBcoDestino.SQL.Clear;
    Lc_SqlTxt := 'SELECT GEN_ID(' + pc_Gen + ',1) FROM RDB$DATABASE';
    Qr_GeneratorBcoDestino.SQL.Add(Lc_SqlTxt);
    Qr_GeneratorBcoDestino.Active := True;

    Result := Qr_GeneratorBcoDestino.fieldbyname('GEN_ID').AsInteger;
...
//  begin
    try
      ...
      Pc_Barra.MinValue := 0;
      { O Max do progress bar é igual o tamanho do arquivo }
      Pc_Barra.MaxValue := FileLength;
      Pc_Barra.Progress := 0;
      while FileLength > 0 do
      begin
        { Lê um pedaço do arquivo de origem }
        BlockRead(ArquivoOrigem, Buffer[0], SizeOf(Buffer), NumRead);
        { Muda o valor da variável tamanho do arquivo retirando o pedaço copiado }
        FileLength := FileLength - NumRead;
        { Escreve no arquivo de destino o pedaço lido do arquivo de origem }
        BlockWrite(ArquivoDestino, Buffer[0], NumRead);
        { Muda a posição do progress bar }
...
var
  Lc_Text : String;
begin
  Lc_Text := Fc_RemoveCaracterInformado(pc_Fone, ['(',')',' ','-','-']);
...
begin
  ...
  // Varre o formulário é procura do componente
...
// Lé valor da propriedade do componente
    ...
    try
      ...
      Begin
        if (MensagemPadrao(TITULO_CONFIRMACAO,
                           ...
                           [NAO, SIM], [bEscape, bNormal], mpConfirmacao, clBlue) = mrBotao2) then
        Begin
        ...
        begin
          ...
          Begin
            ...
            BEgin
              ...
              Begin
                Lc_TabPreco.Preco.Registro.Valor          := Fc_CalcularPrecoPelaMargemLucro(Lc_Qry.FieldByName('VALOR').AsCurrency,
...
begin
  ...
  try
    ...
    Begin
      ...
      while not Lc_Qry.Eof do
      begin
        Lc_valor := Lc_valor + (FieldByName('ITF_VL_UNIT').AsCurrency * FieldByName('ITF_QTDE').AsFloat);
        Next;
      end;
    end;
  finally
...
begin
  ...
  try
    ...
    Begin
      ...
      Active := True;
      Lc_Resultado := Lc_Resultado + FieldByName('ICM_SUBTOTAL').AsCurrency;
      ...
      Active := True;
      Lc_Resultado := Lc_Resultado + FieldByName('IPI_SUBTOTAL').AsCurrency;
      Result := Lc_Resultado;
    end;
  finally
    LcBase.FinalizaQuery(Lc_Qry);
...
begin
  ...
  try
    ...
    Begin
      ...
      Active := True;
      Result := FieldByName('ITF_VL_TOTAL').AsCurrency;
    end;
  finally
    LcBase.FinalizaQuery(Lc_Qry);
    FreeAndNil(LcBase);
  ...
  Begin
    ...
    Begin
      ...
      try
        ...
        Begin
          //Cria a dos produtos que váo ser desativados
...
Begin
  ...
  try
    ...
    Lc_Qry.Active := true;
    Result := Lc_Qry.fieldbyname('emp_vl_cred').AsCurrency;
  finally
    LcBase.FinalizaQuery(Lc_Qry);
    FreeAndNil(LcBase);
  end;
      ...
      Begin
        ...
        Begin
          ...
          Begin
            ...
            ParamByName('emp_codigo').AsInteger := Pc_cd_cli;
            Active := true;
            lc_emp_vlrcred:= (fieldbyname('emp_vl_cred').AsCurrency);
...
begin
  ...
  try
    ...
    while not lc_qry.Eof do
    begin
      LC_VL_UNIT:=LC_VL_UNIT+lc_qry.fieldbyname('FTC_VL_UNIT').AsCurrency;
      LC_cs_unit:=LC_cs_unit+lc_qry.fieldbyname('FTC_CS_UNIT').AsFloat;
      Lc_Subtotal := Lc_Subtotal + (lc_qry.FieldByName('FTC_VL_UNIT').AsCurrency * lc_qry.FieldByName('FTC_CS_UNIT').AsFloat );
      lc_qry.Next;
    end;
...
begin
  ...
  Try
    ...
    Begin
      ...
      if (RecordCount > 0) then
      Begin
        IF (FieldByName('PAR_VL_TOTAL').AsCurrency <> Fc_Vl_pedido) AND
           (FieldByName('PAR_VL_TOTAL').AsCurrency >0) THEN
        BEGIN
...
begin
  ...
  try
    ...
    begin
      ...
      lc_qry_insere.ParamByName('FTC_VL_UNIT').AsCurrency := Lc_Qry.fieldbyname('FTC_VL_UNIT').AsCurrency;
...
begin
  ...
  Situação igual a Destinada e Etapa como [P]rovio e não baixado}
  LcBase := TControllerBase.create(nil);
  Lc_Qry := LcBase.GeraQuery;
  Lc_Qry_Del := LcBase.GeraQuery;
  try
  ...
  Try
    ...
    Begin
      ...
      Begin
        ...
        ParamByName('ITF_VL_CUSTO').AsCurrency := StrToFloatdef(Lc_Mtz[1,Lc_I],0);
        ExecSQL;
      end;
    End;
  Finally
  ...
  Begin
    ...
    try
      ...
      Begin
        ...
        ParamByName('APR_VL_ANTERIOR').AsCurrency :=  Pc_Valor_Anterior;
        ParamByName('APR_VL_ATUAL').AsCurrency :=  Pc_Valor_Atual;
...
Function Fc_PingConectadoSetes():Boolean;
const
  C_HOST_API   = 'admdesktop.setesgestao.com.br';
  C_PORTA_API  = 443;
  C_TIMEOUT_MS = 3000;
Var
  LcTCP: TIdTCPClient;
begin
  {ICMP bloqueado em varios ambientes; testa TCP na porta HTTPS da API}
  Result := False;
  LcTCP := TIdTCPClient.Create(nil);
  try
    try
      LcTCP.ConnectTimeout := C_TIMEOUT_MS;
      LcTCP.ReadTimeout    := C_TIMEOUT_MS;
      LcTCP.Host           := C_HOST_API;
      LcTCP.Port           := C_PORTA_API;
      LcTCP.Connect;
      Result := LcTCP.Connected;
    except
      on E: Exception do
      begin
      ...
      begin
        ...
        begin
          ...
          Begin
            ...
            Begin
              LcImg := TImage(TPanel(Pc_Tela.Components[Lc_I]).Components[Lc_J]);
              FreeAndNil(LcImg);
              LcPanel := TPanel(Pc_Tela.Components[Lc_I]);
              FreeAndNil(LcPanel);
              Lc_Achou := True;
              Break;
            End;
...
//        Lc_Registro.WriteInteger('SecureProtocols', 160);
    ...
    //f para IE não ficar trabalhando off line
    if Lc_registro.OpenKey('Software\Microsoft\Windows\CurrentVersion\Internet Settings', true) then
    begin
...
//        Lc_Registro.WriteInteger('SecureProtocols', 160);
    ...
    // para IE não ficar trabalhando off line
    if Lc_registro.OpenKey('Software\Microsoft\Windows\CurrentVersion\Internet Settings', true) then
    begin
      Lc_Registro.WriteInteger('GlobalUserOffline', 0);
    end;
  ...
  Try
    ...
    Begin
      ...
      Begin
        ...
        while not Lc_Qry.Eof do
        begin
          lc_Mtz_Prazo[0,Lc_I] := FloatToStrF(Lc_Qry.FieldByname('PAR_VALOR').AsCurrency,ffFixed,10,2);
...
begin
  ...
  Try
    with Lc_Qry do
    Begin
      sql.Add(concat('SET GENERATOR ' , FcGenerator , ' TO ' , IntToStr(Sequencia)));
      ExecSQL;
    End;
  Finally
    LcBase.FinalizaQuery(Lc_Qry);
...
function ValidatePafNfceForcedClosure: Boolean;
Var
  Lc_paf_nfce: TControllerPafCtrl;
begin
  Result := False;
  try
    Lc_paf_nfce := TControllerPafCtrl.create(Nil);
    if (Lc_paf_nfce.forceClosure) then
    Begin
      Result := True;
      MensagemPadrao('Mensagem de Alerta', 'A T E N Ç Ã O!.' + EOLN + EOLN +
                     'Pelas regras do PAF-NFC-e Não é permitido '+EOLN+
                     'mais de 3 dias sem fechamento' + EOLN +
                     'Por favor acesse a tela de PAF-NFC-e.' + EOLN,
                     ['OK'], [bEscape], mpAlerta);
    end;
  finally
    FreeAndNil(Lc_paf_nfce);
  end;
end;
