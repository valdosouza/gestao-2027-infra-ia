...
uses
  ControllerComissao, STDatabase, SysUtils, STTransaction, STQuery,
  Datasnap.DBClient, Vcl.Dialogs, ControllerItensICMS, ControllerBase,
  ControllerItensNFL, ControllerOperationsLog, ControllerFinanceiro,
  ControllerMovimentoFinanceiro, ControllerNotaFiscal, ControllerFichaTecnica;
...
uses     Un_DM, UN_Sistema, UN_MSG, STDataSet, Un_Regra_Negocio, RI_MovimentoFinanceiro, env;
Function GeraStored():TSTTransaction;
begin
...
procedure Pc_Log_Sistema( pc_log_codmha    : Integer;
                          ...
                          pc_log_descricao : String );
Var
  Lc_Log_Operacao : TControllerOperationsLog;
begin
  Lc_Log_Operacao := Nil;
  Try
    Lc_Log_Operacao := TControllerOperationsLog.create(nil);
    with Lc_Log_Operacao do
    Begin
      Registro.Codigo       := Fc_Generator('GN_LOG_OPERACAO','','LOG_CODIGO');  //--> Código da log
      Registro.Descricao    := Copy(pc_log_descricao,1,100);     //--> Descrição operção realizada
      Registro.CodMha       := pc_log_codmha;        //--> Código da Empresa
      Registro.CodUsu       := pc_log_codusu;        //--> Código do Usuário
      Registro.Hora         :=  pc_log_timeStamp;     //--> A hora e data Atual
      Registro.InterfaceLog := pc_log_interface;     //--> Nome da tela
      Registro.Registro     := pc_log_registro;     //--> Chave prmimara da Tabela Relacionada
      Registro.Operacao     := copy(pc_log_operacao,1,30);      //--> Tipo de Modificção
    end;
    Lc_Log_Operacao.insert;
  Finally
    FreeAndNil(Lc_Log_Operacao);
  End;
