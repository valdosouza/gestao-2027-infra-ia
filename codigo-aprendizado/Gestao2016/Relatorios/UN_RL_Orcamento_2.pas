...
type
  TRL_Orcamento_2 = class(TForm)
    ...
    procedure QRBand4BeforePrint(Sender: TQRCustomBand;
      var PrintBand: Boolean);
    procedure Qr_CotacaoAfterOpen(DataSet: TDataSet);
  private
...
var
  RL_Orcamento_2: TRL_Orcamento_2;

implementation

uses     Un_Funcoes, UN_Sistema, Un_DM, StdCtrls, Un_Regra_Negocio, Un_Principal, env;
{$R *.dfm}
Procedure TRL_Orcamento_2.PC_VAlidaCampos(Pc_Print : Boolean);
Begin
  if Pc_Print then
    BEgin
...
Procedure TRL_Orcamento_2.PC_PreencheCampos;
Begin
...
procedure TRL_Orcamento_2.QrptBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
Var
  Lc_Ano,Lc_mes,Lc_dia, Lc_Veiculo, Lc_Placa:String ;
  Lc_I:Integer;
begin
  ...
  Lb_Cliente.Caption := Qr_Cotacao.FieldByName('CTC_FANTASIA').AsString;
  Lb_Contato.Caption := Qr_Cotacao.FieldByName('CTC_CONTATO').AsString;

  Lb_Veiculo.Caption := Qr_Ordem.FieldByName('TPV_DESCRICAO').AsString + ' ' +Qr_Ordem.FieldByName('MOD_DESCRICAO').AsString;
  Lb_Placa.Caption := Qr_Ordem.FieldByName('VEI_PLACA').AsString;

  DM.Qr_Endereco.Active := False;
  DM.Qr_Endereco.ParamByName('EMP_CODIGO').ASINTEGER := Qr_Cotacao.FieldByName('CTC_CODEMP').ASINTEGER;
...
procedure TRL_Orcamento_2.DetalheVendaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRL_Orcamento_2.QRBand4BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
Var
  Lc_I : Integer;
begin
  ...
  Lb_Obs.Lines.Clear;
  Lb_Obs.Lines.Add(  concat(Qr_Cotacao.FieldByName('fpt_descricao').AsString, ' - ', Qr_Cotacao.FieldByName('CTC_PRAZO').AsString));
  Lb_Obs.Lines.Add(Qr_Cotacao.FieldByName('CTC_OBS').AsString);
  Lb_Usuario.Caption := UpperCase('Vendedor: '+Gb_Nm_Usuario);
end;

procedure TRL_Orcamento_2.Qr_CotacaoAfterOpen(DataSet: TDataSet);
begin
  Qr_Itens.Active := False;
  Qr_Itens.ParamByName('CTC_CODIGO').AsInteger := Qr_Cotacao.FieldByName('CTC_CODPED').AsInteger;
  Qr_Itens.Active := True;

  Qr_Ordem.Active := False;
  Qr_Ordem.ParamByName('CTC_CODPED').AsInteger := Qr_Cotacao.FieldByName('CTC_CODPED').AsInteger;
