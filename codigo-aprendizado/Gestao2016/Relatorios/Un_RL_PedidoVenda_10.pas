unit Un_RL_PedidoVenda_10;

interface

uses
      Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs, DB, STQuery, QuickRpt, QRCtrls, ExtCtrls, Vcl.StdCtrls, ControllerBase, System.Math;

type
  TRL_PedidoVenda_10 = class(TForm)
    Qrpt: TQuickRep;
    Cabecalho: TQRGroup;
    QRShape20: TQRShape;
    QRLabel3: TQRLabel;
    ...
    Qr_ItensProITF_CODIGO: TIntegerField;
    Qr_ItensProITF_CODPED: TIntegerField;
    Qr_ItensProITF_CODNFL: TIntegerField;
    Qr_ItensProITF_CODPRO: TIntegerField;
    Qr_ItensProPRO_CODIGOFAB: TStringField;
    Qr_ItensProPRO_DESCRICAO: TStringField;
    Qr_ItensProITF_QTDE: TBCDField;
    Qr_ItensProITF_VL_CUSTO: TFMTBCDField;
    Qr_ItensProITF_VL_UNIT: TFMTBCDField;
    Qr_ItensProITF_AQ_COM: TBCDField;
    Qr_ItensProMED_ABREVIATURA: TStringField;
    Qr_ItensProPRO_CODIGOFOR: TStringField;
    Qr_ItensProITF_VL_SUBTOTAL: TFMTBCDField;
    QRLabel2: TQRLabel;
...
var
  RL_PedidoVenda_10: TRL_PedidoVenda_10;

implementation

uses     Un_DM, UN_Sistema, Un_Regra_Negocio, UN_Principal, env, RN_Transportadora;
{$R *.dfm}



procedure TRL_PedidoVenda_10.Pc_PreencheCliente;
begin
  with Qr_Pedido do
  Begin
...
Procedure TRL_PedidoVenda_10.PC_ValidaServicos(Pc_Print : Boolean);
Begin
  if Pc_Print then
    BEgin
...
Procedure TRL_PedidoVenda_10.PC_PreencheServicos;
Begin
...
Procedure TRL_PedidoVenda_10.PC_VAlidaProdutos(Pc_Print : Boolean);
Begin
  if Pc_Print then
    BEgin
...
Procedure TRL_PedidoVenda_10.PC_PreencheProdutos;
Var
  LcValor : Real;
Begin
...
procedure TRL_PedidoVenda_10.Pc_Rodape;
begin
  with Qr_Pedido do
  Begin
    ...
    Lb_Vl_Frete.Caption     := FloattoStrf(Qr_Pedido.FieldByname('PED_VL_FRETE').AsCurrency ,ffFixed,10,2);
    Lb_Vl_SubTotal.Caption  := FloattoStrf(It_Vl_Servico +  It_Vl_Produto + Qr_Pedido.FieldByname('PED_VL_FRETE').AsCurrency,ffFixed,10,2);
    Lb_Vl_Desconto.Caption  := FloattoStrf(It_vl_Desconto ,ffFixed,10,2);
    Lb_Vl_Total.Caption     := FloattoStrf(It_Vl_Servico + It_Vl_Produto + Qr_Pedido.FieldByname('PED_VL_FRETE').AsCurrency - It_vl_Desconto,ffFixed,10,2);



    E_Vendedor.Caption := FieldByname('CLB_NOME').AsString;
    Lb_LocalData.Caption    := DM.Qr_Estabelecimento.FieldByname('CDD_DESCRICAO').AsString + '   ______/______/___________ ';
    E_Assinatura.Caption   := FieldByname('EMP_NOME').AsString;
  end;
end;

procedure TRL_PedidoVenda_10.QrptBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  Pc_Define_Impressora_Qrpt(Qrpt);
  Pc_Cab_Report(bnd_cabecalho);
...
procedure TRL_PedidoVenda_10.Pc_DefineObservacao;
Var
  Lc_Observacao : TMemo;
  Lc_SqlTxt : String;
...
begin
  Try
    ...
    Begin
      ...
      if Copy(Qr_Pedido.FieldByName('PED_PRAZO').AsString,1,3) <> '000' then
        Lc_Observacao.Lines.Add('Prazos: ' + Fc_Parcelamento('T',Qr_Pedido));

      if Trim(Qr_Pedido.FieldByName('PED_OBS').AsString) <> '' then
        Lc_Observacao.Lines.Add(Qr_Pedido.FieldByName('PED_OBS').AsString);
      //Cheque Devolvido
      if (Fc_Tb_Geral('L','FIN_G_INF_CHQ_DEV','') = 'S') then
      Begin
        Lc_str_aux := Fc_HistoricoFinanceiro(1,Qr_Pedido.FieldByname('EMP_CODIGO').AsInteger);
      ...
      Begin
        Lc_str_aux := Fc_HistoricoFinanceiro(2,Qr_Pedido.FieldByname('EMP_CODIGO').AsInteger);
        if Trim(Lc_str_aux) <> '' then
          Lc_Observacao.Lines.Add(Lc_str_aux);
      end;
      //Saldo Devedor
      ...
      Begin
        Lc_str_aux := Fc_HistoricoFinanceiro(3,Qr_Pedido.FieldByname('EMP_CODIGO').AsInteger);
        if Trim(Lc_str_aux) <> '' then
          Lc_Observacao.Lines.Add(Lc_str_aux);
      end;
    ...
    E_OBS.Lines.Clear;
    E_OBS.AutoStretch := FAlse;
    E_OBS.Lines.Assign(Lc_Observacao.Lines);
    E_OBS.AutoStretch := True;
    if Lc_Observacao.Lines.count > 0 then
      Lc_Altura := E_OBS.Top + Lc_Observacao.Lines.count * 17
    else
      Lc_Altura := E_OBS.Top + 17;
    Lc_Altura := Lc_Altura + 20;

    Lb_Vendedor.Top := Lc_Altura;
    E_Vendedor.Top := Lc_Altura;
    Lc_Altura := Lc_Altura + E_Vendedor.Height+10;
    Lc_Altura := Lc_Altura+20;
    Lb_LocalData.Top := Lc_Altura;
    E_Assinatura.Top := Lc_Altura + 17;
    Lc_Altura := Lc_Altura+40;
    Shp_Rodape.Height := Lc_Altura;
    Rodape.Height := Lc_Altura;
    Rodape.Repaint;
    Rodape.Refresh;
...
procedure TRL_PedidoVenda_10.Pc_IniciaVariaveis;
Var
  Lc_I : Integer;
Begin
  It_Vl_Servico := 0;
...
procedure TRL_PedidoVenda_10.RodapeBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Pc_Rodape;
end;

procedure TRL_PedidoVenda_10.StrB_ServicoBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if It_Cab_Srv then
  Begin
...
procedure TRL_PedidoVenda_10.StrB_ProdutoBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if It_Cab_Pro then
    Begin
    ...
    Lb_VlUnit_Pro.Caption := 'V. Unitário';
