unit UN_RL_Produto_MaisVendido_Vendedor;
...
type
  TRL_Produto_MaisVendido_Vendedor = class(TForm)
...
var
  RL_Produto_MaisVendido_Vendedor: TRL_Produto_MaisVendido_Vendedor;
...
procedure TRL_Produto_MaisVendido_Vendedor.Pc_Busca;
VaR
  SqlTxt:String;
...
begin
  with Fr_Imp_Produtos do
    ...
    SqlTxt:='SELECT DISTINCT CLB_CODIGO, CLB_NOME, PRO_CODIGOFAB , PRO_DESCRICAO  ,sum(itf_qtde * itf_vl_unit) VDA, sum(ITF_QTDE) QTD '+
            ...
            '    ON (tb_estoque.EST_CODPRO = tb_itens_nfl.ITF_CODPRO) ';

    if Rb_Vend_Cliente.Checked then
      Begin
      SqlTxt := SqlTxt +
            '    INNER JOIN TB_COLABORADOR tb_colaborador '+
            '    ON (tb_colaborador.CLB_CODIGO = tb_cliente.EMP_CODVDOR) ';
      end
    else
      Begin
      SqlTxt := SqlTxt +
    ...
    if It_Tp_Ordenacao = 0 then
      Qr_Produtos.SQL.Add(SqlTxt+'   group by 1,2,3,4 order by 2 asc ,5 desc ') //Ordenarção pelo Valor
    else
      Qr_Produtos.SQL.Add(SqlTxt+'   group by 1,2,3,4 order by 2, 6 desc'); //Ordenação pela Quantidade
...
procedure TRL_Produto_MaisVendido_Vendedor.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  FreeAndNil(Qrpt);
  Action:=caFree;
  RL_Produto_MaisVendido_Vendedor := nil;
end;

procedure TRL_Produto_MaisVendido_Vendedor.QrptBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TRL_Produto_MaisVendido_Vendedor.Pc_Parametros;
begin
  with Fr_Imp_Produtos do
    begin
...
procedure TRL_Produto_MaisVendido_Vendedor.QRGroup1BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  LB_NomeDoVendedor.Caption := Qr_Produtos.FieldByname('CLB_NOME').AsString;
end;
