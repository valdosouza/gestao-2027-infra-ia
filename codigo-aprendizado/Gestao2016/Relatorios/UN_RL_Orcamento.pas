...
var
  RL_Orcamento: TRL_Orcamento;

implementation

uses     Un_Cotacao, Un_Funcoes, UN_Sistema, Un_DM, UN_Pedido_Vda;
{$R *.dfm}
Procedure TRL_Orcamento.PC_VAlidaCampos(Pc_Print : Boolean);
Begin
  if Pc_Print then
    BEgin
...
Procedure TRL_Orcamento.Pc_PreencheGrade;
Var
  Lc_I : integer;
  Lc_Name : String;
Begin
...
Procedure TRL_Orcamento.PC_PreencheCampos;
Begin
...
procedure TRL_Orcamento.Pc_Orcamento;
Var
...
begin
  ...
  if Assigned(Fr_Cotacao) then Lb_Orcamento.Caption := 'Orçamento Nº '+ Qr_Ordem.FieldByName('CTC_NUMERO').AsString;
...
procedure TRL_Orcamento.DetalheVendaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRL_Orcamento.QRBand4BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
Var
  Lc_I : Integer;
begin
  ...
  Lb_Usuario.Caption := UpperCase('Vendedor: '+Gb_NomeUsu);
end;

procedure TRL_Orcamento.FormClose(Sender: TObject;
...
procedure TRL_Orcamento.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRL_Orcamento.QrptBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  Pc_Orcamento
end;
