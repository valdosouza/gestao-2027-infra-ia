object RL_Orcamento_2: TRL_Orcamento_2
  Left = 354
  Top = 197
  ...
  object Qr_Ordem: TSTQuery
    ...
    ParamCheck = True
    SQL.Strings = (
      'SELECT os.tb_order_id,TPV_DESCRICAO,VEI_PLACA, MOD_DESCRICAO'
      'FROM tb_vehicle_so os'
      '   INNER JOIN TB_VEICULO ve'
      '   ON (ve.VEI_PLACA = os.tb_vehicle_id)'
      '   INNER JOIN TB_MARCA_VEICULO mrc'
      '   ON (mrc.MRC_CODIGO = ve.VEI_CODMRC)'
      '   INNER JOIN TB_MODELO md'
      '   ON (md.MOD_CODIGO = ve.VEI_CODMOD)'
      '   INNER JOIN TB_TP_VEICULO tp'
      '   ON (tp.TPV_CODIGO = ve.VEI_CODTPV)'
      'WHERE'
      '  os.tb_order_id =:CTC_CODPED')
    Left = 392
    Top = 16
    ParamData = <
      item
  ...
  object Qr_Cotacao: TSTQuery
    ...
    SQL.Strings = (
      ...
      'fp.fpt_descricao'
      'FROM TB_COTACAO ct'
      '  LEFT OUTER JOIN TB_EMPRESA ep'
      '  ON (ep.EMP_CODIGO = ct.CTC_CODEMP)'
      '  inner join tb_formapagto  fp'
      '  on (fp.fpt_codigo = ct.ctc_codfpg)'
