unit Un_RL_PedidoVenda_3;

interface

uses
      Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, Db, StdCtrls, Qrctrls, QuickRpt, ExtCtrls, STQuery, jpeg, System.Math;

type
  TRL_PedidoVenda_3 = class(TForm)
    Qrpt: TQuickRep;
    Rodape: TQRBand;
    Cabecalho: TQRBand;
...
var
RL_PedidoVenda_3:TRL_PedidoVenda_3;
implementation
...
procedure TRL_PedidoVenda_3.Pc_Orcamento;
VaR
  Lc_I:Integer;
  Lc_Ano,Lc_mes,Lc_dia:String;
begin
  Ex_Logo.AutoSize:=TRUE;
  ...
  Lc_dia := Copy(Qr_Pedido.FieldByName('PED_DATA').AsString,1,2);
  Lc_mes := Fc_DesMes(Fc_Month(Qr_Pedido.FieldByName('PED_DATA').AsDateTime),True);
  Lc_Ano := Copy(Qr_Pedido.FieldByName('PED_DATA').AsString,7,4);
...
procedure TRL_PedidoVenda_3.Pc_Detalhe;
Var
  LcValor : Real;
begin
...
procedure TRL_PedidoVenda_3.Pc_Rodape;
begin
  ...
  Lb_Garantia.Caption := '1 ANO';
  IF Length(Trim(Qr_Pedido.FieldByName('PED_VALIDADE').AsString))>0 then
    Lb_Validade.Caption:=Qr_Pedido.FieldByName('PED_VALIDADE').AsString;
...
procedure TRL_PedidoVenda_3.Pc_cliente;
begin
  with Qr_Pedido do
    Begin
...
procedure TRL_PedidoVenda_3.QrptBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TRL_PedidoVenda_3.Bd_DetalheStrBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Pc_Detalhe;
end;

procedure TRL_PedidoVenda_3.RodapeBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Pc_Rodape;
end;
