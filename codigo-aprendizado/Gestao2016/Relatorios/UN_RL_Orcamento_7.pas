...
var
  RL_Orcamento_7: TRL_Orcamento_7;
...
Procedure TRL_Orcamento_7.Pc_PreenheCli;
BEgin
  if ( Qr_Cliente.RecordCount > 0 ) then
  Begin
    with qr_Cliente do
    Begin
      Lb_Cli_Nome.Caption     := FieldByName('EMP_NOME').AsString + ' - ' + FieldByName('EMP_FANTASIA').AsString;
      ...
      Lb_Cli_fone.Caption     := fc_MascaraFone(FieldByName('END_FONE').AsString);
      ...
      Lb_Cli_ie.Caption       := FieldByName('EMP_INSC_EST').AsString;
  ...
  Begin
    ...
    Begin
      ...
      Lb_Cli_fone.Caption     := fc_MascaraFone(FieldByName('CTC_CONTATO').AsString);
    End;
  End;
  if Qr_Cotacao.FieldByName('ctc_email_e').AsString <> '' then
    Lb_Cli_email.Caption    := Qr_Cotacao.FieldByName('ctc_email_e').AsString;
end;

Procedure TRL_Orcamento_7.PC_VAlidaCampos(Pc_Print : Boolean);
Var
  Lc_Altura:Integer;
Begin
...
Procedure TRL_Orcamento_7.PC_PreencheCampos;
Var
  Lc_Altura : Integer;
  Lc_Obs : TMemo;
  Lc_Produto : String;
...
procedure TRL_Orcamento_7.Pc_Cabecalho;
begin
...
procedure TRL_Orcamento_7.QrptBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
Var
...
begin
  ...
  Qr_Cliente.ParamByName('EMP_CODIGO').AsInteger := Qr_Cotacao.FieldByName('CTC_CODEMP').AsInteger;
...
procedure TRL_Orcamento_7.Bd_DetalheStrBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRL_Orcamento_7.QRBand4BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
Var
  Lc_I : Integer;
  Lc_obs : TMemo;
...
procedure TRL_Orcamento_7.FormClose(Sender: TObject;
...
procedure TRL_Orcamento_7.GetCliente;
Begin
  with Qr_Cliente do
  BEgin
    sql.Clear;
    sql.add(concat(
              ...
              '  ON (cd.CDD_CODIGO = ed.END_CODCDD) ',
              '  INNER JOIN TB_UF uf ',
              '  ON (uf.UFE_CODIGO = ed.END_CODUFE) ',
              'WHERE(EMP_CODIGO =:EMP_CODIGO) ',
              ' and (ed.end_principal = ''S'') '
    ));
  End;
...
procedure TRL_Orcamento_7.GetCotacao;
begin
  with Qr_Cotacao do
  BEgin
    sql.Clear;
...
procedure TRL_Orcamento_7.LimpaCampos;
begin
  Lb_Cli_Nome.Caption     := '';
  ...
  Lb_Cli_cnpj.Caption     := '';
  Lb_Cli_ie.Caption       := '';
  Lb_Cli_Codigo.Caption   := '';
  Lb_Cli_email.Caption    := '';
end;
