...
type
  TRL_PedidoVenda_8 = class(TForm)
    ...
    QRShape1: TQRShape;
    Lb_Qtde_Srv: TQRLabel;
    QRShape4: TQRShape;
    Lb_VlUnit_Srv: TQRLabel;
    QRLabel7: TQRLabel;
...
var
  RL_PedidoVenda_8: TRL_PedidoVenda_8;
...
procedure TRL_PedidoVenda_8.Pc_PreencheCliente;
begin
  with Qr_Pedido do
  Begin
    Lb_Cli_Nome.Caption         := FieldByName('PED_CODEMP').AsString + ' - ' + FieldByName('EMP_NOME').AsString;
...
Begin
  ...
  BEgin
    ...
    Lb_Qtde_Srv.Font.Color := clWhite;
    Lb_VlUnit_Srv.Font.Color := clWhite;
    Lb_SubVl_Srv.Font.Color := clWhite;
  end;
end;

Procedure TRL_PedidoVenda_8.PC_PreencheServicos;
Begin
  ...
  Lb_Qtde_Srv.Caption := formatFloat('#,##0.###',Qr_ItensSrv.FieldByName('ITF_QTDE').AsFloat);
  Lb_VlUnit_Srv.Caption := FloatToStrf(Qr_ItensSrv.FieldByname('ITF_VL_UNIT').AsCurrency,ffFixed,10,2);
...
Procedure TRL_PedidoVenda_8.PC_VAlidaProdutos(Pc_Print : Boolean);
Begin
  if Pc_Print then
    BEgin
...
Procedure TRL_PedidoVenda_8.PC_PreencheProdutos;
Var
  LcValor : Real;
Begin
...
procedure TRL_PedidoVenda_8.Pc_Rodape;
begin
  with Qr_Pedido do
    Begin
...
procedure TRL_PedidoVenda_8.QrptBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TRL_PedidoVenda_8.Pc_IniciaVariaveis;
Var
  Lc_I : Integer;
Begin
  It_Vl_Servico := 0;
  ...
  if Qr_Pedido.FieldByname('PED_FATURADO').AsString = 'S' then
    Begin
    Lb_Titulo.Caption := 'REVISÃO O.S:';
    Lb_Cli_Data.Caption := Qr_Pedido.FieldByname('PED_DATA').AsString + ' - ' + TimetoStr(Time) + ' hs';
    end
  else
    Begin
    Lb_Titulo.Caption := 'ORDEM DE SERVIÇO :';
    Lb_Cli_Data.Caption := Qr_Pedido.FieldByname('PED_DT_ALTERA').AsString + ' hs';
    end;
end;
procedure TRL_PedidoVenda_8.RodapeBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Pc_Rodape;
end;

procedure TRL_PedidoVenda_8.StrB_ServicoBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if It_Cab_Srv then
    Begin
    ...
    Lb_Qtde_Srv.Caption := 'Qtde';
    Lb_VlUnit_Srv.Caption := 'V. Unitário';
...
procedure TRL_PedidoVenda_8.StrB_ProdutoBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if It_Cab_Pro then
    Begin
