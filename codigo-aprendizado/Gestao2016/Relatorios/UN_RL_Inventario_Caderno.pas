...
type
  TRL_Inventario_Caderno = class(TForm)
    Qrpt: TQuickRep;
    QRBand2: TQRBand;
...
var
  RL_Inventario_Caderno: TRL_Inventario_Caderno;

implementation

uses     Un_DM, UN_Imp_Produtos, UN_Sistema, Un_Regra_Negocio, UN_Principal, env;
{$R *.dfm}


procedure TRL_Inventario_Caderno.Pc_Busca;
VaR
   Lc_Desc_Produto,  Lc_RelCusto_Tipo,SqlTxt:String;
...
begin
  ...
  Qr_Produtos.ParamByName('IVT_DATA').AsDateTime  :=  Fr_Imp_Produtos.Qr_Invetario.FieldByName('IVT_DATA').AsDateTime;
  ...
  IF Lc_Desc_Produto <>'' THEN Qr_Produtos.ParamByName('PRO_DESCRICAO').AsString:='%'+ Copy(Lc_Desc_Produto,1,98)+'%';

  Qr_Produtos.Open;
  Lb_Titulo.Caption := 'Inventário do Estoque do Dia ' + Qr_Produtos.FieldbyName('IVT_DATA').AsString;
  Screen.Cursor:=crDefault;
end;

procedure TRL_Inventario_Caderno.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  FreeAndNil(QRPT);
  Action:=caFree;
  RL_Inventario_Caderno:=nil;
end;

procedure TRL_Inventario_Caderno.QrptBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TRL_Inventario_Caderno.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Try
  ...
  Except
    ShowMessage(Qr_Produtos.FieldByName('PRO_DESCRICAO').AsString);
  End;
end;

procedure TRL_Inventario_Caderno.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRL_Inventario_Caderno.Pc_Parametros;
begin
