...
uses
       Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs, Buttons, DB, STStoredProc, StdCtrls, DBCtrls, Mask, ExtCtrls, STQuery, Printers, ComCtrls, Grids, System.Math, QEdit_Setes, STDatabase, STTransaction;

type

   TFr_Fatura_Web = class(TForm)
      Qr_Acao: TSTQuery;
...
var
   Fr_Fatura_Web: TFr_Fatura_Web;

implementation


uses
Un_DM, Un_Msg, Un_Funcoes, UN_Empresa, UN_Sistema, Un_MinhaEmpresa, Un_GeraNFe, un_Padrao, Un_Fc_Sored_Procedures, UN_NF, Un_Regra_Negocio, Un_Transportadora, RN_NotaFiscalEletronica, UN_Principal, env, RN_Inventario, RN_Estoque, RN_NotaFiscal, RN_Transportadora, RN_PlanoContas, RN_Permissao;

{$R *.dfm}

procedure TFr_Fatura_Web.Pc_RollBack_Faturamento;
Begin
  ...
  if (It_Etapa_Faturamento >= 1) then Pc_ApagaImpostos;
  if (It_Etapa_Faturamento >= 2) then
    Begin
    Pc_ApagaNF(IBT_Faturamento,It_Cd_Nota);
    Pc_Apaga_conserto(IBT_Faturamento,It_cd_Pedido);
    end;
  if (It_Etapa_Faturamento >= 3) then Pc_ApagaObservacao(NotaFiscal.Registro.codigo,NotaFiscal.Registro.Terminal);
...
procedure TFr_Fatura_Web.Pc_ImagemBotao;
Begin
...
procedure TFr_Fatura_Web.Pc_FormataTela;
Begin
...
procedure TFr_Fatura_Web.Pc_Inicia_Variaveis;
Begin
...
procedure TFr_Fatura_Web.Pc_CalculaTotalPedido;
begin
   Qr_Itens.First;
   It_Vl_TL_Pedido := 0;
...
function TFr_Fatura_Web.Fc_tributacao(Pc_Cd_Produto: Integer; Pc_St: string; Pc_Origem:String; Pc_Finalidade:String;Pc_NCM:String): Integer;
var
...
procedure TFr_Fatura_Web.Pc_ApagaImpostos;
begin

end;

procedure TFr_Fatura_Web.Pc_AtivarTabela;
begin
...
procedure TFr_Fatura_Web.Pc_Totalizador;
var
   Lc_Qr_Acao: TSTQuery;
...
function TFr_Fatura_Web.Fc_Valida_Itens_Nota():Boolean;
Var
...
Begin
  ...
  while not Qr_Itens.Eof do
    begin
    IF (Fc_Tributacao(Qr_Itens.FieldByName('ITF_CODPRO').AsInteger,
                      ...
                      Qr_Itens.FieldByName('PRO_CODIGONCM').AsString) = -1) then
      Begin
      E_M_Fisco.Lines.Add(Qr_Itens.FieldByName('ITF_CODPRO').AsString + ' - ' + Qr_Itens.FieldByName('PRO_DESCRICAO').AsString);
      //Monta a mensagem para produtos com Substituição tributária
      if (Qr_Itens.FieldByName('PRO_SUB_TRIB').AsString <> 'S') then
        E_M_Fisco.Lines.Add('      ' + 'Este produto não está sob o regime de Substituição tributária.')
      else
        E_M_Fisco.Lines.Add('      ' + 'Este produto está sob o regime de Substituição tributária.');
      //Monta a Mensagem para a Finalidade de Tributação
      if Trim(Qr_Itens.FieldByName('PRO_TRIBUTACAO').AsString) = '' then
        E_M_Fisco.Lines.Add('      ' + 'Finalidade para a tributação não foi definido.')
      else
        Begin
        case StrToIntDef(Qr_Itens.FieldByName('PRO_TRIBUTACAO').AsString,0) of
        ...
        E_M_Fisco.Lines.Add('      ' + 'Finalidade para a tributação está como '+ Lc_Tp_tributa +'.');
        end;
       //Monta a Mensagem para a origem da Mercadoria
        case StrToIntDef(Qr_Itens.FieldByName('PRO_ORIGEM').AsString,0) of
          0:E_M_Fisco.Lines.Add('      ' + 'A origem deste produto É Nacional.');
          1:E_M_Fisco.Lines.Add('      ' + 'A origem deste produto Importação Direta.');
          2:E_M_Fisco.Lines.Add('      ' + 'A origem deste produto Estrangeira Adquirida no Mercado Interno.');
        end;
      //Pede verificação do CFOP em Relação a operação
      E_M_Fisco.Lines.Add('');
      E_M_Fisco.Lines.Add('Verifique se o CFOP utilizado é o correto para operação de ' + Lc_Tp_tributa);
      E_M_Fisco.Lines.Add('Verifique se o CFOP utilizado é o correto para estado do ' + Qr_PedidoUFE_SIGLA.AsString);
      end;
    Qr_Itens.Next;
    end;

  if (Length(E_M_Fisco.Text) >0 ) then
    Begin
    E_M_Fisco.Lines.Add('');
    E_M_Fisco.Lines.Add('Existe regra para estes critérios relacionado ao cliente?');
    //Mostra o Estado do cliente
    E_M_Fisco.Lines.Add('       1º Para o estado do '+ Qr_PedidoUFE_SIGLA.AsString);
    //Monta a mensagem para tipo do Cliente consumidor / Revenda
    if Qr_PedidoEMP_CONSUMIDOR.AsString = 'S' then
      Lc_Tp_Cliente := 'Consumidor'
    else
      Lc_Tp_Cliente := 'Revenda';
    E_M_Fisco.Lines.Add('       º Para cliente do tipo ' + Lc_Tp_Cliente);

    //Monta a mensagem Cliente micro empresa ou não 
    if Qr_PedidoEMP_MICRO.AsString = 'S' then
      E_M_Fisco.Lines.Add('       3º Este cliente está marcado como micro empresa.')
    else
      E_M_Fisco.Lines.Add('       3º Este cliente não está marcado como micro empresa.');
  ...
  //valida o Preenchimento do NCM
  Pc_SelecionaItensNota(0,0,'');
  E_M_Fisco.Clear;
  Qr_Itens.First;
  while not Qr_Itens.Eof do
    begin
    if (TRIM(Qr_Itens.FieldByName('PRO_CODIGONCM').AsString) = '')or
      (Length(Qr_Itens.FieldByName('PRO_CODIGONCM').AsString)<>8) or
      (TRIM(Qr_Itens.FieldByName('PRO_CODIGONCM').AsString) = '00000000') then
      E_M_Fisco.Lines.Add(Qr_Itens.FieldByName('ITF_CODPRO').AsString + ' - ' + Qr_Itens.FieldByName('PRO_DESCRICAO').AsString);
    Qr_Itens.Next;
    end;
  if (Length(E_M_Fisco.Text) >0 ) then
  ...
  while not Qr_Itens.Eof do
    begin
    if (TRIM(Qr_Itens.FieldByName('MED_ABREVIATURA').AsString) = '') then
      E_M_Fisco.Lines.Add(Qr_Itens.FieldByName('ITF_CODPRO').AsString + ' - ' + Qr_Itens.FieldByName('PRO_DESCRICAO').AsString);
    Qr_Itens.Next;
    end;
  if (Length(E_M_Fisco.Text) >0 ) then
  ...
  if not (Fc_Tb_Geral('L','PRO_G_LABEL_PRO','')='S') then
    ...
    while not Qr_Itens.Eof do
      begin
        if (TRIM(Qr_Itens.FieldByName('PRO_CODIGOBAR').AsString) <> '') and
          ((Length(Trim(Qr_Itens.FieldByName('PRO_CODIGOBAR').AsString))<8) or (Length(Trim(Qr_Itens.FieldByName('PRO_CODIGOBAR').AsString))>14)) then
        E_M_Fisco.Lines.Add(Qr_Itens.FieldByName('ITF_CODPRO').AsString + ' - ' + Qr_Itens.FieldByName('PRO_DESCRICAO').AsString);
      Qr_Itens.Next;
      end;
    if (Length(E_M_Fisco.Text) >0 ) then
...
function TFr_Fatura_Web.Fc_VerificaEntregaDiferente(Pc_cd_cliente,Pc_cd_Pedido:Integer): Boolean;
var
  Lc_SqlTxt: String;
...
function TFr_Fatura_Web.fc_ValidaFaturamento(): Boolean;
Var
  Lc_Aux : String;
  Lc_Nr_Nota: Integer;
...
begin
  ...
  If It_Usa_Nfe then
    begin
    if NOT Fc_ValidaCep(Qr_PedidoEND_CEP.AsString) then
      begin
       Result:=false;
       exit;
       end;
    ...
    if not Fc_Valida_Dados_Destinatario_NFE(Qr_Pedido) then
      Begin
      Result := False;
      exit;
      end;
...
procedure TFr_Fatura_Web.Pc_GravaUltimasConfiguracoes;
Begin
...
procedure TFr_Fatura_Web.Pc_Faturar_Venda_Servico;
Var
  Lc_Result_Fatura : Boolean;
Begin
  It_Nr_Pedido := Qr_PedidoPED_NUMERO.AsInteger;
  Pc_Define_FormatoRegional;
  Lc_Result_Fatura := Fc_Faturar_Normal;
  if Lc_Result_Fatura then
    Begin
...
procedure TFr_Fatura_Web.SB_ConfirmarClick(Sender: TObject);
begin
...
procedure TFr_Fatura_Web.SB_Sair_0Click(Sender: TObject);
begin
   Close;
end;

procedure TFr_Fatura_Web.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TFr_Fatura_Web.DBLCB_TransportadoraKeyDown(Sender: TObject;
...
procedure TFr_Fatura_Web.FormCreate(Sender: TObject);
Var
  Lc_Nfe : boolean;
begin
  Pc_AtivarTabela;
...
procedure TFr_Fatura_Web.Sb_TransportadoraClick(Sender: TObject);
begin
...
procedure TFr_Fatura_Web.FormKeyDown(Sender: TObject; var Key: Word;
   Shift: TShiftState);
begin
   if shift = [] then
   begin
...
procedure TFr_Fatura_Web.FormShow(Sender: TObject);
var
  CanSelect: boolean;
begin
  Pc_FormataTela;
...
procedure TFr_Fatura_Web.Pc_SelecionaItensNota(Fc_Nr_Itens: Integer; Fc_Inicio: Integer; Fc_TP_Produto: string);
var
   Lc_SqlTxt: string;
begin
...
procedure TFr_Fatura_Web.Pc_AtualizarItensNota;
var
  Lc_Despachar:String;
...
procedure TFr_Fatura_Web.Pc_CalculaTotalProduto;
begin
   Qr_Itens.First;
   It_Vl_TL_Pedido := 0;
   while not Qr_Itens.Eof do
...
procedure TFr_Fatura_Web.Pc_Calcula_Impostos;
var
  Lc_Tx_Prop: Real;
  Lc_VL_frete : Real;
  Lc_Vl_Seguro :Real;
...
begin
  ...
  For Lc_I := 1 to Lc_Nr_Registros do
    ...
    if (Lc_I < Lc_Nr_Registros) then
      Begin
      Lc_Tx_Prop := (Qr_Itens.FieldByName('ITF_VL_UNIT').AsCurrency * Qr_Itens.FieldByName('ITF_QTDE').AsFloat) - Qr_Itens.FieldByName('ITF_VL_DESC').AsCurrency;
...
procedure TFr_Fatura_Web.Pc_AtualizarItensICMS;
Begin
...
procedure TFr_Fatura_Web.Pc_AtualizarItensIPI;
begin
...
procedure TFr_Fatura_Web.Pc_AtualizarItensPIS;
begin
...
procedure TFr_Fatura_Web.Pc_AtualizarItensCOFINS;
begin
...
function TFr_Fatura_Web.Fc_ConfirmaValores(): boolean;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_NotaFiscal;
Begin
  Qr_Acao.Active := False;
  Qr_Acao.Transaction := IBT_Faturamento;
...
procedure TFr_Fatura_Web.Pc_Obs_Financeira;
var
   Lc_I: Integer;
   Lc_Fim: Integer;
   lc_texto: string;
...
Procedure TFr_Fatura_Web.Pc_Obs_Manual;
Begin
...
Procedure TFr_Fatura_Web.Pc_Obs_Regra_Geral;
var
  Lc_SqlTxt : String;
...
procedure TFr_Fatura_Web.Pc_Obs_CST00;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_CST10;
Var
  Lc_vl_Bc_St : Real;
  Lc_vl_St : Real;
  Lc_SqlTxt : String;
...
procedure TFr_Fatura_Web.Pc_Obs_CST20;
Var
  Lc_Aq_Rd_bc : Real;
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
...
procedure TFr_Fatura_Web.Pc_Obs_CST30;
Var
  Lc_vl_Bc_St : Real;
  Lc_vl_St : Real;
  Lc_SqlTxt : String;
...
procedure TFr_Fatura_Web.Pc_Obs_CST40;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_CST50;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_CST60;
Var
  Lc_vl_Bc_St : Real;
  Lc_vl_St : Real;
  Lc_SqlTxt : String;
...
procedure TFr_Fatura_Web.Pc_Obs_CST70;
Var
  Lc_Aq_rd_Bc_Nr : Real;
  Lc_vl_Bc_St : Real;
  Lc_vl_St : Real;
...
procedure TFr_Fatura_Web.Pc_Obs_CST80;
Begin

end;

procedure TFr_Fatura_Web.Pc_Obs_CST90;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_CSOSN101;
Var
  Lc_Aq : Real;
  Lc_VL : Real;
  Lc_SqlTxt : String;
...
procedure TFr_Fatura_Web.Pc_Obs_CSOSN102;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_CSOSN103;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_CSOSN201;
Var
  Lc_vl_bc_st : Real;
  Lc_vl_st : Real;
  Lc_Aq : Real;
...
procedure TFr_Fatura_Web.Pc_Obs_CSOSN202;
Var
  Lc_vl_bc_st : Real;
  Lc_vl_st : Real;
  Lc_SqlTxt : String;
...
procedure TFr_Fatura_Web.Pc_Obs_CSOSN203;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_CSOSN300;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_CSOSN400;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_CSOSN500;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Obs_CSOSN900;
Var
  Lc_SqlTxt : String;
  Lc_texto :TMemoryStream;
begin
...
procedure TFr_Fatura_Web.Pc_Gera_NotaFiscal;
var
  Lc_SerieNF: Integer;
...
function TFr_Fatura_Web.Fc_SomaCreditoICMS(): Real;
var
   Lc_Vl_Credito: Real;
   Lc_Aq_ICMS: Real;
begin
...
procedure TFr_Fatura_Web.Pc_Condicao_STDPRREV;
begin
...
procedure TFr_Fatura_Web.Pc_Condicao_STDPRCSD;
begin
  {Não se Aplica
  ...
  alguns Protocolos/Convânios que dispõe sobre o regime da Substituição Tributária
  ...
  (No caso específico de lêmpadas os Estados de BA e PR não exigem a Substituição
...
procedure TFr_Fatura_Web.Pc_Condicao_STDPRREVREQ;
begin
  //Calculo do icms da opereação propria
  It_Vl_Bc_ICMS := (Qr_Itens.fieldByname('ITF_VL_UNIT').AsCurrency * Qr_Itens.fieldByname('ITF_QTDE').AsFloat) - Qr_Itens.fieldByname('ITF_VL_DESC').AsCurrency;
  It_Vl_Bc_ICMS := It_Vl_Bc_ICMS + It_VL_frete_Produto + It_Vl_Seguro_Produto + It_Vl_Outras_Produto;
...
procedure TFr_Fatura_Web.Pc_Condicao_NRDPRREV;
begin
  //Calculo do icms da opereação propria
  It_Vl_Bc_ICMS := (Qr_Itens.fieldByname('ITF_VL_UNIT').AsCurrency * Qr_Itens.fieldByname('ITF_QTDE').AsFloat) - Qr_Itens.fieldByname('ITF_VL_DESC').AsCurrency;
  It_Vl_Bc_ICMS := It_Vl_Bc_ICMS + It_VL_frete_Produto + It_Vl_Seguro_Produto + It_Vl_Outras_Produto;
...
procedure TFr_Fatura_Web.Pc_Condicao_NRDPRCSD;
begin
  //Calculo do icms da opereação propria
  It_Vl_Bc_ICMS := (Qr_Itens.fieldByname('ITF_VL_UNIT').AsCurrency * Qr_Itens.fieldByname('ITF_QTDE').AsFloat) - Qr_Itens.fieldByname('ITF_VL_DESC').AsCurrency;
  It_Vl_Bc_ICMS := It_Vl_Bc_ICMS + It_VL_frete_Produto + It_Vl_Seguro_Produto + It_Vl_Outras_Produto;
...
procedure TFr_Fatura_Web.Pc_Condicao_STFPRREV;
begin
  //Calculo do icms da opereação propria
  It_Vl_Bc_ICMS := (Qr_Itens.fieldByname('ITF_VL_UNIT').AsCurrency * Qr_Itens.fieldByname('ITF_QTDE').AsFloat) - Qr_Itens.fieldByname('ITF_VL_DESC').AsCurrency;
  It_Vl_Bc_ICMS := It_Vl_Bc_ICMS + It_VL_frete_Produto + It_Vl_Seguro_Produto + It_Vl_Outras_Produto;
...
procedure TFr_Fatura_Web.Pc_Condicao_STFPRCSD;
begin
...
procedure TFr_Fatura_Web.Pc_Condicao_NRFPRREV;
begin
  //Calculo do icms da opereação propria
  It_Vl_Bc_ICMS := (Qr_Itens.fieldByname('ITF_VL_UNIT').AsCurrency * Qr_Itens.fieldByname('ITF_QTDE').AsFloat) - Qr_Itens.fieldByname('ITF_VL_DESC').AsCurrency;
  It_Vl_Bc_ICMS := It_Vl_Bc_ICMS + It_VL_frete_Produto + It_Vl_Seguro_Produto + It_Vl_Outras_Produto;
...
procedure TFr_Fatura_Web.Pc_Condicao_NRFPRCSD;
begin
  //Calculo do icms da opereação propria
  It_Vl_Bc_ICMS := (Qr_Itens.fieldByname('ITF_VL_UNIT').AsCurrency * Qr_Itens.fieldByname('ITF_QTDE').AsFloat) - Qr_Itens.fieldByname('ITF_VL_DESC').AsCurrency;
  It_Vl_Bc_ICMS := It_Vl_Bc_ICMS + It_VL_frete_Produto + It_Vl_Seguro_Produto + It_Vl_Outras_Produto;
...
function TFr_Fatura_Web.Fc_Faturar_Normal():Boolean;
begin
  Result := True;
  ...
  try
    ...
    if IBT_Faturamento.InTransaction then IBT_Faturamento.Commit;
    tributacao.
    Pc_AlteraStatusPedido(IBT_Faturamento,It_Cd_Pedido,'S');
    ...
    if ChBx_AutorizacaoNFE.Checked then
      begin
        if Fc_VerificaPermissao('Fr_Nota_Fiscal_3_X','Nota Fiscal Eletrônica 3.x','VISUALIZAR') then
          Fc_ChamarGeraNFE3X('N',It_Cd_Pedido,It_Cd_Nota,'1')
        else
          Fc_ChamarGeraNFE('N',It_Cd_Pedido,It_Cd_Nota,'1');
      end;
...
procedure TFr_Fatura_Web.Pc_Atualiza_Itens_Devolucao;
Var
...
Procedure TFr_Fatura_Web.chbx_data_saidaClick(Sender: TObject);
begin
  E_Dt_Saida.Enabled := chbx_data_saida.Checked;
end;

procedure TFr_Fatura_Web.E_Qt_ProdutoChange(Sender: TObject);
Var
  Lc_Ativa : boolean;
begin
...
procedure TFr_Fatura_Web.ChBx_AutorizacaoNFEClick(Sender: TObject);
begin
  if (ChBx_AutorizacaoNFE.Checked) then
  Begin
    CkBx_Nota_Manual.Checked := False;
    CkBx_Nota_Manual.Enabled := False;
  end
  else
  Begin
    CkBx_Nota_Manual.Enabled := True;
  end;
end;
