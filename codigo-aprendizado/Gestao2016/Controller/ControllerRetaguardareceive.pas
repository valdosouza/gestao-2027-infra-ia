...
uses     STDatabase, Classes, SysUtils, ControllerBase, tblRetaguardaSync, Generics.Collections, STQuery, ControllerUsuario, tblSincronia, ControllerCargo, ControllerColaborador, ControllerEmpresa, ControllerFormaPagamento, ControllerGrupos, ControllerSubGrupos, ControllerMarcaProduto, ControllerMedida, ControllerEmbalagem, ControllerProduto, ControllerEstoques, ControllerEstoque, ControllerTabelaPreco, ControllerPreco;
...
begin
  FreeAndNil(FCtrlUsuario);
  FreeAndNil(FCtrlCargo);
  FreeAndNil(FCtrlColaborador);
  FreeAndNil(FCtrlEmpresa);
  FreeAndNil(FCtrlFormaPagto);
  FreeAndNil(FCtrlGrupo);
  FreeAndNil(FCtrlSubGrupo);
  FreeAndNil(FCtrlMarca);
  FreeAndNil(FCtrlMedida);
  FreeAndNil(FCtrlEmbalagem);
  FreeAndNil(FCtrlProduto);
  FreeAndNil(FCtrlEstoques);
  FreeAndNil(FCtrlEstoque);
  FreeAndNil(FCtrltTabela);
  FreeAndNil(FCtrlPreco);
