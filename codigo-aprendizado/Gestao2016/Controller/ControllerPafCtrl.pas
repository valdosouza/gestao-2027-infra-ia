...
uses     STDatabase, Classes, STQuery, SysUtils, ControllerBase, tblPafCtrl, Un_MSg, Generics.Collections, ControllerPafRegA2, ControllerPafRegE2;
  ...
  Try
    ...
    Begin
      SQL.Add(concat(
              ...
              ' FP.fpt_descricao, ',
              ' NF.nfl_modelo, ',
              ' CAST(COALESCE(SUM(fnc.fin_vl_pago), 0) AS NUMERIC(18, 2)) TotalPago, ',
              ' CAST(COALESCE(SUM(fnc.fin_vl_parcela), 0) AS NUMERIC(18, 2)) TotalPagar ',
      ...
      Begin
        ...
        Begin
          ...
          if FieldByName('TOTALPAGO').AsCurrency > 0 then
            RegA2.Registro.Valor         := FieldByName('TOTALPAGO').AsCurrency
          else
            RegA2.Registro.Valor         := FieldByName('TOTALPAGAR').AsCurrency;
          RegA2.salva;
