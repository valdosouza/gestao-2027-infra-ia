...
uses     STDatabase, STQuery, System.Classes, System.SysUtils, ControllerBase, tblImpostos, prm_taxes;
...
begin
  ...
  Try
    if Registro.Codigo = 0 then
      Registro.Codigo := getNextByField(Registro,'IPT_CODIGO',0);
    SaveObj(Registro);
  Except
    Result := False;
  End;
...
procedure TControllerImpostos.getFirst;
var
  Lc_Qry : TSTQuery;
begin
  Lc_Qry := GeraQuery;
  Try
    with Lc_Qry do
    Begin
      SQL.Text := ' SELECT * FROM TB_IMPOSTOS';
      Active := True;
      FetchAll;
      First;
      exist := recordCount > 0;
