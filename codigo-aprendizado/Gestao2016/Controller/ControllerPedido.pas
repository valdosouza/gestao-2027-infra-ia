...
interface
uses      STDatabase, Classes, STQuery, SysUtils, ControllerBase, tblPedido, Un_MSg, ControllerPedidoCtrlMmb, Datasnap.DBClient, ControllerEmpresa, ControllerEndereco, ControllerCliente, ControllerGestaoWeb, ControllerItensNFL, Generics.Collections, ControllerParcelamento, ControllerColaborador, System.Math, objOrderSale, tblOrderItem, ObjCustomer, ControllerFormaPagamento, ControllerCtrlEstoque, ControllerRestaurante, ACBrUtil.Math, prm_order, ControllerDespacho;
 ...
 //Cria a Qry de Atualizção
  ...
  Try
    ...
    Begin
      ...
      Begin
        SQL.Clear;
        //O desconto vai atingir somente os produtos e não os serviços devidoa questães fiscais na NFS-e
        //Seleção do valor unitario acima de 0,05 centavos pois no rateio o valor do desconto poderã ser maior do que o valor unitario
        ...
        Active := True;
        FetchAll;
        //Calcula o valor total da Cotção
        First;
        while not eof do
        Begin
          Lc_VL_Pedido := Lc_VL_Pedido + (FieldByName('ITF_VL_UNIT').AsCurrency * FieldByName('ITF_QTDE').AsFloat);
          next;
        end;
        ...
        begin
          ...
          Lc_Vl_Subtotal := (FieldByName('ITF_VL_UNIT').AsCurrency * FieldByName('ITF_QTDE').AsFloat);
          ...
          //Avanãa para o proximo registro
          Next;
          //Verifica se e o ultimo registro para fazer compensção de arredondamento
          ...
          Lc_Upt.ParamByName('ITF_VL_DESC').AsCurrency := Lc_Vl_Desconto;
          Lc_Upt.ExecSQL;
...
begin
  ...
  Despacho  := TControllerDespacho.create(Self);
  ...
  Try
    ...
    Begin
      ...
      begin
        ...
        ParamByName('PED_DATAINI').AsDate := FParametros.DataInicial;
        ParamByName('PED_DATAFIM').AsDate := FParametros.DataFinal;
      end;
...
procedure TControllerPedido.UpdateFormaPagto;
Var
  Lc_Qry: TSTQuery;
Begin
  Lc_Qry := GeraQuery;
...
begin
  ...
  try
    ...
    Begin
      ...
      ParamByName('PED_VL_PRODUTO').AsCurrency := Registro.ValorProdutos;
      ParamByName('PED_VL_IPI').AsCurrency := Registro.ValorIPI;
      ParamByName('PED_VL_FRETE').AsCurrency := Registro.ValorFrete;
      ParamByName('PED_ALIQ_DESCONTO').AsCurrency := Registro.AliqDesconto;
      ParamByName('PED_VL_DESCONTO').AsCurrency := Registro.ValorDesconto;
      ParamByName('PED_VL_PEDIDO').AsCurrency := Registro.ValorPedido;
      ParamByName('PED_FATURADO').asString := Registro.Faturado;
      ...
      ParamByName('PED_VL_SERVICO').AsCurrency := Registro.valorServico;
      ...
      ParamByName('PED_VL_ODESPESA').AsCurrency := Registro.ValorOutrasDEspesas;
      ...
      ParamByName('PED_VL_CREDITO').AsCurrency := Registro.ValorCredito;
      ...
      ParamByName('PED_VL_ST').AsCurrency := Registro.ValorSubstTributaria;
      ParamByName('PED_TERMINAL').asInteger := Registro.Terminal;
  ...
  Try
    ...
    Begin
      sql.add(concat(
                ...
                'WHERE ( NFL_CODIGO =:NFL_CODIGO ) '//O CONTORLE ã FEITO PELA TB_NOTA_FISCAL
      ));
      if registro.Tipo > 0 then
        sql.add(' AND ( PED_TIPO =:PED_TIPO ) ');
...
Begin
  ...
  try
    ...
    Begin
      ...
      ParamByName('PED_APROVADO').AsString := SIGLA_S;
      ParamByName('PED_CODIGO').AsInteger := Registro.Codigo;
      ExecSQL;
    end;
  finally
  ...
  Try
    ...
    Begin
      ...
      begin
        ...
        Lc_Valor := Lc_Valor - FieldByName('ITF_VL_DESC').AsCurrency;
  ...
  Try
    ...
    Begin
      ...
      begin
        ...
        Lc_Subtotal := Lc_Subtotal - FieldByName('ITF_VL_DESC').AsCurrency;
        Result := Result + Lc_Subtotal;
        Next;
      end;
      Result := Result + FieldByName('ped_vl_frete').AsCurrency;
      Result := RoundTO( Result,-2);
    End;
