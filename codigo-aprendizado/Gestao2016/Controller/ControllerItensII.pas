...
uses     STDatabase, System.Classes, Vcl.Grids, STQuery, System.SysUtils, ControllerBase, tblItensII, System.Generics.Collections, controllerProduto;
...
procedure TControllerItensII.deleteByNF;
Var
  Lc_Qry : TSTQuery;
Begin
  Lc_Qry := GeraQuery;
...
begin
  ...
  if not Assigned(Registro) then
  begin
    geralog('ControllerItensII.insere', 'Registro nil');
    Result := False;
    Exit;
  end;
  try
    Registro.Codigo := Generator('GN_ITENS_II');
    InsertObj(Registro);
  Except
    on E: Exception do
    begin
      geralog('ControllerItensII.insere', E.Message);
      Result := False;
    end;
  end;
