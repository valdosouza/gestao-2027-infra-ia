...
uses
      ControllerBase, System.Math, ACBrEAD, ComCtrls, ACBrBase, ACBrPAF_W, ACBrPAF, System.Classes, ControllerEstabelecimento, System.SysUtils, ControllerPafCtrl, ACBrPAF_E, ACBrPAFRegistros;
...
Type
  ...
  private
    ...
    //U1 - Identificção   do Estabelecimento Usuário do PAF-NFC-e
    procedure GeraPafNfcE_U1;
    //A2 - Total Di�rio de Meios de Pagamento
    procedure GeraPafNfcE_A2;
    //P2 - Relção das Mercadorias e Serviços
    procedure GeraPafNfcE_P2;
    //E2 - Relção das Mercadorias em Estoque
    procedure GeraPafNfcE_E2;
    //D2 - Relção dos DAV Emitidos
    procedure GeraPafNfcE_D2;
    //D3 - Detalhe do DAV 	Número do DAV Número do Item
    procedure GeraPafNfcE_D3;
    //D4 - Log de Alterção de Itens do DAV 	Número do DAV Data da alterção Hora da alterção
    ...
    //J2 - Detalhe da NFC-e emitida em conting�ncia pelo PAF-NFC-e
    procedure GeraPafNfcE_J;

    //Z1 	Identificção do usuário do PAF-NFC-e 	1º registro (�nico)
    procedure GeraPafNfcE_Z1;
    //Z2 	Identificção da Empresa desenvolvedora do PAF-NFC-e 	2º registro (�nico)
    procedure GeraPafNfcE_Z2;
    //Z3 	Identificção do PAF-NFC-e 	3º registro (�nico)
    procedure GeraPafNfcE_Z3;
    //Z4 	Totalizção de Vendas a CPF/CNPJ 	Vendas a CPF/CNPJ
    procedure GeraPafNfcE_Z4;
    //Z9 	Totalizção de registros 	Pen�ltimo registro (�nico)
    procedure GeraPafNfcE_Z9;

    //W1 	Identificção do usuário do PAF-NFC-e 	1º registro (�nico)
    procedure GeraPafNfcE_W1;
    //W2 	Identificção da empresa desenvolvedora do PAF-NFC-e 	2º registro (�nico)
    procedure GeraPafNfcE_W2;
    //W3 	Identificção do PAF-NFC-e 	3º registro (�nico)
    procedure GeraPafNfcE_W3;
    //W4 	Identificção da RE 	Detalhamento RE 	A
    procedure GeraPafNfcE_W4;
    //W5 	Totalizção de Registros 	Pen�ltimo registro (�nico)
...
uses     ControllerPafRegA2, ACBrPAF_A, ACBrPAF_P, ControllerPafRegP2, ControllerPafRegE2, ControllerPafRegJ1, ControllerPafRegJ2, ACBrPAF_J, ControllerPafRegZ4, ACBrPAF_Z;
...
begin
  FreeAndNil(FListaEstoqueParcial);
  FreeAndNil(FUsuarioPAF);
...
begin
  Try
    ...
    begin
      ...
      NovoRegistroA2.TIPO_DOC       := RegA2.Lista[I].TipoDocumento; // 1-NFC-e 2-NF-e 3-Operção não tributável, identificando o CPF ou CNPJ do cliente.
      NovoRegistroA2.VL             := RegA2.Lista[I].Valor;
      NovoRegistroA2.CNPJ           := RegA2.Lista[I].CnpjCpfCliente; // S� � preciso informar para o tipo de documento "3"
      NovoRegistroA2.NUMDOCUMENTO   := RegA2.Lista[I].NumeroDocumento ; // S� � preciso informar para o tipo de documento "3"
