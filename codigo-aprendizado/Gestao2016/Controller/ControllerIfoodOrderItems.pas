...
uses     STDatabase, Classes, STQuery, SysUtils, ControllerBase, tblIfoodOrderItems, Un_MSg, Generics.Collections, ControllerIfoodOrderItemsOptions;
...
begin
  ...
  Try
    ...
    Begin
      sql.add(concat(
                'SELECT CAST(COALESCE(sum(quantity), 0) AS DOUBLE PRECISION) qtty ',
                'FROM TB_IFOOD_ORDER_ITEMS ',
                'Where TB_IFOOD_ORDER_ID =:TB_IFOOD_ORDER_ID '
      ));
      ParamByName('TB_IFOOD_ORDER_ID').AsString := Registro.OrderId;
