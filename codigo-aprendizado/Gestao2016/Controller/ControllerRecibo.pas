...
uses     ControllerBase, tblrecibo, System.Classes, System.SysUtils, STQuery, prm_receipt, System.Generics.Collections;
...
begin
  ...
  if registro.Numero = 0 then
    registro.Numero := getNextByField(Registro,'REC_NUMERO',Registro.Estabelecimento);
  ...
  Try
    ...
    Begin
      ...
      if FParametros.Periodo then
        sql.add(' and REC_DATA BETWEEN :DATAINI and :DATAFIM ');

      if FParametros.FieldName.Emitente <> EmptyStr then
        sql.add(' and REC_EMITENTE like :REC_EMITENTE ');

      if FParametros.FieldName.Sacado <> EmptyStr then
        sql.add(' and REC_SACADO like :REC_SACADO ');

      //Passagem de PArametro
      IF FParametros.FieldName.Numero > 0 THEN
        ParamByName('REC_NUMERO').asInteger := FParametros.FieldName.Numero;

      IF FParametros.FieldName.Valor > 0 THEN
        ParamByName('REC_VALOR').AsCurrency := FParametros.FieldName.Valor;

      if FParametros.Periodo then
      Begin
        ParamByName('DATAINI').asDateTime := FParametros.DataInicial;
        ParamByName('DATAFIM').asDateTime := FParametros.DataFinal;
      End;

      if FParametros.FieldName.Emitente <> EmptyStr then
        ParamByName('REC_EMITENTE').asString := concat('%',FParametros.FieldName.Emitente,'%');

      if FParametros.FieldName.Sacado <> EmptyStr then
        ParamByName('REC_SACADO').asString := concat('%',FParametros.FieldName.Sacado,'%');
