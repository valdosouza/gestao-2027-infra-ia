...
uses     STDatabase, Classes, STQuery, SysUtils, ControllerBase, Un_MSg, Generics.Collections;
...
Type
  ...
  public
    ...
    //03 	Totalizção Mensal 	Total de vendas no mes, com duas casas decimais, ao CPF/CNPJ indicado no campo
...
begin
  ...
  Try
    ...
    Begin
      sql.add(concat(
                'SELECT ep.emp_cnpj, CAST(COALESCE(sum(n.nfl_vl_tl_nota), 0) AS NUMERIC(18, 2)) TOTAL ',
      ...
      Begin
        ...
        LITem.TotalizadorMensal  := FieldByname('TOTAL').AsCurrency;
        Lista.add(LITem);
        next;
      end;
    end;
