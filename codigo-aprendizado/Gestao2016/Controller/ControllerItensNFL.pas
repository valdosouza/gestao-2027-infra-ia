...
uses     STDatabase, Classes, STQuery, SysUtils, ControllerBase, prm_ItensNFL, tblItensNFL, ControllerItensICMS, ControllerItensIPI, ControllerItensPis, ControllerItensCofins, ControllerItensII, Generics.Collections, controllerProduto, ControllerItensISSQN, ControllerItensNFLTributacao, ControllerExchangeBasis, System.Generics.Collections, ControllerItensRTR, ControllerItensAvulso, ControllerIbscbs;
...
begin
  ...
  try
    ...
    Begin
      ...
      ParamByName('ITF_VL_CUSTO').AsCurrency := Registro.ValorCusto;
...
begin
  ...
  try
    ...
    Begin
      ...
      ParamByName('ITF_VL_UNIT').AsCurrency := Registro.ValorUnitario;
      ParamByName('ITF_CODIGO').AsInteger := Registro.Codigo;
      ExecSQL;
    end;
  finally
...
begin
  ...
  Ibscbs := TControllerIbscbs.create(Self);
end;
...
begin
  ...
  FreeAndNil( ItensAvulso );
  FreeAndNil( ItensRTR );
  FreeAndNil( BaseTroca );
  FreeAndNil( Tributacao );
  FreeAndNil( ItensISSQN );
  FreeAndNil( Lista );
  FreeAndNil( ItensII );
  FreeAndNil( ItensCofins );
...
Begin
  ...
  Try
    ...
    Begin
      ...
      ParamByName('ITF_VL_CUSTO').AsCurrency :=  ValorCusto;
      ParamByName('ITF_VL_UNIT').AsCurrency :=  ValorUnitario;
      ...
      ParamByName('ITF_VL_DESC').AsCurrency :=  ValorDesconto;
      ParamByName('ITF_AQ_DESC').AsFloat      :=  AliqDesconto;
      ParamByName('ITF_AQ_IPI').AsCurrency :=  AliqIPI;
      ParamByName('ITF_OPER').AsString        :=  Operacao;
      ParamByName('ITF_AQ_ICMS').AsCurrency :=  AliqICMS;
...
begin
  ...
  Try
    ...
    Begin
      ...
      if FParametros.Estabelecimento > 0 then
      begin
        SQL.Text := SQL.Text + ' AND Tb_pedido.PED_CODMHA = :PED_CODMHA';
        ParamByName('PED_CODMHA').AsInteger := FParametros.Estabelecimento;
      end;
      ...
      Begin
        ...
        LITem.SubTotal := Lc_Qry.FieldByName('ITF_QTDE').AsFloat * Lc_Qry.FieldByName('ITF_VL_UNIT').AsCurrency;//Lc_Qry.FieldByName('ITF_VL_SUBTOTAL').AsCurrency;
