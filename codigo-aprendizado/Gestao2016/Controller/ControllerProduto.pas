...
interface
uses     STDatabase, Classes, STQuery, SysUtils, ControllerBase, Vcl.Forms, tblProduto, Un_MSg, ControllerMedida, Generics.Collections, controllerPreco, ObjMerchandise, ControllerEmbalagem, controllerEstoques, ControllerMarcaProduto, ControllerGRupos, ControllerCtrlEstoque, ControllerTabelaPreco, ControllerReposicao, tblStockBalance, tblPrice, ObjStockList, ObjPriceList, System.StrUtils;
Type
  ...
  public
    ...
    Reposicao : TControllerReposicao;
    Obj  : TObjMerchandise;
...
begin
  ...
  Try
    ...
    Begin
      ...
      ParamByName('PRO_VL_CUSTO').AsCurrency := REgistro.VAlorCusto;
      ParamByName('PRO_VL_CUSTOMED').AsCurrency := REgistro.ValorCustoMedio;
...
begin
  ...
  Try
    ...
    Begin
      ...
      ParamByName('PRO_VL_CUSTO_LAST').AsCurrency := REgistro.UltimoCusto;
      ParamByName('PRO_CODIGO').asfloat         := REgistro.Codigo;
      execSql;
    End;
  Finally
...
begin
  ...
  Reposicao := TControllerReposicao.Create(Self);
end;
...
begin
  ...
  Try
    ...
    End;
  Finally
    FreeAndNil(LcOriMarca);
    FreeAndNil(LcOriMedida);
    FreeAndNil(LcOriEmbalagem);
  End;
end;
...
begin
  ...
  Try
    if Registro.Codigo = 0 then
      Registro.Codigo := Generator('GN_PRODUTO'); //getNextByField(Registro,'PRO_CODIGO',0);
...
begin
  ...
  Begin
    ...
    Detalhes              :=  DObj.Produto.Observacao;
  ...
  Try
    ...
    Begin
      ...
      if Registro.CodigoEstabelecimento >0  then
        SQL.Add('  AND (PRO_CODMHA =:PRO_CODMHA) ');
      ...
      if Registro.CodigoEstabelecimento >0  then
        ParamByName('PRO_CODMHA').AsInteger := Registro.CodigoEstabelecimento;
...
begin
  ...
  Try
    ...
    Begin
      ...
      ParamByName('pro_codigo').AsInteger := Registro.Codigo;
      Active := True;
      Result := FieldByName('PRO_VL_CUSTO').AsCurrency;
    End;
  Finally
    FinalizaQuery(Lc_Qry);
  End;
...
begin
  ...
  Try
    ...
    Begin
      SQL.Add(concat(
                'select CAST((COALESCE(PRO_VL_BASE_TROCA, 0) * COALESCE(PRO_PESO, 0)) AS NUMERIC(18, 2)) PRO_VL_BASE_TROCA ',
                'from tb_produto ',
                'where (pro_codigo=:pro_codigo )'
      ));
      ParamByName('pro_codigo').AsInteger := Registro.Codigo;
      Active := True;
      Result := FieldByName('PRO_VL_BASE_TROCA').AsCurrency;
    End;
  Finally
    FinalizaQuery(Lc_Qry);
  End;
