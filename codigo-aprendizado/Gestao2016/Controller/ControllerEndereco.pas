...
interface
uses     STDatabase, Classes, STQuery, SysUtils, ControllerBase, tblEndereco, Generics.Collections, ControllerUf, ControllerCidade, ControllerPais;
...
Type
  ...
  public
    ...
    function getPrincipal:TEndereco;
...
uses     Un_Regra_Negocio, UnFunctions, Un_Funcoes;
...
begin
  Lc_Qry := GeraQuery;
  TRy
    //Guarda a informção da empresa caso o resultado seja zero
...
procedure TControllerEndereco.getByEmpresaCep;
Var
  Lc_Qry : TSTQuery;
begin
  Lc_Qry := GeraQuery;
  TRy
    ...
    Begin
      ...
      parambyname('END_CEP').asString := Registro.Cep;
      parambyname('END_CODEMP').asInteger := Registro.CodigoEmpresa;
      active := True;
      fetchall;
      exist := (RecordCount > 0);
...
function TControllerEndereco.getPrincipal: TEndereco;
Var
  Lc_Qry : TSTQuery;
Begin
  Lc_Qry := GeraQuery;
