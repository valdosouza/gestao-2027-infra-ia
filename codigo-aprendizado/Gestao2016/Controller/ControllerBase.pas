...
uses Classes, SysUtils, GenericORM, Windows, Gauges, Un_DM, STQuery, STDatabase,
      STTransaction, STScript;
...
begin
  LcTran := Script.Transaction;
  try
    if (LcTran <> nil) and LcTran.InTransaction then
      LcTran.Commit;
  finally
    Script.Transaction := nil;
    FreeAndNil(LcTran);
    FreeAndNil(Script);
  end;
end;
...
begin
  ...
  Lc_Tsc := TSTTransaction.Create(Result);
  if not FDataBase.Connected then
    FDataBase.Connected := True;
  Lc_Tsc.DefaultDatabase := FDataBase;
  Lc_Tsc.Params.Clear;
  Lc_Tsc.Params.Add('read_committed');
  Lc_Tsc.Params.Add('rec_version');
  Lc_Tsc.Params.Add('wait');
...
begin
  ...
  Try
    ...
    Begin
      ...
      sql.Add('SET GENERATOR ' + Generator + ' TO ' + Sequencia);
      ExecSQL;
    end;
  Finally
    FinalizaQuery(Lc_Qry);
...
function TControllerBase.GeraScript: TSTScript;
VAr
  Lc_Tsc : TSTTransaction;
begin
