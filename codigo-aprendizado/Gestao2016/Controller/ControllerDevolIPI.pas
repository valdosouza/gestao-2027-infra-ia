...
interface
uses     STDatabase, Classes, SysUtils, ControllerBase, Un_sistema, Un_Regra_Negocio, tblDevolIpi, Un_MSg, Datasnap.DBClient, STQuery;
...
procedure TControllerDevolIPI.getByItem;
Var
  Lc_Qry : TSTQuery;
begin
  Lc_Qry := GeraQuery;
  try
    ...
    Begin
      ...
      SQL.Add(concat(
                'select * ',
                'from tb_devol_ipi  ',
                'where TB_ORDER_ITEM_ID =:TB_ORDER_ITEM_ID '
      ));
...
begin
  ...
  try
    ...
    Begin
      ...
      sql.Clear;
      SQL.Add(concat(
                'select CAST(COALESCE(sum(di.v_ipi), 0) AS NUMERIC(18, 2)) total ',
