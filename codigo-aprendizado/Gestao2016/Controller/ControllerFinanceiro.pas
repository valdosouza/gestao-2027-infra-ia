...
uses     STDatabase, Classes, Vcl.Grids, STQuery, SysUtils, ControllerBase, Un_sistema, tblFinanceiro, ObjFinancialBills, Generics.Collections, ControllerMovimentoFinanceiro, System.AnsiStrings, ControllerGestaoWeb, ControllerFormaPagamento;
...
begin
  ...
  Try
    ...
    Begin
      ...
      FieldByName('FIN_VL_PARCELA').AsCurrency := Registro.ValorParcela;
...
procedure TControllerFinanceiro.getByCodigoNF;
Var
  Lc_Qry:TSTQuery;
Begin
  Lc_Qry := GeraQuery;
...
begin
  ...
  Try
    ...
    Begin
      ...
      if Registro.DocumentoBaixado <> '' then
      Begin
        sql.add(' AND FIN_BAIXA =:FIN_BAIXA ');
        ParamByName('FIN_BAIXA').AsString := Registro.DocumentoBaixado;
      End;
...
Begin
  ...
  Try
    ...
    Begin
      sql.Clear;
      sql.Add(concat(
              'select CAST(COALESCE(sum(F.FIN_VL_PARCELA), 0) AS NUMERIC(18, 2)) VALOR ',
