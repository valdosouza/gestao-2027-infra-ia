...
uses     STDatabase, Classes, STQuery, SysUtils, ControllerBase, tblCartao, Un_sistema, Un_Regra_Negocio, tblBoletoBancario, ACBrBoleto, Generics.Collections, ControllerContaBancaria, ControllerBoletoEletronico, ACBrBoletoFCFortesFr, ACBrBoletoConversao, ControllerFinanceiro, ControllerEspecieDoc, ControllerCarteiraCobranca, ControllerCliente, ControllerBanco;
...
uses     Un_DM, RN_BoletoEletronico, RN_Mailing, env, Un_Funcoes;
...
begin
  ...
  Begin
    ...
    Config.Registro.ContaCorrente := Registro.CodigoContaCorrente;
    Config.getByKey;
    //Cliente
...
begin
  ...
  FreeAndNil(EspecieDocumento);
  FreeAndNil(Financeiro);
  FreeAndNil(Boleto);
  FreeAndNil(BoletoFortes);
  FreeAndNil(Config);
  FreeAndNil(ContaCorrente);
  FreeAndNil(Lista);
  FreeAndNil(Registro);
  inherited;
end;
...
begin
  ...
  Try
    ...
    Begin
      ...
      FieldByName('FIN_VL_PARCELA').AsCurrency := Registro.Valor;
...
begin
  ...
  Begin
    ...
    CASE FNumeroBanco of
      ...
      Cedente.Modalidade := config.Registro.VariacaoCarteira.ToString;
  end;
