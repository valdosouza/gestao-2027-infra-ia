...
uses     STDatabase, Classes, Vcl.Grids, STQuery, SysUtils, ControllerBase, tblMdfe, Un_MSg, Generics.Collections, ControllerMdfeConfig, ControllerMdfeLoadLocal, ControllerMdfeStateRoute, ControllerMdfeRoad, ControllerMdfeTrailer, ControllerMdfeConductor, ControllerMdfeNfe, ControllerMdfeTotalizer, ControllerMdfeUnloadLocal, ControllerEmpresa, ACBrMDFe, AnsiStrings, pcnConversao, pmdfeConversaoMDFe, tblMdfeLoadLocal, tblMdfeUnloadLocal, tblMdfeConductor, ControllerMdfeRetorno, ControllerARquivo, Data.DB, ACBrMDFeDAMDFeClass, ACBrMDFeDAMDFeRLClass;
...
begin
  ...
  FOperador.Consultar;
  //Este campos s�o prenchidos aqui pois o Webservice retorna em objetos diferentes
...
begin
  ...
  FreeAndNil(Config);
  FreeAndNil(LoadLocal);
  FreeAndNil(StateRoute);
  FreeAndNil(UnloadLocal);
  FreeAndNil(Road);
  FreeAndNil(Trailler);
  FreeAndNil(Conductor);
  FreeAndNil(Nfe);
  FreeAndNil(Totalizer);
...
begin
  ...
  begin
    //
    // Dados de Identificção do MDF-e
    //
    ...
    Ide.cMDF    := Registro.codigo;  // Código Aleat�rio
    ...
    begin
      ...
      case AnsiIndexStr(Road.Veiculo.Registro.TipoRodado, ['Truck','Toco','Cavalo Mec�nico','VAN','Utilit�rio','Outros']) of
      ...
      case AnsiIndexStr(Road.Veiculo.Registro.Carroceria, ['Não Aplic�vel','Aberta','Fechada/Ba�','Granelera','Porta-Cont�iner','Sider']) of
    ...
    //MDFe.UnloadLocal.getlist;  - Carregou no getallByKey
    //Esta seção utiliza o load local para vincular a NFE � Cidade
    for I := 0 to unloadLocal.Lista.Count -1 do
    Begin
      LcunloadLocal := TMdfeunLoadLocal.Create;
      LcunloadLocal := unLoadLocal.Lista[I];
...
begin
  ...
  //da suporte no preenchimento da tela mas no xml � feito de outra maneira
...
begin
  ...
  //Este campos serão prenchidos conforme a operção executada anteriormente
    //Retorno.Registro.protocolo        := FOperador.WebServices.Consulta.Protocolo;
    //Retorno.Registro.Situacao         := FOperador.WebServices. Retorno.cStat;
    //Retorno.Registro.Motivo           := FOperador.WebServices.Retorno.xMsg;
