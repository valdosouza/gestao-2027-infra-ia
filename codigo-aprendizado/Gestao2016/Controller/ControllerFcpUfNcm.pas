...
uses     System.Classes, System.SysUtils, Generics.Collections, STQuery, ControllerBase, tblFcpufNcm, prm_fcp_uf_ncm;
...
begin
  ...
  Try
    ...
    Begin
      SQL.add(concat(
              'update TB_FCP_UF_NCM ',
              'set ',
              '  NCM = :NCM, ',
              '  ALIQUOTA = :ALIQUOTA ',
              'where ',
              '  TB_STATE_ID = :TB_STATE_ID AND ',
              '  NCM = :OLD_NCM '
      ));
      ParamByName('TB_STATE_ID').AsInteger := Registro.CodigoUF;
      ParamByName('NCM').AsString := Registro.NCM;
      ParamByName('ALIQUOTA').AsFloat := Registro.Aliquota;
      ParamByName('OLD_NCM').AsString := Registro.OLDNCM;
      ExecSQL;
    end;
  Finally
    FinalizaQuery(Lc_Qry);
