...
uses     STDatabase, Classes, STQuery, SysUtils, ControllerBase, tblColaborador, Un_MSg, Generics.Collections, ControllerUsuario, ControllerCargo, ObjSalesMan, tblphone, tblAddress, ControllerUF, prm_collaborator;
...
Type
  ...
  public
    ...
    function getFirstVendedor:Boolean;
    function Replace:boolean;
    procedure RemoverDemissao;
...
begin
  ...
  BEgin
    ...
    Colaborador.Sessao          := Colab.SecaoZona;
...
procedure TControllerColaborador.RemoverDemissao;
Var
  Lc_Qry : TSTQuery;
begin
  Lc_Qry := GeraQuery;
  Try
    with Lc_Qry do
    Begin
      sql.Add(concat('UPDATE TB_COLABORADOR SET ',
                     'CLB_DEMISSAO = null ',
...
function TControllerColaborador.VerificaExistenciaUsuario: Boolean;
Var
  Lc_Qry : TSTQuery;
begin
  Result := True;
  ...
  Try
    ...
    Begin
      sql.Add(concat('select * ',
                     'from TB_COLABORADOR ',
                     'where ( (CLB_CODUSU =:CLB_CODUSU) and (CLB_CODUSU > 0) ) ',
                     '  AND ( (CLB_CODIGO <>:CLB_CODIGO) and (CLB_CODIGO >0 )) '
                     ));
      ParamByName('CLB_CODUSU').AsInteger := FParametros.FieldName.Usuario;
      ParamByName('CLB_CODIGO').AsInteger := FParametros.FieldName.Codigo;
      Active := True;
      FetchAll;
      exist := (recordCount > 0);
      if exist then
...
function TControllerColaborador.getFirstVendedor: Boolean;
Var
  Lc_Qry : TSTQuery;
begin
  Result := True;
