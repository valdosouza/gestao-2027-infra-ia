...
uses     STDatabase, System.Classes, STQuery, System.SysUtils, ControllerBase, tblCtrlEstoque, Un_MSg, ControllerCtrlEstoqueZerado, System.Generics.Collections, ControllerDskSyncTable, ControllerComposicao, tblDskSyncTable;
...
Type
  ...
  private
    ...
    procedure SendEmail;
    procedure ReposicaoAutomatica(pQtdeMinima:Real);
...
uses     UN_Sistema, Un_Regra_Negocio, Un_tr_send_email_estoque_negativo, un_tr_estoque_auto_reposicao, env;
...
begin
  ...
  Try
    ...
    Begin
      ...
      Active := False;
      sql.clear;
      sql.Add(concat('SET GENERATOR GN_CTRL_ESTOQUE TO ' , IntToStr(lC_Sequencia)));
      ExecSQL;
    End;
  Finally
...
begin
  ...
  Begin
    ...
    Self.update;
    //Aqui define zero no codigo para que seja inserida a invers„o do lanÁamento
...
begin
  FreeAndNil(EstoqueZerado);
  FreeAndNil(ListaProdutoSimples);
  FreeAndNil(ListaProdutoMovimentado);
  ...
  Try
    ...
    Begin
      sql.Add(concat(
                ...
                //--SELECT DOS DOCUMENTOS FATURADOS E COM COMPOSI«√O
                'UNION ',
                ...
                '  AND ((NFL_STATUS <> ''C'') or (NFL_STATUS IS NULL)) ',
                '  and ( ITF_CODEST =:ETS_CODIGO ) ',
                //--SELECT DOS DOCUMENTOS N„O FATURADOS E SEM COMPOSI«√O ',
                'union ',
                ...
                '  and ( pro_composicao <>2 ) ',
                '  and ( ITF_CODEST =:ETS_CODIGO ) ',
                //--sELECT DOS DOCUMENTOS N„O FATURADOS E COM COMPOSI«√O ',
                'UNION ',
                'SELECT DISTINCT PPD.ppd_codpro_d PRO_CODIGO, ITF_CODEST EST_CODETS, PRO_COMPOSICAO ',
                '  FROM TB_ITENS_NFL ',
                '    INNER JOIN tb_pedido ',
                ...
                '  AND ( tb_produto.pro_composicao = 2 ) ',
                '  and ( ITF_CODEST =:ETS_CODIGO ) ',
                //--SELECT DOS DOCUMENTOS DE PRODU«√O MATERIA PRIMA ',
                'union ',
                ...
                //--SELECT DOS DOCUMENTOS DE PRODU«√O PRODUTO ACABADO ',
                'UNION ',
                ...
                'WHERE (orp.ORP_DTCAD between :DATA_INI AND :DATA_FIM ) ',
                ' AND ( orp.ORP_ESTMP =:ETS_CODIGO ) ',
                //--SELECT DOS DOCUMENTOS DE ORDEM DE SERVI«O - EXINTORES ',
                'UNION ',
                ...
                //Est„o zerados no inventario
                //Tem saldo de estoque devido algum erro
                //e n„o tem movimento algum que o faÁa participar desta lista.
                //uma opÁ„o seria zerar todo o saldo dos produtos desta forma quem tem saldo pelo inventario e quem tem movimento seria recontado
                'UNION ',
      ...
      sql.Add('WHERE pd.pro_codigo is not null ');
...
Var
  LcSyncTable : TDskSyncTAble;
  LcSyT : TControllerDskSyncTable;
  Lc_Qry:TSTQuery;
  LcReg :  TCtrlEstoque;
begin
  Lc_Qry := GeraQuery;
  TRY
    LcSyT := TControllerDskSyncTable.Create(nil);
...
Begin
  ...
  Try
    with Lc_Qry do
    Begin
      ///SELECT DOS DOCUMENTOS FATURADOS E SEM COMPOSI«√O
      SQL.Clear;
      Lc_SqlTxt := concat(
                   'SELECT  0 CET_CODIGO, ',
                   '   1 CET_TERMINAL, ',
      ...
      ///SELECT DOS DOCUMENTOS N„O FATURADOS E COM COMPOSI«√O
                    ...
                    ' AND ( ITF_CODPRO =:PRO_CODIGO ) ',
                    ' and ( ITF_CODEST =:ETS_CODIGO ) ',
      //SELECT DOS DOCUMENTOS DE PRODU«√O MATERIA PRIMA - FINALIZADO
                   ' union ',
                   'SELECT 0 CET_CODIGO, ',
                   '       1 CET_TERMINAL, ',
                   '       ''M'' CET_VINCULO, ',
                   ...
                   '       ORP_QTDREAL * PRE_QTDUNI  CET_QTDE, ',
                   '       orp.ORP_DTCAD CET_DATA, ',
                   '       current_timestamp UPDATE_AT, ',
                   '       ''PRODU«√O MP'' as  CET_TIPO ',
                   'FROM tb_orp orp ',
                   '  INNER JOIN tb_orpprev orpma ',
                   '  ON (orpma.PRE_CODORP = orp.ORP_CODIGO) ',
                   'WHERE ORP_INDSIT = ''F'' ',
                   ' AND (orp.ORP_DTCAD between :DATA_INI AND :DATA_FIM ) ',
                   ' AND ( orpma.PRE_CODPRO =:PRO_CODIGO ) ',
                   ' AND ( orp.ORP_ESTMP =:ETS_CODIGO ) ',
      //SELECT DOS DOCUMENTOS DE PRODU«√O PRODUTO ACABADO - FINALIZADO
                   'UNION ',
                   'SELECT 0 CET_CODIGO, ',
                   '       1 CET_TERMINAL, ',
                   '       ''O'' CET_VINCULO, ',
...
Begin
  ...
  Try
    ...
    Begin
      ...
      FetchAll;
      First;
      if ( (FieldByname('PRO_COMPOSICAO').AsString = '1') or
           (FieldByname('PRO_COMPOSICAO').AsString = '3') or
           (FieldByname('PRO_COMPOSICAO').AsString = '6') ) then
      Begin
        Self.insert;
      end;
  ...
  Try
    ...
    Begin
      ...
      SQL.Add(concat(
                      ...
                      '     OR ( upper(CET_TIPO) = ''PRODU«√O MP'' ) ',
                      '     OR ( upper(CET_TIPO) = ''PRODU«√O PA'' ) ',
                      ...
                      '     OR ( upper(CET_TIPO) = ''LOTE CANCELADA'' ) ',
                      '     ) '
                  ));
      ParamByName('CET_CODETS').AsInteger := FEstoque;
      ParamByName('CET_CODPRO').AsInteger := FCodigoProduto;
...
begin
  ...
  Begin
    ClonarObj(Lista[I],Registro);
    Recontagem; //Estou fazendo externamento por que j· tenho a informÁ„o de composiÁ„o - 16/02/2022
  End;
end;
...
Begin
  ...
  Try
    ...
    BEgin
      ...
      if ( FieldByname('PRO_COMPOSICAO').AsString = '6')  then
      Begin
        //Registra a operÁ„o do produto
        Save;
        //Executa a thread para repor o produto se precisar
        ReposicaoAutomatica(FieldByname('PRO_QTDE_MIN_AUTO_REPO').AsFloat);
      End;
...
procedure TControllerCtrlEstoque.ReposicaoAutomatica(pQtdeMinima:Real);
var
  LcRepo : TTrEstoqueAutoReposicao;
  LcSaldo : Real;
begin
  if Registro.operacao = 'S' then
  Begin
    LcSaldo := getSaldo;
    if (LcSaldo <= pQtdeMinima ) then
    begin
      LcRepo := TTrEstoqueAutoReposicao.Create;
      ClonarObj(Registro,LcRepo.Registro);
      LcRepo.Start;
    end;
  End;
end;
...
var
  LcEmail : TTRSendEmailEstoqueNegativo;
  LcSaldo : Real;
begin
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        LcEmail := TTRSendEmailEstoqueNegativo.Create;
        try
          LcEmail.CodigoProduto     := EstoqueZerado.Registro.Produto;
          LcEmail.CodigoPedido      := Registro.Ordem;
          ...
          // Antes do Start o FreeOnTerminate ainda nao protege; libera manualmente.
          LcEmail.Free;
          raise;
        end;
      End;
