...
uses     STDatabase, Classes, Vcl.Grids, STQuery, SysUtils, ControllerBase, tblPrePaidCard, Un_MSg, Generics.Collections, prm_dsk_base, prm_pre_paid_card;
...
begin
  ...
  Try
    ...
    Begin
      sql.add(concat(
                  'SELECT CAST(COALESCE(SUM(credit_value - debit_value), 0) AS NUMERIC(18, 2)) SALDO ',
                  'FROM TB_PRE_PAID_CARD  ',
                  'WHERE ( TB_INSTITUTION_ID =:TB_INSTITUTION_ID ) '
      ));
