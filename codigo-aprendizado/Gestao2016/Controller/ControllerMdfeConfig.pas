...
uses     STDatabase, Classes, Vcl.Grids, STQuery, SysUtils, ControllerBase, tblMdfeConfig, Un_MSg, Generics.Collections, Registry, Winapi.Windows, Vcl.StdCtrls, blcksock, TypInfo, pcnConversaoNFe, ACBrMDFe, pcnConversao, ACBrDFeSSL, pmdfeConversaoMDFe, Vcl.ExtCtrls, Vcl.Graphics, System.StrUtils;
...
uses     UN_Sistema, Un_DM, env;

procedure TControllerMdfeConfig.Pc_SSL2_TLS1;
var
  Lc_Registro: TRegistry;
begin
  //TESTANDO O EMAIL VAMOS DESABILITAR A CONFIGUIRÇÃO
  EXIT;

  try
    //acertando opções da internet (revogados / SSL / TSL)
    //verificar revogção de certificados do servidor
...
//        Lc_Registro.WriteInteger('SecureProtocols', 160);
    ...
    //for�a para IE não ficar trabalhando off line
    if Lc_registro.OpenKey('Software\Microsoft\Windows\CurrentVersion\Internet Settings', true) then
    begin
...
begin
  ...
  BEgin
    ...
    case AnsiIndexStr(UpperCase(Registro.Camada), ['P', 'H']) of
      0: Configuracoes.WebServices.Ambiente := taProducao;
      1: Configuracoes.WebServices.Ambiente := taHomologacao;
    end;
...
begin
  ...
  Begin
    Result :=
          concat('Verifique a instalção dos drives do dispositivo.',#13,
                 'Utilize o bot�o "Carregar o certificado".'
          );
  End;
end;
