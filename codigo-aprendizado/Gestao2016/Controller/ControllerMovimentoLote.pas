...
uses     STDatabase, Classes, STQuery, SysUtils, ControllerBase, tblMovimentoLote, Generics.Collections, controllerProduto;
...
begin
  ...
  Try
    ...
    Begin
      ...
      while not eof do
      Begin
        if (FieldByName('MLT_SENTIDO').AsString = 'E') then
      ...
      ParamByName('CLT_QT_SALDO').AsCurrency := Lc_Entrada - Lc_Saida;
      ParamByName('CLT_CODIGO').AsInteger   := Registro.Lote;
      ExecSQL;
    End;
  Finally
...
function TControllerMovimentoLote.ComMovimento: Boolean;
Var
  Lc_Qry : TSTQuery;
begin
  Lc_Qry := GeraQuery;
  Try
    ...
    Begin
      ...
      ParamByName('MLT_SENTIDO').AsString := 'S';
...
begin
  ...
  Try
    ...
    Begin
      sql.Add(concat(
                'SELECT CAST(COALESCE(SUM(ml.mlt_qtde), 0) AS DOUBLE PRECISION) total ',
