...
uses     System.Classes, System.SysUtils, Generics.Collections, STQuery, ControllerBase, tblUsuario, prm_users, MD5;
...
begin
  Result := True;
  Lc_Qry := GeraQuery;
  Registro.Senha := '827CCB0EEA8A706C4C34A16891F84E7B';//senha12345
  Try
    with Lc_Qry do
    Begin
      sql.add(concat(
      ...
      ParamByName('USU_SENHA').AsString := Registro.Senha;
      ExecSQL;
    end;
  Finally
    FinalizaQuery(Lc_Qry);
...
begin
  ...
  Try
    with Lc_Qry do
    Begin
      SQL.Text := ' SELECT * FROM TB_USUARIO where USU_ATIVO =:USU_ATIVO ';

      //esse parametro desta maneira ser para evitar a obrigatoriedade de preencher antes do Search
      if FParametros.FieldName.Ativo = 'N' then
        ParamByName('USU_ATIVO').AsString := 'N'
      else
        ParamByName('USU_ATIVO').AsString := 'S';
