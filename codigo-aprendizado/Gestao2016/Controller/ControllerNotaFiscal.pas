...
interface
uses      Data.DB, STDatabase, Classes, Vcl.Grids, STQuery, SysUtils, ControllerBase, Un_sistema, tblNotaFiscal, Un_MSg, Generics.Collections, StrUtils, objInvoiceMerchandise, ControllerPedido, ControllerObsNota, ControllerDskCfop, tblRetornoNfe, tblOrderItemIcms, tblOrderItemIpi, tblOrderItemPis, tblOrderItemCofins, tblOrderItemII, tblOrderItemISSqn, tblInvoiceObs, ControllerFinanceiro, ControllerRetornoNFe, ControllerEmpresa, ControllerGestaoWeb;
Type
  ...
  private
    ...
    procedure FillDataObjetoNotaObservacao(PcRegistro: TNotaFiscal);
...
procedure TControllerNotaFiscal.ApagarImpostos;
begin
  with Pedido.Itens do
  Begin
    ItensIcms.registro.Nota := registro.Codigo;
    ItensIcms.deletebyNF;

    ItensIpi.Registro.Nota := registro.Codigo;
    ItensIpi.deletebyNF;

    ItensPis.Registro.Nota := registro.Codigo;
    ItensPis.deletebyNF;

    ItensCofins.Registro.Nota := registro.Codigo;
    ItensCofins.DeleteByNF;

    ItensII.Registro.Nota := registro.Codigo;
    ItensII.deleteByNF;

    ItensISSQN.Registro.Nota := registro.Codigo;
    ItensISSQN.deleteByNF;

    Ibscbs.Registro.Nota := registro.Codigo;
    Ibscbs.deleteAllByNF;
  End;
end;
...
begin
  ...
  FreeAndNil(REtorno55);
  FreeAndNil(CFOP);
  FreeAndNil(Observacoes);
  FreeAndNil(Pedido);
  FreeAndNil(Empresa);
  FreeAndNil(Lista);
  FreeAndNil(Registro);
...
Begin
  ...
  Try
    ...
    Begin
      SQL.Clear;
      SQL.Add(concat(
                'SELECT CAST(COALESCE(SUM(ii.itf_vl_desc), 0) AS NUMERIC(18, 2)) VALORTOTAL ',
...
Begin
  ...
  Try
    ...
    Begin
      SQL.Clear;
      SQL.Add(concat(
                'SELECT CAST(COALESCE(SUM(pi.pis_vl_nr), 0) AS NUMERIC(18, 2)) VALORTOTAL ',
                'FROM tb_nota_fiscal NF ',
...
Begin
  ...
  Try
    ...
    Begin
      SQL.Clear;
      SQL.Add(concat(
                'SELECT CAST(COALESCE(SUM(CO.cfs_vl_nr), 0) AS NUMERIC(18, 2)) VALORTOTAL ',
                'FROM tb_nota_fiscal NF ',
...
Begin
  ...
  Try
    ...
    Begin
      SQL.Clear;
      SQL.Add(concat(
                'SELECT CAST(COALESCE(SUM(ii.iip_vl_ii), 0) AS NUMERIC(18, 2)) VALORTOTAL ',
                'FROM tb_nota_fiscal NF ',
...
begin
  ...
  //Observções
  FillDataObjetoNotaObservacao(PcRegistro);
...
procedure TControllerNotaFiscal.FillDataObjetoNotaObservacao(
  PcRegistro: TNotaFiscal);
Var
  LcObs : TInvoiceObs;
  I : Integer;
...
begin
  ...
  Try
    ...
    Begin
      ...
      Active := True;
      FetchAll;
      Result := FieldByName('nfl_vl_tl_nota').AsCurrency;
    end;
  Finally
    FinalizaQuery(Lc_Qry);
  End;
