...
uses     System.Classes, System.SysUtils, Generics.Collections, STQuery, ControllerBase, tblSalesOrigin, prm_Sales_Origin;
...
begin
  ...
  if Registro.ID = 0 then
    Registro.ID := Generator('GN_SALE_ORIGEN');
  SaveObj(Registro);
end;
...
begin
  ...
  Try
    ...
    Begin
      ...
      if FParametros.FieldName.id > 0 then
      begin
        SQL.Text := SQL.Text + ' AND ID = :ID';
        ParamByName('ID').AsInteger := FParametros.FieldName.id;
      end;

      if FParametros.FieldName.Description <> EmptyStr then
      begin
