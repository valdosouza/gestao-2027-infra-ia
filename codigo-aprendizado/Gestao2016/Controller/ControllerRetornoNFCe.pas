...
interface
uses     STDatabase, Classes, Vcl.Grids, STQuery, SysUtils, ControllerBase, tblRetornoNfCe, Generics.Collections, ObjInvoiceReturn65, ControllerGestaoWeb, UnFunctions;
...
uses     Un_Regra_Negocio, UN_Sistema, Un_Funcoes;
    ...
    Begin
      ...
      Begin
        ...
        Begin
          sql.Clear;
          sql.add(concat(
                  'SELECT MAX(R.NFC_CODIGO) NUMERO ',
                  'FROM TB_RETORNO_NFC R ',
                  'WHERE R.nfc_codmha=:nfc_codmha ',
                  'AND R.nfc_serie=:nfc_serie '
          ));
          ParamByName('nfc_codmha').AsInteger := Registro.Estabelecimento;
          ParamByName('nfc_serie').AsInteger := Registro.Serie;
          Active := True;
          FetchAll;
        End;
        if (Lc_Qry.Active) and  (RecordCount > 0) then
...
function TControllerRetornoNFCe.multiEmpresa: Boolean;
var
  Lc_Qry : TSTQuery;
begin
  Try
