...
uses     Un_DM, UN_Sistema, UN_MSG, un_Padrao, ControllerBase, Un_Regra_Negocio, UN_NCM, Un_Pesq_NCM, UN_TabelasEmListas, RN_Crud, env;
Function  Fc_BuscaDescricaoProduto(Fc_cd_Produto:String):String;
Var
  Lc_SqlTxt : String;
  Lc_Qry : TSTQuery;
    ...
    begin
      ...
      begin
        ...
        if Lc_Qry_Tabelas.FieldByname('TPR_CODIGO').AsInteger = 2 then
          Lc_Qry_Insere.ParamByName('PRC_VL_VDA').AsCurrency := PC_Vl_Promocao
        else
          Lc_Qry_Insere.ParamByName('PRC_VL_VDA').AsCurrency := PC_Vl_Normal;
  ...
  Try
    ...
    Begin
      Lc_Qry.SQL.Add(concat(
                      'SELECT PRO_CODIGO, PRO_DESCRICAO FROM TB_PRODUTO ',
                      'WHERE PRO_CODIGO=:PRO_CODIGO'));


      if (Trim(Fc_Tipo) <> '') then
        Lc_Qry.SQL.Add(' and (PRO_TIPO = ''' + Fc_Tipo + ''') ') ;
      ...
      begin
        ...
        MensagemPadrao(MENSAGEM,ATENCAO+EOLN+EOLN+
                       Lc_Tp_Descricao + ' não encontrado ou não.'+EOLN+
