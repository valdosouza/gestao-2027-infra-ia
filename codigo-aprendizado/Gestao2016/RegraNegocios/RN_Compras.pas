...
uses
  ...
  Procedure Pc_EstornaCustoCompra(Pc_Tipo:Integer;Pc_Dataset:TSTQuery);
  Procedure Pc_DeleteREgistroCustos(Pc_Tipo:Integer;PedidoID:Integer);
implementation

uses     Un_DM, UN_Sistema, UN_MSG, un_Padrao, Un_Regra_Negocio, ControllerCustoPedido, Un_Fc_Sored_Procedures, UN_Principal, env;
Function Fc_UltimoValorUnitarioCompra(Fc_Cd_Produto:Integer):Real;
var
  Lc_Qry: TSTQuery;
...
begin
  Try
    ...
    Begin
      ...
      Active := True;
      FetchAll;
      Result := StrToFloatDef(FieldByname('ITF_VL_UNIT').AsString,0);
    end;
  Finally
  ...
  Begin
    try
      ...
      Begin
        ...
        Active := True;
        FetchAll;
        Lc_Vl_Compra := FieldByname('TOTAL').AsCurrency;
...
Begin
  ...
  Try
    ...
    begin
      ...
      Lc_Vl_Unitario := Lc_Qr_Custo.FieldByName('ITF_VL_UNIT').AsCurrency;

      //Custo Direto ( + )
      Lc_VL_Custo := Lc_VL_Custo + Lc_Qr_Custo.FieldByName('ITF_VL_CUSTO').AsCurrency;

      //Desconto Direto ( - )
      Lc_Vl_Calculo := 0;
      if (Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat > 0) then
        Lc_VL_Custo := Lc_VL_Custo - (Lc_Qr_Custo.FieldByName('ITF_VL_DESC').AsCurrency / Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat);
      ...
      Lc_Vl_Calculo := 0;
      if (Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat > 0) then
        Lc_Vl_Calculo := Lc_Qr_Custo.FieldByName('ICM_VL_ST').AsCurrency / Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat;
      ...
      Lc_Vl_Calculo := 0;
      if (Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat > 0) then
        Lc_Vl_Calculo := Lc_Qr_Custo.FieldByName('ICM_VFCPST').AsFloat / Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat;
      Lc_VL_Custo   := Lc_VL_Custo + Lc_Vl_Calculo;
      ...
      Lc_Vl_Calculo := 0;
      if (Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat>0) then
        Lc_Vl_Calculo := Lc_Qr_Custo.FieldByName('ICM_VL_FRETE').AsCurrency / Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat;
      Lc_VL_Custo   := Lc_VL_Custo + Lc_Vl_Calculo;
      ...
      Lc_Vl_Calculo := 0;
      if (Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat>0) then
        Lc_Vl_Calculo := Lc_Qr_Custo.FieldByName('ICM_VL_SEGURO').AsCurrency / Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat;
      Lc_VL_Custo   := Lc_VL_Custo + Lc_Vl_Calculo;
      ...
      Lc_Vl_Calculo := 0;
      if (Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat>0) then
        Lc_Vl_Calculo := Lc_Qr_Custo.FieldByName('ICM_VL_OUTRAS').AsCurrency / Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat;
      Lc_VL_Custo   := Lc_VL_Custo + Lc_Vl_Calculo;
      ...
      Lc_Vl_Calculo := 0;
      if (Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat >0) then
        Lc_Vl_Calculo := Lc_Qr_Custo.FieldByName('IPI_VL_BC').AsCurrency * Lc_Qr_Custo.FieldByName('IPI_AQ_NR').AsCurrency;
      ...
      if ( DM.Qr_Estabelecimento.FieldByName('EMP_CRT').AsInteger > 1 ) and
         ( Lc_Qr_Custo.FieldByName('ICM_VL_NR').AsCurrency > 0 ) then
      Begin
        //ICMS ( - )
        Lc_Vl_Calculo := 0;
        if (Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat > 0) then
          Lc_Vl_Calculo := Lc_Qr_Custo.FieldByName('ICM_VL_NR').AsCurrency / Lc_Qr_Custo.FieldByName('ITF_QTDE').AsFloat;
        Lc_VL_Custo := Lc_VL_Custo - Lc_Vl_Calculo;
        ...
        //PIS ( - )
        Lc_Vl_Calculo := 0;
        Lc_Vl_Calculo := ( Lc_Qr_Custo.FieldByName('ITF_VL_CUSTO').AsCurrency * DM.Qr_Impostos.FieldByName('IPT_PIS').AsCurrency ) / 100;
        Lc_VL_Custo := Lc_VL_Custo - Lc_Vl_Calculo;
        ...
        //COFINS ( - )
        Lc_Vl_Calculo := 0;
        Lc_Vl_Calculo := ( Lc_Qr_Custo.FieldByName('ITF_VL_CUSTO').AsCurrency * DM.Qr_Impostos.FieldByName('IPT_COFINS').AsCurrency ) / 100;
        Lc_VL_Custo := Lc_VL_Custo - Lc_Vl_Calculo;
      ...
      Begin
        ...
        begin
          ...
          Lc_Vl_Calculo := Lc_Vl_Calculo + (Lc_Qr_Custo.FieldByName('PRO_VL_CUSTOMED').AsCurrency * Lc_Qt_Anterior);
...
Begin
  try
    ...
    Begin
      ...
      Begin
        if (Lc_Qry.FieldByname('PRO_ATIVO').AsString = 'N') Then
        Begin
          Active := False;
