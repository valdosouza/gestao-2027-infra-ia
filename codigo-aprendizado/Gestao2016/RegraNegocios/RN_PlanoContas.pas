...
uses
      Forms, Controls, SysUtils, STQuery, STStoredProc, STDatabase, StdCtrls, classes, db, ControllerBase;
...
uses     Un_DM, UN_Sistema, UN_MSG, un_Padrao, un_principal, Un_Regra_Negocio, Un_Lanca_PlanoContas, env;
function Fc_ValorContaRealizada(Fc_Tipo,Fc_Plano,Fc_Campo:String;Fc_DataIni,Fc_DataFim:TDate):Real;
Var
  Lc_Qry : TSTQuery;
...
begin
  Try
    ...
    Begin
      ...
      ParamByName('PLC_CODPLANO').AsString := Fc_Plano +'%';
      ...
      Begin
        if FieldByName('PLC_VL_FUTURO').AsCurrency >= 0 then
          result := FieldByName('PLC_VALOR').AsCurrency + FieldByName('PLC_VL_FUTURO').AsCurrency
      End
      else
      Begin
        if FieldByName('PLC_VL_FUTURO').AsCurrency <= 0 then
          result := FieldByName('PLC_VALOR').AsCurrency + (FieldByName('PLC_VL_FUTURO').AsCurrency * -1);
...
begin
  Try
    ...
    Begin
      ...
      ParamByName('DATAFIM').AsDate := Fc_DataFim;
      Active := True;
      Result := FieldByName('PLC_VALOR').AsCurrency;
    End;
  Finally
    LcBase.FinalizaQuery(Lc_Qry);
    FreeAndNil(LcBase);
...
begin
  Try
    ...
    Begin
      ...
      ParamByName('PLANO').AsString := Fc_Plano;
      Active := True;
    end;
  Finally
    FreeAndNil(LcBase);
