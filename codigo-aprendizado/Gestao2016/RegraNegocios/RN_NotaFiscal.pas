...
uses
      STDatabase, STQuery, SysUtils, Classes, Vcl.Controls, System.Math, Vcl.Graphics, ControllerBase, STTransaction;
...
uses     Un_DM, UN_Sistema, UN_MSG, un_Padrao, Un_Fc_Sored_Procedures, Un_Nota_Fiscal, Un_NF_65, Un_Pesq_NotaFiscal, RN_Pedido;
procedure Pc_AbreTelaNotaFiscal(PC_Cd_Nota:Integer;Parent:TwinControl);
Var
...
begin
  Try
    ...
    Begin
      ...
      Begin
        Result := concat(
                   'PLACA  : ' + Copy(FieldByName('VEI_PLACA').AsString,1,3) + '-' + Copy(FieldByName('VEI_PLACA').AsString,4,4),' | ',
                   'MARCA  : ' + FieldByName('MRC_DESCRICAO').AsString ,' | ',
                   'MODELO : ' + FieldByName('MOD_DESCRICAO').AsString
                   );
      End;
    End;
  Finally
  ...
  Try
    ...
    Begin
      ...
      Begin
        ...
        Lc_Vl_Base := ( (FieldByName('ITF_VL_UNIT').AsCurrency * FieldByName('ITF_QTDE').AsFloat) - FieldByName('ITF_VL_DESC').AsCurrency);
