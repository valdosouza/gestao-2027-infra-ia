...
uses
      STDatabase, STQuery, SysUtils, RN_Permissao, ControllerItensNfl, STTransaction, Classes, System.Math, ControllerCtrlEstoque, System.StrUtils, ControllerBase;
  ...
  procedure Pc_CopiaItensPedido(Pc_Cd_Pedido: Integer;
                                ...
                                Fc_AlteraVAlor:String;
                                Fc_Tx_Altera:Real);
  procedure Pc_EnviaItensPedido(FcDataBase : TSTDatabase;
                                FcTransaction : TSTTransaction;
...
uses     Un_DM, UN_Sistema, UN_MSG, un_Padrao, Un_Principal, env, Un_Regra_Negocio, Un_CopiaPedidoNota, Un_Fc_Sored_Procedures, RN_Estoque, Un_EnviaPedidoNota, RN_NotaFiscalEletronica3X, Un_Funcoes, RN_Crud, RN_Empresa, ControllerPedido, ControllerEmpresa, ControllerEndereco;
procedure Pc_AtualizaTotalPedido(Pc_CODIGO:Integer;
                                  Pc_QT_PRODUTO,
                                  Pc_VL_PRODUTO,
                                  Pc_VL_SERVICO,
...
begin
  Try
    ...
    Begin
      ...
      ParamByName('PED_VL_PRODUTO').AsCurrency := Pc_VL_PRODUTO;
      ParamByName('PED_VL_SERVICO').AsCurrency := Pc_VL_SERVICO;
      ParamByName('PED_VL_IPI').AsCurrency := Pc_VL_IPI;
      ParamByName('PED_VL_FRETE').AsCurrency := Pc_VL_FRETE;
      ParamByName('PED_ALIQ_DESCONTO').AsCurrency := Pc_ALIQ_DESCONTO;
      ParamByName('PED_VL_DESCONTO').AsCurrency := Pc_VL_DESCONTO;
      ParamByName('PED_VL_PEDIDO').AsCurrency := Pc_VL_PEDIDO;
      ParamByName('PED_VL_ODESPESA').AsCurrency := Pc_VL_ODESPESA;
      ExecSQL;
    End;
  Finally
...
begin
  TRy
    ...
    Begin
      ...
      while not Eof do
      begin
        if (FieldByName('IPI_VL_BC').AsCurrency > 0) then
        Begin
          Lc_Vl_IPI := FieldByName('IPI_VL_BC').AsCurrency * (FieldByName('IPI_AQ_NR').AsCurrency /100);
        ...
        Begin
          ...
          Lc_VL_DESC := FieldByName('ITF_VL_DESC').AsCurrency + ( (FieldByName('ITF_VL_DESC').AsCurrency * Fc_Tx_Altera)/100);
          Lc_Vl_Unitario := FieldByName('ITF_VL_UNIT').AsCurrency + ( (FieldByName('ITF_VL_UNIT').AsCurrency * Fc_Tx_Altera)/100);
        ...
        Begin
          ...
          Begin
            ...
            Lc_VL_DESC := FieldByName('ITF_VL_DESC').AsCurrency;
            Lc_Vl_Unitario := FieldByName('ITF_VL_UNIT').AsCurrency;
        ...
        ValorCusto        := FieldByName('ITF_VL_CUSTO').AsCurrency;
        ...
        AliqIPI           := FieldByName('ITF_AQ_IPI').AsCurrency;
        Operacao          := Pc_TipoItem;
        AliqICMS          := fIeldByName('ITF_AQ_ICMS').AsCurrency;
  ...
  Try
    ...
    Begin
      ...
      Begin
        ...
        PedidoCopia.salva;
        if (fc_Tipo = 1) OR (fc_Tipo = 4) then
          Pc_EnviaItensPedido(FcDataBase, FcTransaction, Fc_Cd_Pedido, PedidoCopia.Registro.Codigo,'S',Lc_TipoItem,Fc_AlteraVAlor,Fc_Tx_Altera)
        else
          Pc_EnviaItensPedido(FcDataBase, FcTransaction, Fc_Cd_Pedido, PedidoCopia.Registro.Codigo,'N',Lc_TipoItem,Fc_AlteraVAlor,Fc_Tx_Altera);
...
begin
  Try
    ...
    with Lc_Qry do
    Begin
      Lc_SqlTxt := 'SELECT TB_ITENS_NFL.*,tb_produto.PRO_CODIGOFOR, '+
                   'tb_produto.PRO_CODIGOFAB, tb_produto.PRO_DESCRICAO, '+
                   ' IPI_VL_BC, IPI_AQ_NR '+
                   'FROM TB_ITENS_NFL '+
      ...
      while not Eof do
      begin
        if Fc_VerificaProdutoDestino(FcDataBase,FcTransaction,FieldByName('PRO_CODIGOFOR').AsString, FieldByName('PRO_DESCRICAO').AsString) then
        Begin
          if (FieldByName('IPI_VL_BC').AsCurrency > 0) then
          Begin
            Lc_Vl_IPI := FieldByName('IPI_VL_BC').AsCurrency * (FieldByName('IPI_AQ_NR').AsCurrency /100);
          ...
          Lc_Cd_Item := Fc_AtualizaItensNflBcoDestino(FcDataBase,
                                                      FcTransaction,
                                                      'I',
                                                      0,
                                                      Pc_Cd_PedNovo,
                                                      0,
                                                      FieldByName('PRO_CODIGOFOR').AsInteger,
                                                      FieldByName('ITF_QTDE').AsFloat,
                                                      FieldByName('ITF_VL_CUSTO').AsCurrency,
                                                      RoundTo(Lc_Vl_Unitario,-2),
                                                      RoundTo(Lc_AQ_DESC,-2),
                                                      RoundTo(Lc_VL_DESC,-2),
                                                      FieldByName('ITF_AQ_COM').AsFloat,
                                                      Pc_TipoItem,
                                                      'S',
                                                      'N',
                                                      FieldByName('ITF_AQ_IPI').AsCurrency,
                                                      FieldByName('ITF_AQ_ICMS').AsCurrency,
                                                      1,
                                                      1,
                                                      '',
                                                      0);
          ...
          //Atualiza Estoque
          if Pc_AtualizaEstoque = 'S' then
          Begin
            Pc_Registra_EstoqueBcoDestino(FcDataBase,
                                          'P',
                                          Pc_Cd_PedNovo,
                                          Lc_Cd_Item,
                                          FieldByName('ITF_CODEST').AsInteger,
                                          'S',//Somente as vendas são imediatamente retiradas  do estoque imeFieldByName('ITF_OPER').AsString,
        ...
        Begin
          MensagemPadrao(MENSAGEM, ATENCAO + EOLN + EOLN +
                         'O produto ' + FieldByName('PRO_CODIGOFAB').AsString + ' - ' + FieldByName('PRO_DESCRICAO').AsString + ' não foi encontrado no banco de destino.'+EOLN+
                         'Verifique se no cadastro do produto o códcódcódigo do fornecedor está preenchido corretamente.' +EOLN,
...
begin
  Try
    ...
    Begin
      ...
      ParamByName('PED_VL_PRODUTO').AsCurrency := 0;
      ParamByName('PED_VL_IPI').AsCurrency := 0;
      ParamByName('PED_VL_FRETE').AsCurrency := 0;
      ParamByName('PED_ALIQ_DESCONTO').AsCurrency := 0;
      ParamByName('PED_VL_DESCONTO').AsCurrency := 0;
      ParamByName('PED_VL_PEDIDO').AsCurrency := 0;
      ParamByName('PED_VL_SERVICO').AsCurrency := 0;
      ParamByName('PED_VL_ODESPESA').AsCurrency := 0;
      ParamByName('PED_VL_CREDITO').AsCurrency := 0;
      ParamByName('PED_CODIGO').AsInteger := Pc_Codigo;
      ExecSQL;
    End;
  Finally
...
Begin
  ...
  Begin
    MensagemPadrao(MENSAGEM, ATENCAO + EOLN + EOLN +
                   ...
                   'O códcódcódcódcódcódigo do produto de destino esteja referenciado'+EOLN+
                   'no campo codcódcódcódigo do fornecedor'+EOLN+
