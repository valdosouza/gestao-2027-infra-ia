...
uses
      Forms, Controls, SysUtils, STQuery, STStoredProc, STDatabase, STTransaction, StdCtrls, classes, db, TypInfo;
  ...
  procedure Pc_Imprime_Duplicata(Pc_Cd_Nota : Integer);
...
uses     Un_DM, UN_Sistema, UN_MSG, un_Padrao, Un_Regra_Negocio, Un_Fc_Sored_Procedures, Un_Funcoes, UN_RL_Duplicata, RI_MovimentoFinanceiro, RN_FormaPagto, tblCartao, ControllerCartao, Un_DadosCartaoEletronica, Un_DadosCheques, UN_Principal, env, ControllerBase;
Function Fc_SaldoDevedorCliente(Pc_cd_Cliente:Integer):Real;
Var
  Lc_Qry : TSTQuery;
...
Begin
  Try
    ...
    Begin
      ...
      while not Eof do
      Begin
        Result := Result + FieldByName('CHQ_VALOR').AsCurrency;
        Next
      end;
    End;
  Finally
...
Begin
  TRy
    ...
    Begin
      ...
      while not Eof do
      Begin
        Lc_Vl_Amortizado := Lc_Vl_Amortizado + FieldByName('CHQ_VL_AMORT').AsCurrency;
        Lc_Vl_cheque := Lc_Vl_cheque + FieldByName('CHQ_VALOR').AsCurrency;
        Next;
      end;
...
Begin
  Try
    ...
    Begin
      ...
      ParamByName('BLT_TX_DESCONTO').AsCurrency := 0;
      ParamByName('BLT_BAIXA').AsString := Pc_BAIXA;
      ParamByName('BLT_VALOR').AsCurrency := Pc_VALOR;
      ParamByName('BLT_CODQTC').asinteger := Pc_CODQTC;
      ParamByName('BLT_VL_TARIFA').AsCurrency := 0;
      ParamByName('BLT_TX_MORA').AsCurrency := 0;
...
procedure Pc_Imprime_Duplicata(Pc_Cd_Nota : Integer);
Var
  Lc_Form :TFr_RL_Duplicata;
Begin
  try
    Lc_form := TFr_RL_Duplicata.Create(nil);
    Lc_form.It_Financeiro := Pc_Cd_Nota;
