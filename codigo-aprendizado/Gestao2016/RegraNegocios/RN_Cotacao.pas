...
uses
      Forms, Classes, Controls, SysUtils, STQuery, STStoredProc, STDatabase, StdCtrls, db;
...
uses     Un_DM, UN_Sistema, UN_MSG, un_Padrao, Un_Regra_Negocio, Un_Fc_Sored_Procedures, RN_Crud;
Function Fc_AtualizaCotacao(Fc_CODIGO,
                            Fc_CODPED,
                            Fc_NUMERO,
                            Fc_TIPO,
...
Begin
  ...
  with Lc_Qry do
    ...
    ParamByName('CTC_TIPO').AsString := Fc_TIPO;
    ...
    ParamByName('CTC_FANTASIA').AsString := Fc_FANTASIA;
    ParamByName('CTC_CODFPG').AsInteger := StrToInt(Fc_CODFPG);
    ParamByName('CTC_PRAZO').AsString := Fc_PRAZO;
    ParamByName('CTC_APROVADO').AsString := Fc_APROVADO;
    ParamByName('CTC_QT_PRODUTO').AsFloat := StrToFloatDef(Fc_QT_PRODUTO,0);
    ParamByName('CTC_VL_PRODUTO').AsCurrency := StrToFloatDef(Fc_VL_PRODUTO,0);
    ParamByName('CTC_VL_SERVICO').AsCurrency := StrToFloatDef(Fc_VL_SERVICO,0);
    ParamByName('CTC_VL_FRETE').AsCurrency := StrToFloatDef(Fc_VL_FRETE,0);
    ParamByName('CTC_ALIQ_DESCONTO').AsCurrency := StrToFloatDef(Fc_ALIQ_DESCONTO,0);
    ParamByName('CTC_VL_DESCONTO').AsCurrency := StrToFloatDef(Fc_VL_DESCONTO,0);
    ParamByName('CTC_VL_COTACAO').AsCurrency := StrToFloatDef(Fc_VL_COTACAO,0);
    ParamByName('CTC_CONTATO').AsString := Fc_CONTATO;
    ParamByName('CTC_VALIDADE').AsString := Fc_VALIDADE;
    ParamByName('CTC_PRZ_ENTREGA').AsString := Fc_PRZ_ENTREGA;
    ParamByName('CTC_GARANTIA').AsString := Fc_GARANTIA;
    ParamByName('CTC_FROTA').AsString := Fc_FROTA;
    ParamByName('CTC_EQUIPAMENTO').AsString := Fc_EQUIPAMENTO;
    ParamByName('CTC_TP_FRETE').AsString := Fc_TP_FRETE;
    ParamByName('CTC_EMAIL_E').AsString := Fc_EMAIL_E;
    ParamByName('CTC_CODVDO').AsInteger := StrToInt(Fc_CODVDO);
    ParamByName('CTC_TP_CONTATO').AsString := Fc_TP_CONTATO;
    ParamByName('CTC_CODMHA').AsInteger := StrToInt(Fc_CODMHA);
    ParamByName('CTC_CONTATOEMPRESA').AsString := Fc_CONTATOEMPRESA;
    ParamByName('CTC_CODTRP').AsInteger := StrToInt(Fc_CODTRP);
    ParamByName('CTC_VL_CUSTO').AsCurrency := StrToFloatDef(Fc_VL_CUSTO,0);
    ParamByName('CTC_OBS_CORTE').AsString := Fc_OBS_CORTE;
    ParamByName('CTC_OBS').AsString := Fc_OBS;
    ParamByName('CTC_DT_ENV_EMAIL').AsString := Fc_DT_ENV_EMAIL;
    ParamByName('CTC_CODSIT').AsInteger := StrToInt(Fc_CODSIT);
    ParamByName('CTC_SOLICITANTE').AsString := Fc_SOLICITANTE;
    ParamByName('CTC_VL_ADICIONAL').AsCurrency := StrToFloatDef(Fc_VL_ADICIONAL,0);
    Try
      ExecSQL;
...
Begin
  ...
  with Lc_Qry do
    ...
    ParamByName('ICT_TIPO').AsString := FC_TIPO;
    ParamByName('ICT_CODVCL').AsInteger := StrToIntDef(FC_CODVCL,0);
    ParamByName('ICT_DESCRICAO').AsString := FC_DESCRICAO;
    ParamByName('ICT_QTDE').AsFloat := StrToFloatDef(FC_QTDE,0);
    ParamByName('ICT_VL_UNIT').AsCurrency := StrToFloatDef(FC_VL_UNIT,0);
    ParamByName('ICT_DISPON').AsString := FC_DISPON;
    ParamByName('ICT_AQ_COM').AsFloat := StrToFloatDef(FC_AQ_COM,0);
    ParamByName('ICT_VL_DESC').AsCurrency := StrToFloatDef(FC_VL_DESC,0);
    ParamByName('ICT_AQ_DESC').AsFloat := StrToFloatDef(FC_AQ_DESC,0);
    ParamByName('ICT_AQ_IPI').AsCurrency := StrToFloatDef(FC_AQ_IPI,0);
    ParamByName('ICT_AQ_ICMS').AsCurrency := StrToFloatDef(FC_AQ_ICMS,0);
    ParamByName('ICT_CODEST').AsInteger := StrToIntDef(FC_CODEST,0);
    ParamByName('ICT_CODTPR').AsInteger := StrToIntDef(FC_CODTPR,0);
    ParamByName('ICT_VL_CUSTO').AsCurrency := StrToFloatDef(FC_VL_CUSTO,0);
    ParamByName('ICT_MR_LUCRO').AsCurrency := StrToFloatDef(FC_MR_LUCRO,0);
    ParamByName('ICT_ALIQ_CT_FINANCEIRO').AsCurrency := StrToFloatDef(FC_ALIQ_CT_FINANCEIRO,0);
    ParamByName('ICT_PRZ_PRATICADO').AsString := FC_PRZ_PRATICADO;
    ParamByName('ICT_VL_PRATICADO').AsCurrency := StrToFloatDef(FC_VL_PRATICADO,0);
    ParamByName('ICT_AQ_VENDEDOR').AsFloat := StrToFloatDef(FC_AQ_VENDEDOR,0);
    ParamByName('ICT_TECIDO').AsString := FC_TECIDO;
    ParamByName('ICT_COR').AsString := FC_COR;
...
Begin
  ...
  with Lc_Itens do
    ...
    ParamByName('FTC_CODVCL').AsString := DM.Qr_Estabelecimento.FieldByname('EMP_CODIGO').AsString;
    Active := True;
    FetchAll;
    First;
    while not Eof do
      ...
      Pc_Fr_FichaTecnica('I',
                         ...
                         0,
                         0,
                         FieldByName('FTC_DESC_INSUMO').AsString,
                         'UND',
                         0,
                         0,
