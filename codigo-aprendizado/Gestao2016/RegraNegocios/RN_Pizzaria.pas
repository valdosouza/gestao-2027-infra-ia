...
uses
      Forms, Controls, SysUtils, STQuery, STStoredProc, STDatabase, StdCtrls, classes, db, RN_Itens_Vda_Avulso, ControllerCtrlEstoque;
...
uses     Un_DM, UN_Sistema, UN_MSG, un_Padrao, Un_Regra_Negocio, Un_CopiaPedidoNota, Un_Fc_Sored_Procedures, RN_Estoque, Un_EnviaPedidoNota, RN_Crud, env;
...
Begin
  ...
  with Lc_Qry do
    ...
    if (StrToIntDef(Lc_Cd_Empresa,0)>0) then
    Begin
      ParamByName('EMP_CODIGO').AsString := Lc_Cd_Empresa;
      ParamByName('EMP_NOME').AsString := Trim(Fc_NOME);
      ParamByName('EMP_FANTASIA').AsString := Trim(Fc_Nome);
      ParamByName('EMP_OBSERV').AsString := Trim(Fc_Obs_Linha_1 + #13 + Fc_Obs);
      ParamByName('EMP_ATIVA').AsString := Trim(Fc_Ativa);
    end
    else
    Begin
      Lc_Cd_Empresa := IntToStr(Fc_Generator('GN_EMPRESA','TB_EMPRESA','EMP_CODIGO'));
      ParamByName('EMP_CODIGO').AsString := Lc_Cd_Empresa;
      ParamByName('EMP_TIPO').AsString := '1';
      ParamByName('EMP_NOME').AsString := Trim(Fc_Nome);
      ParamByName('EMP_FANTASIA').AsString := Trim(Fc_Nome);
      ParamByName('EMP_OBSERV').AsString := Trim(Fc_Obs_Linha_1 + #13 + Fc_Obs);
      ParamByName('EMP_PESSOA').AsString := 'F';
      ParamByName('EMP_MULTIPLICADOR').AsString := '1';
      ParamByName('EMP_CONSUMIDOR').AsString := 'S';
      ParamByName('EMP_MICRO').AsString := 'N';
      ParamByName('EMP_ATIVA').AsString := Trim(Fc_Ativa);
    end;
      Try
        ExecSQL;
...
Begin
  ...
  Begin
    ...
    ParamByName('END_FONE').AsString := Fc_Fone;
    Active := True;
    FetchAll;
    if (RecordCount > 0) then
    Begin
      Result.Append( FieldByName('END_CODIGO').AsString);
      Result.Append( FieldByName('END_FONE').AsString);
      Result.Append( FieldByName('EMP_CODIGO').AsString);
      Result.Append( FieldByName('EMP_NOME').AsString);
      Result.Append( FieldByName('END_CEP').AsString);
      Result.Append( FieldByName('END_ENDER').AsString);
      Result.Append( FieldByName('END_NUMERO').AsString);
      Result.Append( FieldByName('END_COMPLEM').AsString);
      Result.Append( FieldByName('END_BAIRRO').AsString);
      Result.Append( FieldByName('EMP_OBSERV').AsString);
      Result.Append( FieldByName('EMP_ATIVA').AsString);
    end
    else
    Begin
      Result.Append('0');
    ...
    Begin
      ...
      begin
        if (Fields.Fields[Lc_I].DataType = ftFloat) OR (Fields.Fields[Lc_I].DataType = ftBCD) then
          ...
          Result.Append( Fields[Lc_I].AsString);
      end;
    end
    else
    Begin
...
Begin
  ...
  with Lc_Qry do
    ...
    //Passagem de Parametros
    ParamByName('ITR_CODITF').AsString := Fc_CODITF;
    ParamByName('ITR_NR_ITEM').AsString := Fc_NR_ITEM;
    ParamByName('ITR_DESCRICAO').AsString := Fc_DESCRICAO;
    ParamByName('ITR_QTDE').AsFloat := StrToFloatDef(Fc_Qtde,0);
    ParamByName('ITR_VL_UNIT').AsCurrency := StrToFloatDef(Fc_Vl_Unit,0);
    ParamByName('ITR_OBS').AsString := Fc_OBS;
    ParamByName('ITR_CODGRC').AsString := Fc_CODGRC;
    ParamByName('ITR_CODSBG').AsString := Fc_CODSBG;
    ParamByName('ITR_CORTESIA').AsString := Fc_CORTESIA;
      Try
        ExecSQL;
        if DM.IBT_Crud.InTransaction then DM.IBT_Crud.CommitRetaining;
