...
uses
      Forms, Controls, SysUtils, STQuery, STStoredProc, STDatabase, StdCtrls, classes, db, TypInfo, Un_DM, RN_CRUD;
...
Begin
  ...
  with Lc_Qry do
    ...
    ParamByName('HTB_CODIGO').AsString := Fc_CODIGO;
    ParamByName('HTB_DESCRICAO').AsString := Fc_Descricao;
      Try
        ExecSQL;
...
begin
  ...
  with lc_qry do
    ...
    SQL.Add(sql_txt);
    ParamByName('DESCRICAO').AsString:= Fc_Descricao;
    Open;
    FetchAll;
    First;
    if (RecordCount >= 1) then
      ...
      ParamByName('FPT_DESCRICAO').AsString := Fc_Descricao;
      ParamByName('FPT_BLOQUEA').AsString := 'N';
      ExecSQL;
      IF DM.IB_Transacao.InTransaction THEN DM.IB_Transacao.CommitRetaining;
      end;
    end;
