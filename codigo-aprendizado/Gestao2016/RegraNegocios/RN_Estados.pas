...
uses     Un_DM, Un_Regra_Negocio, UN_Sistema, ControllerBase;
function Fc_GetInscrSubstTributario(Fc_Estado:String):String;
Var
  Lc_Qry : TSTQuery;
...
begin
  Try
    ...
    Begin
      ...
      ParamByName('UFE_SIGLA').AsString := Fc_Estado;
      Active := True;
      FetchAll;
      if (recordcount > 0) then
        Result := FieldByName('UFE_NUMINSC_SUBS').AsString
      else
        Result := '';
    End;
  Finally
...
begin
  ...
  begin
    ...
    Gb_Mg_VA_St_Icms := Qr_Uf_Mva_NCM.FieldByname('UFE_MR_VL_AGREGADO').AsCurrency;
    //Verifica se precisamos pegar o MVA de um NCM especifico
    if Trim(Pc_NCM)<> '' then
      Begin
      IF (Qr_Uf_Mva_NCM.Locate('MUN_CODIGONCM', Pc_NCM, [])) then
        Begin
        Gb_Mg_VA_St_Icms := Qr_Uf_Mva_NCM.FieldByname('MUN_MR_VL_AGREGADO').AsCurrency;
        end;
      end;
...
begin
  Try
    ...
    Begin
      ...
      FetchAll;
      IF (Recordcount > 0) then
        Result := FieldByName('UFE_SIGLA').AsString
      else
        Result := 'PR';
    End;
  Finally
