...
uses
      Forms, Controls, SysUtils, STQuery, STStoredProc, STDatabase, StdCtrls, classes, db, RN_Empresa;
...
uses     Un_DM, UN_Sistema, UN_MSG, un_Padrao, ControllerBase, Un_Regra_Negocio, RN_Crud, Un_Funcoes, Un_Transportadora, UN_TabelasEmListas, env;
procedure Pc_AbreListaTransportadora(Pc_Order:String);
Begin
...
begin
  Try
    ...
    with Lc_Qry do
    Begin
      SQL.Add('SELECT CAST(COALESCE(SUM(tb_produto.pro_peso * itf_qtde), 0) AS DOUBLE PRECISION) PESO '+
              'from tb_itens_nfl '+
