...
uses
       SysUtils, WinTypes, WinProcs, Messages, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, ComCtrls, Spin, Grids, Inifiles, StrUtils, Mask, System.Math, ExtCtrls, STQuery, Windows, Variants, DBCtrls, DBGrids, DB, Buttons, STStoredProc, Printers, STDatabase, jpeg, Menus, QRCtrls, QuickRpt, OleCtrls, SHDocVw, QEdit_Setes, Typinfo, Registry, dbclient, Gauges, Winsock, ACBrdevice, FavoritoButtons, ShellApi, ACBrBoletoConversao, ACBrBase, ACBrBoleto, ACBrUtil, ACBrBoletoFCFortesFr;
...
uses     RN_Financeiro, Un_Funcoes, Un_Regra_Negocio, env, un_nf, un_principal, UN_Produtos, un_padrao, Un_caixa, Un_sistema, UN_DM;
procedure Pc_PreenhceDadosCedente(Pc_Banco:Integer;
                                  Pc_Agencia:String;
                                  Pc_AgenciaDv:String;
                                  Pc_ContaCorrente:String;
...
begin
  ...
  begin
    ...
    //Data da Multa utilizada pelo Itau (caso Clara Lua) mas só  utilizada quando opercentual  maior que zero
    ...
    Begin
      DataProtesto  := 0;
      //Quando não prostestar baixar e devolver em 30 dias
  ...
  Try
    ...
    Begin
      ...
      begin
        ...
        //Verificar se não dar prolbema em outros bancos.....
        NossoNumero       := FieldByName('BLT_NUMERO').AsString;
        ValorDocumento    := FieldByName('BLT_VALOR').AsCurrency + FieldByName('BLT_VL_TARIFA').AsCurrency;
        ...
        if (FieldByName('BLT_VL_TARIFA').AsCurrency > 0) then
          Mensagem.Add('Valor da Tarifa de emissão R$ ' + FloatToStrF(FieldByName('BLT_VL_TARIFA').AsCurrency,ffFixed,10,2));
        if (FieldByName('BLE_TX_JUROS').AsFloat > 0) then
          Mensagem.Add('Cobrança de '+ FloatToStrF(FieldByName('BLE_TX_JUROS').AsFloat,ffFixed,10,2) + '% Juros ao Mês ');
        if (FieldByName('BLE_TX_MORA').AsCurrency > 0) then
          Mensagem.Add('Cobrança de '+ FloatToStrF(FieldByName('BLE_TX_MORA').AsCurrency,ffFixed,10,2) + '% de Mora ao dia ');
        if (FieldByName('BLE_VL_MORA_MIN').AsCurrency > 0) then
          Mensagem.Add('Cobrança de Mora Mênima no valor de  '+ FloatToStrF(FieldByName('BLE_VL_MORA_MIN').AsCurrency,ffFixed,10,2));
        Lc_Tx_Multa := (FieldByName('BLE_TX_MULTA').AsCurrency / 100);
        if (Lc_Tx_Multa > 0) then
        begin
          Lc_VL_Aux := FieldByName('BLT_VALOR').AsCurrency;
