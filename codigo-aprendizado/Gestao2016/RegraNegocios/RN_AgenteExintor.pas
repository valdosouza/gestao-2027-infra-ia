...
uses     Un_DM, UN_Sistema, UN_MSG, un_Padrao, Un_Regra_Negocio;
Function Fc_Ag_ValorComprado(Fc_Nr_Lote:String):TStringList;
Var
  Lc_Qry:TSTQuery;
Begin
  ...
  with Lc_Qry do
    ...
    SQL.Add('select '+
            ...
            'NFL_DT_EMISSAO, '+
            'ITF_VL_UNIT , '+
            'CAST(COALESCE(SUM(MLT_QTDE), 0) AS DOUBLE PRECISION) ITF_QTDE '+
            'from tb_itens_nfl '+
            ...
            'GROUP BY 1,2,3 ');
    ParamByName('CLT_NUMERO').AsString := Fc_Nr_Lote;
    Active := True;
    Result.Add(FieldByName('NFL_NUMERO').AsString);
    Result.Add(FieldByName('ITF_VL_UNIT').AsString);
    Result.Add(FieldByName('ITF_QTDE').AsString);
    Result.Add(FieldByName('NFL_DT_EMISSAO').AsString);
    end;
  Lc_Qry.Close;
  FreeAndNil(Lc_Qry);
end;
...
Begin
  ...
  with Lc_Qry do
    ...
    SQL.Add('select '+
            'CLT_NUMERO, '+
            'CAST(COALESCE(SUM(IOS_QTDE), 0) AS DOUBLE PRECISION) IOS_VL_USADO '+
    ...
    ParamByName('CLT_NUMERO').AsString := Fc_Nr_Lote;
    Active := True;
    Result := FieldByName('IOS_VL_USADO').AsCurrency;
    end;
  Lc_Qry.Close;
  FreeAndNil(Lc_Qry);
end;
...
Begin
  ...
  with Lc_Qry do
    ...
    SQL.Add('select '+
            'CLT_NUMERO, '+
            'CAST(COALESCE(SUM(MLT_QTDE), 0) AS DOUBLE PRECISION) MLT_VL_QUEBRA '+
            'from TB_CTRL_LOTE '+
            '  INNER JOIN tb_movimento_lote '+
    ...
    ParamByName('CLT_NUMERO').AsString := Fc_Nr_Lote;
    Active := True;
    Result := FieldByName('MLT_VL_QUEBRA').AsCurrency;
    end;
  Lc_Qry.Close;
  FreeAndNil(Lc_Qry);
end;
