...
uses
      Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Un_Fm_ListaEstados, Vcl.DBCtrls, Vcl.StdCtrls, Vcl.ExtCtrls, Data.DB, STQuery, STDatabase, STTransaction, base_frame_list;
...
type
  TFm_ListaCFOP_UF = class(TFrame)
    ...
    procedure Fm_ListaBuscaEstadoDblcb_ListaClick(Sender: TObject);
    procedure Fm_ListaBuscaEstadoDblcb_ListaKeyDown(Sender: TObject;
...
begin
  ...
  Fm_ListaBuscaEstado.Dblcb_Lista.KeyValue := Fc_Estado(Festabelecimento,'S');
  Filtra_CFOP('S','E');
...
begin
  ...
  Begin
    if (Fm_ListaBuscaEstado.Dblcb_Lista.Text = 'EX') then
      Filtra_CFOP('S','X')
    else
    if (Fm_ListaBuscaEstado.Dblcb_Lista.Text = '') then
      Filtra_CFOP('S','T')
    else
    if (Fm_ListaBuscaEstado.Dblcb_Lista.KeyValue = Fc_Estado(Festabelecimento,'S')) then
      Filtra_CFOP('S','E')
    else
      Filtra_CFOP('S','N');
  end
...
procedure TFm_ListaCFOP_UF.Fm_ListaBuscaEstadoDblcb_ListaClick(Sender: TObject);
begin
  VerificaParametros;
end;

procedure TFm_ListaCFOP_UF.Fm_ListaBuscaEstadoDblcb_ListaKeyDown(
  Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  Fm_ListaBuscaEstado.Dblcb_ListaKeyDown(Sender, Key, Shift);
  if Shift = [] then
    case Key of
      VK_delete:
      begin
