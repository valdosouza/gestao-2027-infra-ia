...
uses
      Winapi.Windows, Winapi.Messages, SysUtils, Variants, Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Data.DB, STQuery, Vcl.DBCtrls, Vcl.StdCtrls, Vcl.Buttons, STDatabase, STTransaction;
...
uses     Un_DM, sea_situation, RN_Permissao, UN_Principal, env;
...
Var
  Lc_Form : TSeaSituation;
begin
  if Fc_VerificaPermissao('Fr_Situacao','Cadastro do Situação','VISUALIZAR',GB_Cd_Usuario,'S') then
  Begin
    Lc_Form := TSeaSituation.Create(nil);
    try
      Lc_Form.ShowModal;
    finally
      Listar;
      DBLCB_Situacao.KeyValue := Lc_Form.cds_searchcodigo.AsInteger;
      FreeAndNil(Lc_Form)
    end;
  End;
end;
